...
export default function OnlinePlayersPanel({ value, error, ownerPlayerId, controller, onClose, onProfile, onDirectory }: { value: ConnectedPlayers | null; error: boolean; ownerPlayerId: string; controller: FriendshipController; onClose: () => void; onProfile: (id: string) => void; onDirectory: () => void }) {
  ...
  const action = (playerId: string) => {
    ...
    if (relation.state === 'LOADING') return <AppButton disabled>Chargement…</AppButton>
  ...
  return <div className="modal-layer" onMouseDown={onClose}><section ref={ref} tabIndex={-1} className="floating-panel players-panel" role="dialog" aria-modal="true" aria-labelledby="players-title" onMouseDown={e => e.stopPropagation()}>
    ...
    <div className="players-summary">{value ? <><span className="status-dot" aria-hidden="true" />{value.total} joueur{value.total > 1 ? 's' : ''} connecté{value.total > 1 ? 's' : ''}</> : error ? 'Présence indisponible.' : 'Chargement…'}</div>
    ...
    <footer className="players-footer"><div className="social-feedback" role="status">{controller.feedbackScope === 'connected' ? controller.feedback : ''}</div><AppButton className="players-directory-button" onClick={onDirectory}>Voir tous les joueurs →</AppButton></footer>
  </section></div>
}
