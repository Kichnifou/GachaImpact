  ...
  return <aside className="chat-panel panel" aria-label="Chat global">
    ...
    <div className="chat-composer-wrap">
      ...
      <div className="chat-composer-accessory">{reply && <div className="chat-composer-reply"><span>Réponse à {reply.authorLabel}<small>« {reply.deletionState === 'ACTIVE' ? reply.content : 'Message supprimé'} »</small></span><button type="button" onClick={() => setReply(null)} aria-label="Fermer la réponse">×</button></div>}
        {!!failedIntents.length && <div className="chat-status chat-error chat-failed-status" role="alert"><span title={`${failedIntents[0]!.reason} — ${failedIntents[0]!.content}`}>{failedIntents.length > 1 ? `${failedIntents.length} envois refusés : ` : 'Envoi refusé : '}{failedIntents[0]!.content}</span><button type="button" disabled={commandPending} onClick={() => void submitIntent(failedIntents[0]!)}>Réessayer</button><button type="button" disabled={commandPending || !!draft} title={draft ? 'Terminer le brouillon courant pour récupérer ce message' : undefined} onClick={recoverFailed}>Récupérer</button><button type="button" aria-label="Fermer le feedback" onClick={() => setFailedIntents([])}>×</button></div>}
        {!!ambiguousIntents.length && !failedIntents.length && <div className="chat-status">{ambiguousIntents.length === 1 ? 'Envoi non confirmé.' : `${ambiguousIntents.length} envois non confirmés.`} <button type="button" disabled={commandPending} onClick={() => void submitIntent(ambiguousIntents[0]!)}>Réessayer</button><button type="button" aria-label="Fermer le feedback" onClick={() => setAmbiguousIntents([])}>×</button></div>}
