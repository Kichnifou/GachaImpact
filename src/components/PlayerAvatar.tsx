...
export default function PlayerAvatar({ displayName, elementKey, avatarAssetPath, className = 'mini-avatar' }: { displayName: string; elementKey: string | null; avatarAssetPath?: string | null; className?: string }) {
  ...
  const element = elementKey && ['pyro', 'hydro', 'cryo', 'electro', 'anemo', 'geo', 'dendro'].includes(elementKey) ? getElementAssetPath(elementKey) : null
  const source = custom && !failed.includes(custom) ? custom : element && !failed.includes(element) ? element : null
  const kind = source && source === custom ? 'custom' : source ? 'element' : 'initial'
  return <span className={`${className} player-avatar ${elementKey ?? ''}`} data-avatar-kind={kind} aria-hidden="true">{source ? <img src={source} alt="" onError={() => setFailed(previous => [...previous, source])} /> : Array.from(displayName.trim())[0]?.toLocaleUpperCase('fr-FR') ?? '✦'}</span>
}
