...
function GachaDetailModal({ teams, onClose, onGetHistory }: { teams: PlayerTeamsDto; onClose: () => void; onGetHistory: (page: number) => Promise<GachaHistoryDto> }) {
...
export function PassivesPanel({ teams }: { teams: PlayerTeamsDto }) {
  const activePassives = new Map((teams.teams.find(({ active }) => active)?.passives ?? []).map((passive) => [passive.elementKey, passive.stacks]))
