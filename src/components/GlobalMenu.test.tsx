...
describe('GlobalMenu', () => {
  it('renders at most nine slots, pages without resizing its structure, navigates real destinations and closes on Escape', () => {
    ...
    expect(renderedIds).toEqual(navigationDestinations.slice(0, 18).map(({ id }) => id))
    expect(new Set(renderedIds).size).toBe(renderedIds.length)
    expect(renderedIds).toContain('catalog')
    expect(renderedIds).toContain('friends')
    expect(renderedIds).not.toContain('social')
    expect(container.querySelectorAll('.global-menu-grid > button').length).toBeLessThanOrEqual(9)
    expect(container.textContent).toContain('Échanges')
    act(() => Array.from(container.querySelectorAll<HTMLButtonElement>('.global-menu-grid > button')).find((button) => button.textContent?.includes('Échanges'))!.click())
    ...
    act(() => Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Suivant')!.click())
    act(() => Array.from(container.querySelectorAll<HTMLButtonElement>('.global-menu-grid > button')).find((button) => button.textContent?.includes('Amis'))!.click())
    expect(onNavigate).toHaveBeenCalledWith('social')
  })
  it('is controlled, excludes every hidden id, and clamps an obsolete page', async () => {
    const container = document.createElement('div'); document.body.append(container); const root = createRoot(container); roots.push(root)
    const hidden = defaultNavigationPreference.order.slice(0, -9)
