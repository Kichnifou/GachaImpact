...
function BannerHero({ gacha, compact = false, showDetails = false, onSetTarget }: Props) {
  ...
  if (choosing || !selected) {
    return <section className={`invocation-panel target-picker${compact ? ' home-banner' : ''}`} aria-labelledby="invocation-title">
      ...
      <div className="target-choice-grid">{gacha.banner.featuredFiveStars.map((character) => <button type="button" className={`banner-character-card character-display-card target-choice ${character.elementKey}`} style={{ '--character-element': elementThemes[character.elementKey].color } as CSSProperties} disabled={pending !== null} onClick={() => void choose(character)} key={character.id}><GameAssetIcon className="banner-card-element character-element-badge" src={getElementAssetPath(character.elementKey, 'badge')} fallback="" /><span className="target-choice-portrait character-display-portrait"><CharacterAssetImage characterName={character.name} className="target-choice-image" assetPaths={[character.iconPath, character.fullbodyPath, character.wishPath, character.splashPath]} fallback={character.name.slice(0, 1)} alt="" /></span><span className="target-choice-copy character-display-copy"><strong>{character.name}</strong><span className="character-rarity">★★★★★</span></span></button>)}</div>
    </section>
  }
