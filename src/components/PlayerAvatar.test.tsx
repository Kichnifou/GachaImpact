...
describe('PlayerAvatar', () => {
  it('prefers an official equipped asset, then the element, then the initial on image errors', () => {
    ...
    try {
      ...
      expect(container.querySelector('img')?.getAttribute('src')).toBe('/assets/genshin/elements/pyro-badge.png')
      act(() => container.querySelector('img')!.dispatchEvent(new Event('error')))
  ...
  it('ignores external and traversal asset paths', () => {
    ...
    try {
      ...
      expect(container.querySelector('img')?.getAttribute('src')).toBe('/assets/genshin/elements/hydro-badge.png')
    } finally { act(() => root.unmount()) }
  })
})
