...
export default function DirectMessagePanel({ playerId, isActive, intent, resetToken, onIntentConsumed, onUnreadChange, onOpenProfile }: { playerId: string; isActive: boolean; intent: DirectMessageOpenIntent | null; resetToken?: string | null; onIntentConsumed: (token: string) => void; onUnreadChange: (count: number) => void; onOpenProfile: (id: string) => void }) {
  ...
  const [messageMenuId, setMessageMenuId] = useState<string | null>(null), [editingId, setEditingId] = useState<string | null>(null), [editDraft, setEditDraft] = useState(''), [deleteId, setDeleteId] = useState<string | null>(null)
  ...
  const renderMessage = (message: DirectMessageDto) => <article className={`dm-message ${message.own ? 'own' : 'other'}${message.id.startsWith('optimistic:') ? ' pending' : ''}`} data-message-id={message.id} data-submission-order={message.submissionOrder ?? undefined} data-read-by-other={message.readByOther ? 'true' : 'false'} data-read-by-other-at={message.readByOtherAt ?? undefined} key={message.id}>
    {editingId === message.id ? <div className="dm-message-edit"><textarea aria-label="Modifier le message" value={editDraft} disabled={messageActionPending === message.id} onChange={event => setEditDraft(Array.from(event.target.value).slice(0, 1000).join(''))} /><small>{Array.from(editDraft).length} / 1 000</small><span><button type="button" disabled={messageActionPending === message.id || !editDraft.trim()} onClick={() => void saveEdit(message.id)}>Sauvegarder</button><button type="button" disabled={messageActionPending === message.id} onClick={() => { setEditingId(null); setEditDraft('') }}>Annuler</button></span></div> : <><div className={`dm-message-bubble${message.deletedAt ? ' deleted' : ''}`}>{message.content ? <p>{linkedText(message.content)}</p> : <p>Message supprimé</p>}{message.editedAt && !message.deletedAt && <small className="dm-message-edited">Modifié</small>}</div>{message.own && !message.id.startsWith('optimistic:') && <div className="dm-message-actions"><button type="button" aria-label="Actions pour ce message" aria-expanded={messageMenuId === message.id} disabled={messageActionPending === message.id} onClick={() => setMessageMenuId(value => value === message.id ? null : message.id)}>⋯</button>{messageMenuId === message.id && <div role="menu">{message.deletedAt ? <button type="button" role="menuitem" onClick={() => { setMessageMenuId(null); void mutateMessage({ kind: 'restore', messageId: message.id, key: crypto.randomUUID() }) }}>Restaurer</button> : <><button type="button" role="menuitem" onClick={() => { setEditingId(message.id); setEditDraft(message.content ?? ''); setMessageMenuId(null) }}>Modifier</button><button type="button" role="menuitem" onClick={() => { setDeleteId(message.id); setMessageMenuId(null) }}>Supprimer</button></>}</div>}</div>}{deleteId === message.id && <div className="dm-message-delete-confirm" role="dialog" aria-label="Confirmer la suppression"><p>Supprimer ce message ?</p><button type="button" disabled={messageActionPending === message.id} onClick={async () => { const result = await mutateMessage({ kind: 'delete', messageId: message.id, key: crypto.randomUUID() }); if (result !== 'deterministic') setDeleteId(null) }}>Confirmer</button><button type="button" disabled={messageActionPending === message.id} onClick={() => setDeleteId(null)}>Annuler</button></div>}</>}
  </article>
