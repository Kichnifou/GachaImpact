...
export default function DirectMessagePanel({ playerId, isActive, intent, onIntentConsumed, onUnreadChange, onOpenProfile }: { playerId: string; isActive: boolean; intent: DirectMessageOpenIntent | null; onIntentConsumed: (token: string) => void; onUnreadChange: (count: number) => void; onOpenProfile: (id: string) => void }) {
  ...
  const intentRef = useRef<SendIntent | null>(null), list = useRef<HTMLDivElement>(null), composer = useRef<HTMLTextAreaElement>(null), atBottom = useRef(true), initialScroll = useRef(false), seenIds = useRef(new Set<string>()), prepend = useRef<{ top: number; height: number } | null>(null), readId = useRef<string | null>(null), sendBusy = useRef(false), readBusy = useRef(false), queuedRead = useRef<{ conversationId: string; messageId: string } | null>(null)
  ...
  useLayoutEffect(() => {
    if (!list.current || view !== 'conversation') return
    if (initialScroll.current && model.messagesLoaded) { list.current.scrollTop = list.current.scrollHeight; initialScroll.current = false; atBottom.current = true; seenIds.current = new Set(model.messages.map(message => message.id)); setNewCount(0) }
      ...
      if (fresh.length && atBottom.current) list.current.scrollTop = list.current.scrollHeight
