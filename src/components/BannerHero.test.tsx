...
describe('BannerHero real Gacha state', () => {
  it('offers exactly four showcase targets without featured four-star cards', () => {
    ...
    expect((html.match(/class="large-character-portrait character-display-portrait"/g) ?? [])).toHaveLength(4)
  ...
  it('shows the selected hero hierarchy, real zero state and disabled pull controls', () => {
    ...
    expect((html.match(/featured-character-card/g) ?? [])).toHaveLength(6)
    expect((html.match(/class="character-card-topline"/g) ?? [])).toHaveLength(6)
    expect((html.match(/class="character-portrait"/g) ?? [])).toHaveLength(6)
    expect((html.match(/class="character-card-copy"/g) ?? [])).toHaveLength(6)
    ...
    expect(html).not.toContain('Épée')
    expect(html).not.toContain('Fontaine')
    for (const featured of four) expect(html).toContain(`<h3>${featured.name}</h3>`)
    expect((html.match(/<div class="character-rarity">★★★★<\/div>/g) ?? [])).toHaveLength(6)
