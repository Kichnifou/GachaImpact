...
export function createGameApiClient(dependencies: ApiClientDependencies) {
  ...
  return {
    ...
    directMessages: {
      ...
      send: (conversationId: string, content: string, idempotencyKey: string, replyToMessageId: string | null = null) => request<DirectMessageSendDto>(`/api/v1/me/direct-conversations/${encodeURIComponent(conversationId)}/messages`, { method: 'POST', body: JSON.stringify({ content, idempotencyKey, replyToMessageId }) }),
