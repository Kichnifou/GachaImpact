...
describe('game API client', () => {
  ...
  it('loads the authenticated Player permanent Missions without client identity input', async () => {
    const payload = { catchUpApplied: false, ranks: { B: [], A: [], S: [] }, z: { status: 'LOCKED' } }
    ...
    await expect(client.getMissions()).resolves.toEqual(payload)
  ...
  it('forwards an abort signal to partner discovery', async () => {
    const fetchImplementation = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ partners: [], page: 2, pageSize: 10, total: 0, totalPages: 1 })))
    const client = createGameApiClient({ baseUrl: 'http://127.0.0.1:3001', getAccessToken: async () => 'token', fetchImplementation })
    const controller = new AbortController()
    await client.trades.partners('Éloïse', 2, controller.signal)
    expect(fetchImplementation.mock.calls[0]?.[0]).toContain('/api/v1/me/trades/partners?q=%C3%89lo%C3%AFse&page=2')
  ...
  it('uses the personal Bank endpoints, including paginated history, and preserves future MAX support', async () => {
    ...
    expect(fetchImplementation.mock.calls.map(([url, init]) => [url, init?.method, init?.body ? JSON.parse(String(init.body)) : null])).toEqual([
      ...
      ['http://127.0.0.1:3001/api/v1/me/bank/history?page=2', undefined, null],
