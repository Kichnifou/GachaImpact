...
describe('navigation shell registry', () => {
  ...
  it('adds Codes near the economic destinations without changing the seven main tiles', () => {
    expect(navigationDestinations.map(({ id }) => id)).toEqual(['home', 'invocation', 'box', 'team', 'catalog', 'activities', 'dailies', 'missions', 'combat', 'event', 'contest', 'inventory', 'shop', 'bank', 'codes', 'history', 'tutorial', 'configuration'])
    expect(new Set(navigationDestinations.map(({ id }) => id)).size).toBe(18)
  })
})
