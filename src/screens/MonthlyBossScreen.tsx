...
import type { DailyCombatDto, MonthlyBossAttackDto, MonthlyBossDto, MonthlyBossHistoryDto, MonthlyBossHistoryEntryDto, MonthlyBossRecordsDto } from '../api/types'
...
export default function MonthlyBossScreen({ value, dailyCombat, box, onSetSlot, onRemoveSlot, onCopyActive, onClear, onAttack, onLoadHistory }: Props) {
  ...
  return <div className="monthly-boss-shell compact">
    <section className={`panel boss-identity-panel ${value.status.toLowerCase()}`}>
      ...
      <div className="boss-compact-title"><h2>{value.boss.name}</h2><div><strong className="boss-state-badge">{status}</strong><span>Res :</span><span className="boss-resistance-accessible" role="img" aria-label={`Résistance ${elementLabels[value.boss.resistanceElementKey]} — dégâts ×0,5`} title={`Résistance ${elementLabels[value.boss.resistanceElementKey]} — dégâts ×0,5`}><GameAssetIcon className="boss-element-icon" src={getElementAssetPath(value.boss.resistanceElementKey)} fallback="✦" /></span></div></div>
    ...
    <section className="panel boss-command-panel compact" aria-label="Attaque du jour">
      ...
      <div className="boss-preview"><small>Dégâts prévus</small><strong>{value.preview ? formatResourceAmount(value.preview.totalDamage) : '—'}</strong><button type="button" disabled={!value.preview} onClick={() => setCalculationOpen(true)}>Détails →</button></div>
...
function BossBilanModal({ value, onLoadHistory, onClose }: { value: MonthlyBossDto; onLoadHistory: (page: number) => Promise<MonthlyBossHistoryDto>; onClose: () => void }) {
  ...
  return <div className="history-modal-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="panel history-modal boss-bilan-modal" role="dialog" aria-modal="true" aria-label="Bilan Boss"><header><div><span className="eyebrow">{formatMonth(value.boss.monthStart)}</span><h2>Bilan Boss</h2></div><button type="button" onClick={onClose} aria-label="Fermer">✕</button></header><nav className="boss-bilan-tabs" aria-label="Sections du bilan"><button type="button" className={tab === 'contribution' ? 'active' : ''} onClick={() => setTab('contribution')}>Contribution</button><button type="button" className={tab === 'stats' ? 'active' : ''} onClick={() => setTab('stats')}>Vos statistiques Boss</button><button type="button" className={tab === 'history' ? 'active' : ''} onClick={() => { setTab('history'); if (!history && !pending) void loadPage(1) }}>Historique</button></nav><div className="history-modal-body boss-bilan-body">
    {tab === 'contribution' && (value.status === 'DEFEATED' && value.defeatedSummary ? <DefeatedSummary value={value} /> : <section className="panel boss-ranking"><header><div><span className="eyebrow">Contribution</span><h2>Classement du mois</h2></div>{value.participation && <strong>Votre place : #{value.participation.rank}</strong>}</header>{value.ranking.length ? <RankingList entries={value.ranking} /> : <p>Aucune participation pour le moment.</p>}</section>)}
