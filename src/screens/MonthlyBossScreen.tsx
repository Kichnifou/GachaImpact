...
export default function MonthlyBossScreen({ value, dailyCombat, box, onSetSlot, onRemoveSlot, onCopyActive, onClear, onAttack, onLoadHistory }: Props) {
  ...
  return <div className="monthly-boss-shell compact">
    <section className={`panel boss-identity-panel ${value.status.toLowerCase()}`}>
      ...
      <div className="boss-compact-title"><h2>{value.boss.name}</h2><div className="boss-identity-status"><strong className="boss-state-badge">{status}</strong><span className="boss-resistance-label">Res :</span><span className="boss-resistance-accessible" role="img" aria-label={`Résistance ${elementLabels[value.boss.resistanceElementKey]} — dégâts ×0,5`} title={`Résistance ${elementLabels[value.boss.resistanceElementKey]} — dégâts ×0,5`}><GameAssetIcon className="boss-element-icon" src={getElementAssetPath(value.boss.resistanceElementKey)} fallback="✦" /></span></div></div>
...
function BossPicker({ value, position, selectedIds, pending, onClose, onSelect }: { value: MonthlyBossDto; position: number; selectedIds: ReadonlySet<string>; pending: boolean; onClose: () => void; onSelect: (id: string) => void }) {
  ...
  return <div className="modal-layer" role="presentation" onMouseDown={onClose}><section ref={dialogRef} tabIndex={-1} className="floating-panel boss-picker" role="dialog" aria-modal="true" aria-labelledby="boss-picker-title" onMouseDown={(event) => event.stopPropagation()}><header className="floating-panel-heading"><div><span className="eyebrow">Emplacement {position}</span><h2 id="boss-picker-title">Choisir un personnage</h2></div><ModalCloseButton onClose={onClose} /></header><div className="boss-picker-grid">{value.availableCharacters.map((character) => { const used = selectedIds.has(character.id) && character.id !== currentId; return <button type="button" disabled={pending || used} onClick={() => onSelect(character.id)} key={character.id}><span>{character.name.slice(0, 1)}</span><strong>{character.name}</strong><small>{'★'.repeat(character.rarity)} · C{character.constellation}{used ? ' · Déjà sélectionné' : ''}</small></button> })}</div></section></div>
}
