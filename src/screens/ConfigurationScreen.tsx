...
type DropIntent = Readonly<{ target: NavigationMenuDestinationId; mode: 'before' | 'swap' | 'after' }>

function reorder(order: readonly NavigationMenuDestinationId[], dragged: NavigationMenuDestinationId, intent: DropIntent) {
  const next = [...order]
  const from = next.indexOf(dragged); const to = next.indexOf(intent.target)
  if (from < 0 || to < 0 || from === to) return next
  if (intent.mode === 'swap') { [next[from], next[to]] = [next[to]!, next[from]!]; return next }
  next.splice(from, 1)
  const targetAfterRemoval = next.indexOf(intent.target)
  next.splice(targetAfterRemoval + (intent.mode === 'after' ? 1 : 0), 0, dragged)
  return next
}
...
function ConfigurationScreen({ preference, onSave, onReset }: { preference: NavigationMenuPreferenceDto; onSave: (value: NavigationMenuPreferenceDto) => Promise<void>; onReset: () => Promise<void> }) {
  ...
  const previewDrop = (event: DragEvent, id: NavigationMenuDestinationId) => {
    if (!dragged || pending || dragged === id) return
    event.preventDefault()
    const rectangle = event.currentTarget.getBoundingClientRect()
    const ratio = rectangle.height > 0 ? (event.clientY - rectangle.top) / rectangle.height : .5
    const nextIntent: DropIntent = { target: id, mode: ratio < .25 ? 'before' : ratio > .75 ? 'after' : 'swap' }
    setDropIntent(nextIntent)
    replaceDraft({ ...dragBase.current, order: reorder(dragBase.current.order, dragged, nextIntent) })
  }
  const commitDrop = (event: DragEvent) => { if (!dragged || pending || !dropIntent) return; event.preventDefault(); dropped.current = true; const next = draftRef.current; const rollback = confirmedRef.current; setDragged(null); setDropIntent(null); if (next.order.join('|') !== rollback.order.join('|')) void persist(next, rollback); else replaceDraft(rollback) }
