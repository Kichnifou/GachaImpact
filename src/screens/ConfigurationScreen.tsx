...
function ConfigurationScreen({ preference, onSave, onReset, socialActions, initialTab = 'menu' }: { socialActions?: SocialActions; initialTab?: 'menu' | 'privacy'; preference: NavigationMenuPreferenceDto; onSave: (value: NavigationMenuPreferenceDto) => Promise<void>; onReset: () => Promise<void> }) {
  ...
  return <div className="screen-content configuration-screen long-screen-layout">
    ...
    <nav className="configuration-tabs" aria-label="Sections Configuration"><button type="button" className={tab === 'menu' ? 'active' : ''} aria-current={tab === 'menu' ? 'page' : undefined} onClick={() => setTab('menu')}>Menu</button><button type="button" className={tab === 'privacy' ? 'active' : ''} aria-current={tab === 'privacy' ? 'page' : undefined} disabled={!socialActions} onClick={() => setTab('privacy')}>Confidentialité</button><button type="button" disabled>Apparence</button></nav>
