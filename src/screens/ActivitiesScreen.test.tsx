...
const shared: SharedProps = { sessionUserId: 'player-1', wheelToday: { spun: false, businessDate: '2026-09-11', result: null }, onSpinWheel: vi.fn(), dailyRewardToday: { claimed: false, businessDate: '2026-09-11', rewards: { primogems: '800', moras: '50000', mainElementParticles: '500' } }, dailyChallenge: availableChallenge, elementKey: 'hydro', onClaimDailyReward: vi.fn(), onPurchaseDailyChallenge: vi.fn(), onSwitchDailyChallenge: vi.fn(), onOpenParticleConversion: vi.fn(), onNavigate: vi.fn() }
const eventBeforeJoin: EventDto = { businessDate: '2026-09-15', refreshAfterMs: 3600000, festival: { key: 'harvest', month: 9, title: 'Festival des Récoltes', emoji: '🌾', currency: { key: 'harvest-tokens', label: 'Jetons de Récolte', unit: 'Jeton de Récolte', emoji: '🌾' }, collection: { key: 'harvest-sheaf', label: 'Gerbe de Récolte' } }, edition: { id: 'edition-2026', year: 2026, startsAt: '2026-08-31T22:00:00.000Z', endsAt: '2026-09-30T22:00:00.000Z' }, participation: { joined: false, joinedAt: null, points: 0 }, currency: { amount: '0' }, shop: { available: false, balance: '0', rates: { primogems: '160', moras: '20000' }, collection: { itemExternalKey: 'gerbe_de_recolte', label: 'Gerbe de Récolte', cost: '80', obtainedThisEdition: false, available: true } }, canJoin: true, dailyBonus: { claimedToday: false, canClaim: false }, milestones: { currentPoints: 0, thresholds: [] }, gameA: { available: false, theme: { key: 'recolte', label: 'Récolte' }, completedToday: false, attemptsToday: 0, windows: [], activeWindowIndex: null, canAttempt: false, cooldownRemainingMs: 0 }, gameB: { available: false, theme: { key: 'harvest', label: 'Festival des Récoltes' }, solvedToday: false, resolvedCode: null, discoveredBy: null, attemptsUsed: 0, attemptsRemaining: 0, testedCodes: [], remainingCodes: Array.from({ length: 32 }, (_, index) => index.toString(2).padStart(5, '0')), canAttempt: false }, gameC: { available: false, theme: { key: 'harvest', label: 'Panier' }, sentToday: false, canSend: false, receivedMessages: [], unviewedCount: 0 } }
const eventAfterJoin: EventDto = { ...eventBeforeJoin, participation: { joined: true, joinedAt: '2026-09-15T12:00:00.000Z', points: 0 }, currency: { amount: '1' }, canJoin: false, dailyBonus: { claimedToday: false, canClaim: true }, gameA: { ...eventBeforeJoin.gameA, available: true, windows: [{ startAt: '2026-09-15T18:00:00.000Z', endAt: '2026-09-15T19:00:00.000Z', state: 'FUTURE' }], canAttempt: false }, gameB: { ...eventBeforeJoin.gameB, available: true, attemptsRemaining: 3, canAttempt: true }, gameC: { ...eventBeforeJoin.gameC, available: true, canSend: true } }
...
describe('Activities shells', () => {
  ...
  it('shows Event access only while a real action remains and restores it from the next-day snapshot', () => {
    ...
    expect(afterCard.textContent).toContain('Jeu du jour disponible · 1 Jeton de Récolte')
    expect(afterCard.querySelector('.daily-overview-complete')).toBeNull()
    ...
    expect(completeCard.textContent).toContain('Énigme du jour disponible · 1 Jeton de Récolte')
    expect(completeCard.querySelector<HTMLButtonElement>('button')?.textContent).toBe('Accéder')
    ...
    expect(expiredCard.textContent).toContain('Énigme du jour disponible · 1 Jeton de Récolte')
    expect(expiredCard.querySelector<HTMLButtonElement>('button')?.textContent).toBe('Accéder')
    const noAttempts = mount({ event: { ...eventAfterJoin, gameA: { ...eventAfterJoin.gameA, windows: eventAfterJoin.gameA.windows.map((window) => ({ ...window, state: 'PAST' as const })) }, gameB: { ...eventAfterJoin.gameB, attemptsUsed: 3, attemptsRemaining: 0, canAttempt: false } } })
    expect(activity(noAttempts.container, 'Événement').querySelector<HTMLButtonElement>('button')?.textContent).toBe('Accéder')
    const solved = mount({ event: { ...eventAfterJoin, gameA: { ...eventAfterJoin.gameA, completedToday: true }, gameB: { ...eventAfterJoin.gameB, solvedToday: true, canAttempt: false } } })
    expect(activity(solved.container, 'Événement').querySelector<HTMLButtonElement>('button')?.textContent).toBe('Accéder')
    const allDone = mount({ event: { ...solved.props.event!, gameC: { ...solved.props.event!.gameC, sentToday: true, canSend: false } } })
    expect(activity(allDone.container, 'Événement').querySelector<HTMLButtonElement>('button')?.textContent).toBe('Accéder')
    const bonusClaimed = mount({ event: { ...allDone.props.event!, dailyBonus: { claimedToday: true, canClaim: false } } })
    const doneCard = activity(bonusClaimed.container, 'Événement')
    expect(doneCard.querySelector('.daily-overview-action-slot')).toBeNull()
    expect(doneCard.querySelector('.daily-overview-complete')?.textContent).toBe('✅ Terminé')
    expect(doneCard.textContent).toContain('1 Jeton de Récolte')
    expect(doneCard.textContent).not.toContain('1 Jetons de Récolte')
  ...
  it('keeps an honest fallback if the Mission loader is unavailable', () => { const { container } = mount({}, 'activities-missions'); ['B', 'A', 'S', 'Z'].forEach((label) => expect(container.textContent).toContain(label)); expect(container.textContent).toContain('Missions indisponibles'); expect(container.textContent).not.toContain('Bientôt disponible') })
