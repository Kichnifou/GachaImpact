...
describe('Activities shells', () => {
  ...
  it.each([
    ['TODO', [], 'À faire', 'Aucune tentative aujourd’hui.', true],
    ['IN_PROGRESS', ['a', 'b', 'c', 'd'], 'En cours', '4 personnages KO.', true],
    ['BLOCKED', ['a', 'b', 'c', 'd'], 'Bloqué aujourd’hui', 'Moins de 4 personnages non-KO sont disponibles.', true],
    ['COMPLETED', [], '✅ Terminé', 'Victoire obtenue aujourd’hui.', false],
  ] as const)('projects Combat %s honestly in Quotidiennes', (status, kos, label, detail, hasAccess) => { const { container } = mount({ dailyCombat: combat(status, kos) }); const card = activity(container, 'Combat'); expect(card.textContent).toContain(label); expect(card.textContent).toContain(detail); expect(Boolean(card.querySelector('button'))).toBe(hasAccess); if (status === 'COMPLETED') expect(card.textContent).toContain('Obtenu : +800 Primogemmes · +20 000 Moras') })
  it.each([
