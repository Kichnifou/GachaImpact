...
const eventBeforeJoin: EventDto = { businessDate: '2026-09-15', festival: { key: 'harvest', month: 9, title: 'Festival des Récoltes', emoji: '🌾', currency: { key: 'harvest-tokens', label: 'Jetons de Récolte', emoji: '🌾' }, collection: { key: 'harvest-sheaf', label: 'Gerbe de Récolte' } }, edition: { id: 'edition-2026', year: 2026, startsAt: '2026-08-31T22:00:00.000Z', endsAt: '2026-09-30T22:00:00.000Z' }, participation: { joined: false, joinedAt: null, points: 0 }, currency: { amount: '0' }, canJoin: true }
const eventAfterJoin: EventDto = { ...eventBeforeJoin, participation: { joined: true, joinedAt: '2026-09-15T12:00:00.000Z', points: 0 }, currency: { amount: '1' }, canJoin: false }
...
describe('Activities shells', () => {
  ...
  it('keeps Event access before join and hides it after join without hiding the real summary', () => {
    ...
    expect(afterCard.textContent).toContain('Inscrit · 1 Jetons de Récolte')
    expect(afterCard.querySelector('button')).toBeNull()
    expect(afterCard.querySelector('.daily-overview-action-slot')).toBeNull()
  })
  ...
  it('renders the real Event surface with disabled structural tabs and no future copy', () => { const { container } = mount({ event: eventBeforeJoin, onLoadEvent: vi.fn(async () => eventBeforeJoin), onJoinEvent: vi.fn() }, 'activities-event'); const tabs = Array.from(container.querySelectorAll<HTMLButtonElement>('.event-tabs button')); expect(tabs.map(({ textContent }) => textContent)).toEqual(['Jeux', 'Shop', 'Classement']); expect(tabs.every(({ disabled }) => disabled)).toBe(true); expect(container.textContent).not.toMatch(/À venir|Bientôt disponible|La suite du Festival/) })
