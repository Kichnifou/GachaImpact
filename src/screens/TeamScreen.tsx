...
import type { BoxCharacterDto, DailyCombatDto, PlayerBoxDto, PlayerTeamDto, PlayerTeamsDto, StellaUseDto, TeamCharacterDto } from '../api/types'
...
import { canOpenNextTeamPage, filterTeamCharacters, insertTeamOrder, swapTeamOrder, swapTeamSlots, teamElementFilters, teamPageForPosition, teamsPerPage, type TeamElementFilter } from '../team/team-presentation'
...
function TeamBoxCharacterDetail({ characterId, dailyCombat, initialBox, refreshToken, stellaRetryCharacterId, onLoadBox, onSetBoxFavorite, onUseStella, onLoad, onClose }: Pick<TeamScreenProps, 'dailyCombat' | 'initialBox' | 'refreshToken' | 'stellaRetryCharacterId' | 'onLoadBox' | 'onSetBoxFavorite' | 'onUseStella' | 'onLoad'> & { characterId: string; onClose: () => void }) { const detail = useBoxCollection({ initialBox, refreshToken, onLoadBox, onSetFavorite: onSetBoxFavorite, onUseStella, stellaRetryCharacterId, onCharacterProgressed: onLoad }); const character = detail.box?.characters.find(({ id }) => id === characterId) ?? null; const combatCharacter = dailyCombat?.availableCharacters.find(({ id }) => id === characterId); const combatState = dailyCombat && combatCharacter ? { ko: dailyCombat.koCharacterIds.includes(characterId), stats: combatCharacter.combatStats } : undefined; if (!detail.box && !detail.error) return <TeamDetailStatus title="Ouverture de la fiche…" detail="Chargement de votre possession." onClose={onClose} />; if (!detail.box || !character) return <TeamDetailStatus title="Fiche indisponible" detail={detail.error ?? 'Ce personnage ne figure plus dans votre Box.'} onClose={onClose} />; return <BoxCharacterDetailModal character={character} combatState={combatState} stellaQuantity={detail.box.stella.quantity} stellaRetryAvailable={detail.stellaRetryId === character.id} favoritePending={detail.favoritePendingId === character.id} stellaPending={detail.stellaPendingId === character.id} stellaFeedback={detail.stellaFeedback} actionError={detail.error} onToggleFavorite={() => void detail.toggleFavorite(character)} onUseStella={() => void detail.useStella(character)} onClose={onClose} /> }
