...
function TeamScreen(props: TeamScreenProps) {
  ...
  return <div className="screen-content team-screen">
    ...
    <section className="panel team-bonuses"><div className="team-bonus-content"><div className="section-heading"><span>Passifs de cette Team</span><small>{teamPassiveStatusLabel(selectedTeam.active)}</small></div>{selectedTeam.passives.length > 0 ? <div className={`bonus-grid passive-count-${selectedTeam.passives.length}`}>{selectedTeam.passives.map((passive) => <article key={passive.elementKey}><GameAssetIcon className={`${passive.elementKey} bonus-element-icon`} src={getElementAssetPath(passive.elementKey)} fallback="✦" /><div><strong>{passive.displayName} {roman(passive.stacks)} — </strong><span>{passive.description}</span></div></article>)}</div> : <p className="team-no-passive">Aucun passif actif</p>}</div><button type="button" className="team-reference-toggle" onClick={() => setShowReference(true)}>Voir les passifs</button></section>
