...
export default function ProfileScreen({ playerId, ownerPlayerId, actions, controller, onDirectory, onPrivacy }: { playerId: string; ownerPlayerId: string; actions: SocialActions; controller: FriendshipController; onDirectory: () => void; onPrivacy: () => void }) {
  ...
  const socialActions = relation.state === 'SELF' ? null : <div className="profile-social-actions">
    ...
    {relation.state === 'FRIEND' && <><AppButton disabled={controller.pending || !relation.friend?.canSend} onClick={() => void controller.mutate(playerId, 'HEART', undefined, feedbackScope)}>{relation.friend?.heartSent ? 'Cœur envoyé ✓' : 'Envoyer un cœur'}</AppButton><AppButton variant="danger" disabled={controller.pending} onClick={() => void controller.mutate(playerId, 'REMOVE', undefined, feedbackScope)}>Retirer</AppButton></>}
  </div>
  return <div className="screen-content profile-screen long-screen-layout">
    ...
    <ScrollableScreenPanel className="profile-frame" fixed={<><div className="profile-navigation"><AppButton onClick={onDirectory}>Joueurs</AppButton>{value?.own && <AppButton onClick={onPrivacy}>Confidentialité</AppButton>}{socialActions}</div><div className="social-feedback" role="status">{controller.feedbackScope === feedbackScope ? controller.feedback : ''}</div><nav className="secondary-navigation" aria-label="Rubriques du profil">{tabs.map(t => <AppButton key={t} className={tab === t ? 'active' : ''} aria-pressed={tab === t} onClick={() => { controller.clearFeedback(); setTab(t) }}>{t}</AppButton>)}</nav>{tab === 'Box' && value?.box.access === 'ALLOWED' && <CollectionFilters placeholder="Rechercher un personnage…" query={query} rarity={rarity} element={element} sortKey={sortKey} direction={direction} onQueryChange={setQuery} onRarityChange={setRarity} onElementChange={setElement} onSortKeyChange={setSortKey} onDirectionChange={() => setDirection(direction === 'asc' ? 'desc' : 'asc')} />}</>}>
      {error ? <p role="alert">{error}</p> : !value ? <p role="status">Chargement du profil…</p> : <>
        {tab === 'Aperçu' && <div className="profile-overview"><div className="profile-identity"><span className={`mini-avatar ${value.player.elementKey ?? ''}`} aria-hidden="true">{value.player.displayName.slice(0, 1).toUpperCase()}</span><div className="profile-identity-copy"><h2>{value.player.displayName}</h2><p className="profile-presence">{value.presence.access === 'ALLOWED' ? <><span className={`presence-dot presence-${value.presence.data.toLowerCase()}`} aria-hidden="true" />{presenceLabels[value.presence.data]}</> : 'Présence privée'}</p><p>Niveau {value.player.level}</p><p className="profile-element">{value.player.elementKey ? <><GameAssetIcon src={getElementAssetPath(value.player.elementKey)} fallback="✦" />{elementLabels[value.player.elementKey]}</> : 'Élément non choisi'}</p></div></div><div className="profile-activity-section"><LastActivity value={value.lastActivity} /></div></div>}
