...
export default function ProfileScreen({ playerId, ownerPlayerId, actions, controller, onDirectory, onRankings = () => undefined, onPrivacy, onMessage, onTrade, onAppearanceChanged = async () => undefined, initialTab = 'Aperçu' }: { playerId: string; ownerPlayerId: string; actions: SocialActions; controller: FriendshipController; onDirectory: () => void; onRankings?: () => void; onPrivacy: () => void; onMessage?: (player: { id: string; displayName: string; elementKey: ElementKey | null }) => void; onTrade?: (player: { id: string; displayName: string }) => void; onAppearanceChanged?: () => Promise<void>; initialTab?: typeof tabs[number] }) {
  const [value, setValue] = useState<Profile | null>(null), [error, setError] = useState(''), [tab, setTab] = useState<typeof tabs[number]>(initialTab)
  ...
  return <div className="screen-content profile-screen long-screen-layout">
    ...
    <ScrollableScreenPanel className="profile-frame" fixed={<><div className="profile-navigation"><AppButton onClick={onDirectory}>Joueurs</AppButton>{tab === 'Statistiques' && <AppButton onClick={onRankings}>Classements</AppButton>}{value?.own && <AppButton onClick={onPrivacy}>Confidentialité</AppButton>}</div><nav className="secondary-navigation" aria-label="Rubriques du profil">{tabs.filter(t => t !== 'Personnalisation' || playerId === ownerPlayerId).map(t => <AppButton key={t} className={tab === t ? 'active' : ''} aria-pressed={tab === t} onClick={() => { controller.clearFeedback(); if (t === 'Missions' && tab !== 'Missions') setMissionsTarget(''); setTab(t) }}>{t}</AppButton>)}</nav>{tab === 'Box' && value?.box.access === 'ALLOWED' && <CollectionFilters placeholder="Rechercher un personnage…" query={query} rarity={rarity} element={element} sortKey={sortKey} direction={direction} onQueryChange={setQuery} onRarityChange={setRarity} onElementChange={setElement} onSortKeyChange={setSortKey} onDirectionChange={() => setDirection(direction === 'asc' ? 'desc' : 'asc')} />}</>}>
      {error ? <p role="alert">{error}</p> : !value ? <p role="status">Chargement du profil…</p> : <>
        {tab === 'Aperçu' && <div className="profile-overview"><div className="profile-identity"><PlayerAvatar {...value.player} /><div className="profile-identity-copy"><div className="profile-name-line"><h2>{value.player.displayName}</h2><span className="profile-element" role="img" aria-label={value.player.elementKey ? `Élément ${elementLabels[value.player.elementKey]}` : 'Élément non choisi'}>{value.player.elementKey && <GameAssetIcon src={getElementAssetPath(value.player.elementKey)} fallback="✦" />}</span></div>{value.player.title && <p className="profile-equipped-title">{value.player.title}</p>}<p className="profile-presence">{value.presence.access === 'ALLOWED' ? <><span className={`presence-dot presence-${value.presence.data.toLowerCase()}`} aria-hidden="true" />{presenceLabels[value.presence.data]}</> : 'Présence privée'}</p><p>Niveau {value.player.level}</p>{socialActions}<div className="social-feedback" role="status">{controller.feedbackScope === feedbackScope ? controller.feedback : ''}</div></div></div><div className="profile-activity-section"><LastActivity value={value.lastActivity} /></div></div>}
        {tab === 'Personnalisation' && value.own && <AppearancePanel actions={actions} displayName={value.player.displayName} elementKey={value.player.elementKey} onChanged={refreshAppearance} />}
