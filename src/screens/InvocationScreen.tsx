...
function InvocationScreen({ gacha, teams, onSetTarget, onPull, pendingPullCount, onPresentationDisclosed, onGetHistory, onOpenHistory }: { gacha: CurrentGachaDto; teams: PlayerTeamsDto; onSetTarget: (id: string) => Promise<void>; onPull: (count: 1 | 10) => Promise<GachaPullDto>; pendingPullCount: 1 | 10 | null; onPresentationDisclosed: (operationId: string) => void; onGetHistory: (page: number) => Promise<GachaHistoryDto>; onOpenHistory?: () => void }) {
  return (
    <div className="screen-content invocation-screen">
      ...
      {onOpenHistory && <button type="button" className="small-primary-button" onClick={onOpenHistory}>Historique des invocations</button>}
    </div>
  )
}
