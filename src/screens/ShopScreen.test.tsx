...
describe('ShopScreen', () => {
  ...
  it('uses the same options and footer structure for both responsive product cards', async () => { const { container } = await mount(); const cards = Array.from(container.querySelectorAll<HTMLElement>('.shop-item')); expect(cards).toHaveLength(2); for (const card of cards) { expect(card.querySelector(':scope > .shop-card-options')).not.toBeNull(); expect(card.querySelector(':scope > .shop-card-controls')?.children).toHaveLength(3) }; expect(cards[0].querySelector('.shop-card-options > .shop-quantity')).not.toBeNull(); expect(cards[1].querySelector('.shop-card-options > .shop-ticket-odds')).not.toBeNull(); expect(cards[1].querySelector('.shop-card-controls .shop-buy-button')?.textContent).toBe('Acheter') })
