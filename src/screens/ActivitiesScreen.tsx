...
function DailiesScreen({ wheelToday, onSpinWheel, dailyRewardToday, dailyChallenge, dailyCombat = unavailableDailyCombat, expedition, expeditionMonotonicNow = 0, dailiesOverviewRequestToken = 0, elementKey, onClaimDailyReward, onPurchaseDailyChallenge, onSwitchDailyChallenge, onOpenParticleConversion, onOpenExpedition, onNavigate }: Omit<ActivitiesScreenProps, 'screen'>) {
...
function ExpeditionOverviewCard({ snapshot = unavailableExpeditionSnapshot, monotonicNow, onAccess }: { snapshot?: ExpeditionClientSnapshot; monotonicNow: number; onAccess: () => void }) {
  ...
  const detail = value.operationalStatus === 'RUNNING'
    ? <>{value.activeCharacter?.name ?? 'Personnage'} · <ExpeditionCountdown snapshot={snapshot} monotonicNow={monotonicNow} />{previousDeparture ? ' · Départ précédent · le départ du jour sera disponible après récupération.' : ''}</>
    : presentation.detail
