...
function DailyChallengeCard({ value, onPurchase, onSwitch }: { value: DailyChallengeDto; onPurchase: (key: string) => Promise<DailyChallengeMutationDto>; onSwitch: (key: string) => Promise<DailyChallengeMutationDto> }) {
  ...
  if (!value.assigned || !value.challenge) return <section className="panel daily-challenge-card available"><div className="daily-challenge-main"><span className="eyebrow">Quotidiennes</span><h2>Défi du jour</h2><p>Obtenez un objectif aléatoire à accomplir avant le reset journalier. L’objectif vous sera révélé après l’achat.</p><dl><div><dt>Prix</dt><dd>{formatResourceAmount(value.purchaseCost)} Moras</dd></div><div><dt>Récompense</dt><dd>800 Primogemmes</dd></div></dl></div><div className="daily-challenge-state-slot">{error && <p role="alert">{error}</p>}</div><div className="daily-challenge-action-slot"><button type="button" className="small-primary-button" disabled={Boolean(pending)} onClick={() => void run('purchase')}>{pending === 'purchase' ? 'Attribution…' : 'Acheter le Défi'}</button></div><div className="daily-challenge-confirm-slot" /></section>
  ...
  return <section className={`panel daily-challenge-card ${value.status.toLowerCase()}`}><div className="daily-challenge-main"><span className="eyebrow">Défi du jour · {value.businessDate}</span><h2>{challenge.displayName}</h2><p>{challenge.description}</p><div className="daily-challenge-progress" aria-label={`${challenge.progress} sur ${challenge.target}`}><span style={{ width: `${progressPercent}%` }} /></div><strong>{challenge.progress} / {challenge.target} {challenge.progressLabel}</strong></div><div className="daily-challenge-state-slot">{value.status === 'COMPLETED' ? <><p className="daily-challenge-complete">✅ Terminé</p><p>{dailyChallengeProgressSentence(challenge)}</p><p>{challenge.rewardPrimogems} Primos reçus.</p></> : <p>État : Actif · Récompense : {challenge.rewardPrimogems} Primogemmes</p>}{error && <p role="alert">{error}</p>}</div><div className="daily-challenge-action-slot">{value.status !== 'COMPLETED' && <button type="button" className="daily-challenge-switch-button" disabled={!value.canSwitch || Boolean(pending)} onClick={requestSwitch}>Changer de Défi · {formatResourceAmount(value.nextSwitchCost ?? '0')} Moras</button>}</div><div className="daily-challenge-confirm-slot">{confirmSwitch && <div className="daily-challenge-confirm" role="dialog" aria-label="Confirmer le remplacement"><p>Votre progression sera perdue. Remplacer ce Défi pour {formatResourceAmount(value.nextSwitchCost ?? '0')} Moras ?</p><button type="button" className="daily-challenge-cancel" onClick={() => setConfirmSwitch(false)}>Annuler</button><button type="button" className="small-primary-button" disabled={Boolean(pending)} onClick={() => void run('switch')}>Confirmer</button></div>}</div></section>
}
