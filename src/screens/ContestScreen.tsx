...
function ContestEmpty({ value, selectedLegendId, onSelect, pending, onOpen }: { value: ContestDto; selectedLegendId: string; onSelect: (id: string) => void; pending: string | null; onOpen: () => void }) {
  ...
  return <section className="panel contest-empty"><span className="contest-theme-glyph" aria-hidden="true">✦</span><h2>Aucun Concours actif</h2><p>Ouvrez un lobby et devenez son premier participant.</p>
    ...
    <AppButton variant="primary" className="primary-button contest-mutation-button" disabled={!value.permissions.canOpen || Boolean(pending) || !selectedLegendId} aria-busy={loading} onClick={onOpen}>{loading ? 'Ouverture…' : 'Ouvrir un lobby'}</AppButton>
  </section>
}
...
function ContestRunning(props: Props & { contest: ContestSnapshotDto; pending: string | null; run: Runner; now: number; scoreFeedback: ScoreFeedback | null }) {
  ...
  return <div className="contest-active-layout running">
    ...
    <div className="contest-footer-actions">{value.permissions.canLeave && <PendingButton signature="leave" pending={props.pending} pendingLabel="Sortie…" onClick={() => contest.viewer.participantSlot ? ask('leave') : void props.run('leave', props.onLeave)}>Quitter le Concours</PendingButton>}{value.permissions.canCancel && <PendingButton signature="cancel" pending={props.pending} pendingLabel="Annulation…" variant="danger" className="danger-button" onClick={() => ask('cancel')}>Annuler le Concours</PendingButton>}</div>
