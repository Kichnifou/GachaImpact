...
function ContestEmpty({ value, selectedLegendId, onSelect, pending, onOpen }: { value: ContestDto; selectedLegendId: string; onSelect: (id: string) => void; pending: string | null; onOpen: () => void }) {
  ...
  return <section className="panel contest-empty"><span className="contest-theme-glyph" aria-hidden="true">✦</span><h2>Aucun Concours actif</h2><p>Ouvrez un lobby et devenez son premier participant.</p>
    ...
    <button type="button" className="primary-button contest-mutation-button" disabled={!value.permissions.canOpen || Boolean(pending) || !selectedLegendId} aria-busy={loading} onClick={onOpen}>{loading ? 'Ouverture…' : 'Ouvrir un lobby'}</button>
  </section>
}
...
function ContestRunning(props: Props & { contest: ContestSnapshotDto; pending: string | null; run: Runner; now: number; scoreFeedback: ScoreFeedback | null }) {
  const { contest, value } = props
  const [confirmation, setConfirmation] = useState<{ kind: 'leave' | 'cancel' | 'remove-spectator'; playerId?: string } | null>(null)
  ...
  return <div className="contest-active-layout running">
    <section className="panel contest-state-header contest-running-header"><div className="contest-running-round"><span className="eyebrow">Manche {contest.currentRound}</span><h2>{contest.phase === 'SUPPORT' ? 'Soutien du public' : 'Concours en cours'}</h2></div><p className="contest-running-status">{status}</p><div className="contest-running-phase"><span>{contest.phase === 'SUPPORT' ? 'Soutien' : 'Tour'} {contest.currentTurnOrder ?? '—'}</span><Countdown label={contest.phase === 'SUPPORT' ? 'Soutien' : 'Tour'} deadline={deadline} now={props.now} /></div></section>
    ...
    <SpectatorStrip contest={contest} organizerCanRemove={contest.viewer.organizer} onRemove={(playerId) => setConfirmation({ kind: 'remove-spectator', playerId })} pending={props.pending} />
    <div className="contest-footer-actions">{value.permissions.canLeave && <PendingButton signature="leave" pending={props.pending} pendingLabel="Sortie…" onClick={() => contest.viewer.participantSlot ? setConfirmation({ kind: 'leave' }) : void props.run('leave', props.onLeave)}>Quitter le Concours</PendingButton>}{value.permissions.canCancel && <PendingButton signature="cancel" pending={props.pending} pendingLabel="Annulation…" className="danger-button" onClick={() => setConfirmation({ kind: 'cancel' })}>Annuler le Concours</PendingButton>}</div>
    {confirmation?.kind === 'leave' && <ContestConfirmation title="Quitter le Concours ?" message="Votre participation quotidienne a déjà été consommée au lancement. Si vous quittez maintenant, elle ne sera pas récupérée et vous ne pourrez plus participer à un Concours aujourd’hui. Êtes-vous sûr ?" confirmLabel="Quitter le Concours" danger onCancel={() => setConfirmation(null)} onConfirm={() => { setConfirmation(null); void props.run('leave', props.onLeave) }} />}
    {confirmation?.kind === 'cancel' && <ContestConfirmation title="Annuler le Concours ?" message="Votre propre participation restera consommée. Les autres participants humains récupéreront la leur. Aucun podium, aucune récompense et aucun résultat sportif ne seront produits." confirmLabel="Annuler le Concours" danger onCancel={() => setConfirmation(null)} onConfirm={() => { setConfirmation(null); void props.run('cancel', props.onCancel) }} />}
    {confirmation?.kind === 'remove-spectator' && confirmation.playerId && <ContestConfirmation title="Retirer ce spectateur ?" message="Le spectateur sera retiré immédiatement du Concours en cours. Cette action ne modifie pas les participations quotidiennes." confirmLabel="Retirer" danger onCancel={() => setConfirmation(null)} onConfirm={() => { const playerId = confirmation.playerId!; setConfirmation(null); void props.run(`remove-spectator:${playerId}`, (key) => props.onRemoveSpectator(playerId, key)) }} />}
  </div>
}
...
function ContestConfirmation({ title, message, confirmLabel, danger = false, onCancel, onConfirm }: { title: string; message: string; confirmLabel: string; danger?: boolean; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-layer contest-confirmation-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel() }}><section ref={dialogRef} tabIndex={-1} className="panel contest-confirmation" role="alertdialog" aria-modal="true" aria-labelledby="contest-confirmation-title"><button type="button" className="contest-confirmation-close" aria-label="Fermer" onClick={onCancel}>×</button><span className="eyebrow">Confirmation</span><h2 id="contest-confirmation-title">{title}</h2><p>{message}</p><div><button type="button" onClick={onCancel}>Non</button><button type="button" aria-label={confirmLabel} className={danger ? 'danger-button' : 'primary-button'} onClick={onConfirm}>Oui</button></div></section></div>
}
...
function LegendsModal({ legends, theme, onClose }: { legends: readonly ContestLegendDto[]; theme: ContestDto['theme']; onClose: () => void }) {
  ...
  return <div className="history-modal-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section ref={dialogRef} tabIndex={-1} className="panel history-modal contest-legends-modal" role="dialog" aria-modal="true" aria-label="Mes Légendes" aria-labelledby="contest-legends-title"><header><div><span className="eyebrow">Concours</span><h2 id="contest-legends-title">Mes Légendes</h2></div><ModalCloseButton onClose={onClose} /></header><div className="contest-legends-controls"><label><span>Rechercher</span><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="Nom" /></label><label><span>Élément</span><select value={element} onChange={(event) => { setElement(event.target.value as 'all' | ElementKey); setPage(1) }}><option value="all">Tous</option>{(Object.keys(elementLabels) as ElementKey[]).map((key) => <option value={key} key={key}>{elementLabels[key]}</option>)}</select></label><label><span>Tri</span><select value={sort} onChange={(event) => { setSort(event.target.value as LegendSort); setPage(1) }}><option value="name">Nom</option><option value="power">Puissance totale</option><option value="theme">Statistique du thème</option><option value="contests">Participations</option><option value="wins">Victoires</option></select></label><button type="button" className="contest-sort-direction" aria-label={descending ? 'Tri descendant' : 'Tri ascendant'} onClick={() => { setDescending((value) => !value); setPage(1) }}>{descending ? '↓' : '↑'}</button></div><div className="history-modal-body contest-legends-list">{Array.from({ length: 3 }, (_, index) => { const legend = entries[index]; return legend ? <LegendCard legend={legend} key={legend.character.id} /> : <div className="contest-legend-placeholder" aria-hidden="true" key={`empty-${index}`} /> })}{ordered.length === 0 && <p className="contest-modal-empty">Aucune Légende ne correspond à ces filtres.</p>}</div><footer className="history-modal-pagination"><button type="button" disabled={currentPage <= 1} onClick={() => setPage(currentPage - 1)}>Précédent</button><span>Page {currentPage} / {pageCount} · {ordered.length} Légende{ordered.length > 1 ? 's' : ''}</span><button type="button" disabled={currentPage >= pageCount} onClick={() => setPage(currentPage + 1)}>Suivant</button></footer></section></div>
}
...
function HistoryModal({ history, error, onPage, onLoadDetail, onClose }: { history: ContestHistoryDto | null; error: string | null; onPage: (page: number) => Promise<void>; onLoadDetail: (contestId: string) => Promise<ContestSnapshotDto>; onClose: () => void }) {
  ...
  return <div className="history-modal-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section ref={dialogRef} tabIndex={-1} className="panel history-modal contest-history-modal" role="dialog" aria-modal="true" aria-label="Historique des Concours" aria-labelledby="contest-history-title"><header><div><span className="eyebrow">Concours terminés</span><h2 id="contest-history-title">{detail ? 'Détail du Concours' : 'Historique'}</h2></div><ModalCloseButton onClose={onClose} /></header>{detail ? <ContestHistoryDetail contest={detail} onBack={() => setDetail(null)} /> : <><div className="history-modal-body contest-history-list">{(error || detailError) && <p className="contest-modal-status" role="alert">{error ?? detailError}</p>}{!history && !error && <p className="contest-modal-status" role="status">Chargement…</p>}{Array.from({ length: 10 }, (_, index) => { const contest = history?.contests[index]; return contest ? <article key={contest.id}><div><strong>{formatBusinessDate(contest.businessDate)} · {contest.theme.label}</strong><span>{contest.currentRound} manche{contest.currentRound > 1 ? 's' : ''} · vainqueur {contest.winner?.displayName ?? '—'}</span></div><button type="button" disabled={detailPending} onClick={() => void loadDetail(contest.id)}>Détails →</button></article> : <div className="contest-history-placeholder" aria-hidden="true" key={`empty-${index}`} /> })}{history?.contests.length === 0 && <p className="contest-modal-empty">Aucun Concours terminé.</p>}</div><footer className="history-modal-pagination"><button type="button" disabled={!history || history.page <= 1} onClick={() => history && void changePage(history.page - 1)}>Précédent</button><span>Page {history?.page ?? 1} / {history?.pageCount ?? 1}</span><button type="button" disabled={!history || history.page >= history.pageCount} onClick={() => history && void changePage(history.page + 1)}>Suivant</button></footer></>}</section></div>
}
