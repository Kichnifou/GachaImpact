...
function ModerationScreen({ actorPlayerId, capabilities, onLoad, onSearchPlayers, onResource, onXp, onGacha, onStella, onTester, onApplied }: Props) {
  ...
  useEffect(() => {
    ...
    let active = true
    const timer = window.setTimeout(() => void onSearchPlayers(query).then((value) => {
      if (active) setPlayers(value)
    }).catch((error) => {
      if (active) setMessage(apiErrorMessage(error))
    }), 120)
    return () => { active = false; window.clearTimeout(timer) }
  }, [isSuper, onSearchPlayers, query])
  ...
  return <div className="screen-content moderation-screen">
    <header className="team-screen-heading moderation-screen-heading">
      <div><span className="eyebrow">MODÉRATION</span><h1>Outils de test</h1><p>Rang : {isSuper ? 'Super' : 'Testeur'}</p></div>
    </header>
    {message && <p className="moderation-feedback error" role="alert">{message}</p>}
    {isSuper && <section className="panel moderation-target" aria-busy={pending}>
      <div className="moderation-target-heading">
        <div><span className="eyebrow">Joueur ciblé</span><strong title={state?.player.displayName}>{state?.player.displayName ?? 'Chargement…'}</strong></div>
        <button type="button" className="moderation-self-button" disabled={pending || isSelf} onClick={() => selectTarget(actorPlayerId)}>Moi</button>
      </div>
      <label className="moderation-target-search"><span>Rechercher un joueur actif</span><input type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPlayers([]) }} placeholder="Saisir un pseudo…" autoComplete="off" /></label>
      {query.trim() && <div className="moderation-target-results" role="listbox" aria-label="Résultats de recherche">
