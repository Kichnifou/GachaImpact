...
describe('Expedition presentation', () => {
  it('uses the four exact Quotidiennes states without a fake obtained reward', () => {
    expect(expeditionOverview(idle, 0)).toEqual({ status: 'À faire', detail: 'Aucune expédition lancée.', completed: false })
    ...
    expect(expeditionOverview({ ...idle, departureUsedToday: true, canStartToday: false }, 0)).toEqual({ status: '✅ Terminé', detail: 'Expédition effectuée.', completed: true })
  })
