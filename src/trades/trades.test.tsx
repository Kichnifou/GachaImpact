...
describe('Particle trades UI', () => {
  ...
  it('sends search to the server and ignores a slow previous search response', async () => {
    ...
    actions.partners.mockImplementationOnce(() => new Promise(resolve => { release = resolve }))
    ...
    expect(node.querySelector('.trade-partner')?.textContent).toContain('Alice')
    expect(node.querySelector('.trade-partner')?.textContent).not.toContain('Bob')
  })
