...
- `VALIDÉ R732` — Identité élémentaire visuelle dans le profil joueur : la carte Profil persistante utilise le symbole existant de l'élément personnel comme grand watermark décoratif surdimensionné, partiellement coupé, de faible opacité et sans interaction, accompagné d'une très légère teinte ou lueur de fond issue d'un mapping de couleurs typé unique. La teinte de fond et le watermark restent deux couches visuellement distinctes ; le symbole doit demeurer identifiable pour les sept éléments, y compris Pyro sans se fondre dans le fond rouge. Le traitement reste subtil, préserve pseudo, niveau, XP, contraste et responsive PC/mobile, peut teinter légèrement un accent existant tel que le contour de l'avatar, et n'ajoute ni nouvelle image ni ligne texte `Élément : ...` obligatoire.
- `VALIDÉ R733` — Structure mobile hybride du profil joueur : la sidebar complète reste à gauche sur desktop. En smartphone portrait, l'ordre est Header → résumé joueur prioritaire (Profil, Primogemmes, Moras) → navigation → contenu principal → informations secondaires (Particules, Équipe, Objectif/cible Gacha, Daily Reward et futures cartes secondaires) → Chat. Cette composition conserve une source d'état unique et évite de dupliquer des composants actifs dans le DOM.

## UX Invocation
- `VALIDÉ R734` — Hiérarchie visuelle du hero Invocation avec cible : le nom du personnage est l'information principale et reçoit derrière son début un symbole élémentaire décoratif réel, transparent, non interactif et masqué aux technologies d'assistance ; aucun badge textuel d'élément n'est affiché dans ce hero. Deux phrases courtes occupent chacune leur ligne, suivies d'une date de fin mise en évidence et du bouton `Changer`, visible uniquement sur Invocation. Le footer donne la priorité à Pity 5★ sur toute la largeur de sa zone d'état ; Pity 4★, Garantie et Capture forment une ligne secondaire de même largeur, tandis que les boutons Invocation restent séparés à droite sur desktop puis passent sous l'état Gacha sur mobile. Le picker de cible emploie prioritairement un portrait/icône cadré sans coupe agressive, tandis que le hero après sélection conserve prioritairement le splash art : ces deux contextes ne partagent pas le même ordre d'assets.
- `VALIDÉ R735` — Identité élémentaire de l'objectif Gacha : lorsqu'une cible est sélectionnée, la carte persistante `Objectif actuel` reprend son symbole élémentaire réel comme watermark surdimensionné, partiellement coupé, transparent, non interactif et placé derrière le portrait, le nom, les étoiles, Garantie, Capture et les deux Pity. Le traitement réutilise le thème élémentaire typé commun et ne doit jamais gêner la lecture des statistiques.
