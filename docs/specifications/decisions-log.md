...
- `R504 / R513 / R514 — VALIDÉES PUBLIQUEMENT` — Édition inline auteur-only, Enter/Ctrl/Meta+Enter, IME, annulation outside/Escape, tombstone `Message supprimé`, confirmation de suppression fermée avant le réseau, restauration dans les 500 derniers, purge définitive du seul contenu supprimé sorti de cette fenêtre, actions absolues hover/focus/tactile, éditeur entièrement visible et patch autoritatif sont acquis. Le micro-polish candidat ferme maintenant l'éditeur avant le réseau après Entrée ou Sauvegarder et ne le rouvre pas sur erreur ; ce seul rendu instantané reste à revalider, sans nouvelle décision ni migration.
...
- `R509 / R510 / R517 — VALIDÉES PUBLIQUEMENT, RÉVISION PREVIEW/SUPPRESSION CANDIDATE` — Le signalement volontaire depuis le fil ou R515, la création du dossier, l'absence de notification immédiate, la consultation privée sans free-browse et le snapshot gelé sont validés publiquement. R509 révisée montre au reporter seulement le voisin immédiat précédent, la cible et le voisin immédiat suivant, tout en l'informant que la modération reçoit le snapshot complet 10/1/10. Le fingerprint SHA-256, le stale 409 et la preuve stockée restent calculés sur ces 21 lignes maximum. MODERATOR/ADMIN peuvent supprimer uniquement le dossier après confirmation ; l'audit conserve acteur, reportId et Player signalé, jamais le contenu privé. Cette révision UI et la suppression de dossier restent candidates à revalidation publique ; aucun motif, statut, sanction ou notification au signalé n'est ajouté.
