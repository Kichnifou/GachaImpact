...
- `VALIDÉ R736` — Langage visuel commun des cartes personnage : « même langage visuel » signifie employer autant que possible les mêmes codes de composition, et non seulement des couleurs proches : structure et placement cohérents, cadre de portrait commun, surface et bordure teintées par l'élément, badge élémentaire en haut à gauche, hiérarchie stable nom → étoiles → métadonnées et informations de bas de carte alignées de façon constante. Les dimensions peuvent varier selon le contexte, mais les variantes ne doivent pas devenir des designs sans rapport. L'écran Équipe est la référence principale des cartes unitaires affichées en faible quantité ; Box / Personnages restent la référence des listes compactes. L'Équipe active de la sidebar et le picker 5★ adaptent ces codes à leur espace sans modifier Box / Personnages.
- `VALIDÉ R737` — Présentation des six 4★ de bannière : les six cartes restent sur **une seule ligne de six**, sur desktop comme sur mobile, sans retour en grille 3×2. Leur variante compacte s'inspire de Box / Personnages et affiche uniquement portrait prioritaire `iconPath`, badge élémentaire en haut à gauche, nom, `★★★★`, surface et bordure élémentaires. Niveau, constellation, rôle, type, arme et région sont absents. Sur mobile, leur gabarit, leurs espacements et leur typographie sont réduits afin que les six cartes restent intégralement visibles sur une ligne, sans scroll horizontal, notamment à 390 px.
- `VALIDÉ R738` — Téléphone paysage : les panneaux secondaires joueur restent toujours empilés verticalement dans l'ordre Particules → Équipe active → Objectif → Daily Reward → Chat, avec scroll naturel. La détection responsive doit couvrir les smartphones paysage réels par une combinaison robuste d'orientation, faible hauteur, largeur raisonnable et capacité de pointeur/tactile ; elle ne doit pas dépendre du seul intervalle arbitraire `761–950 px` avec `max-height: 500px`. Les adaptations paysage internes à la bannière restent autorisées sans casser les fenêtres desktop courtes ni les tablettes.
...
- `VALIDÉ R740` — Navigation des cartes de sidebar : tous les headers de panneau joueur navigables utilisent le même chevron `›`, avec même taille, couleur, espacement, hover et focus. Le chevron appartient au titre et le suit immédiatement — par exemple `OBJECTIF ACTUEL ›` — tandis que les informations secondaires restent alignées à droite. Pour l'équipe, la composition cible est `ÉQUIPE ACTIVE ›` à gauche et `4 / 4` à droite ; toute la zone de heading reste cliquable.
