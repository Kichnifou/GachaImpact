...
- `R504 / R513 / R514 — FONCTIONNALITÉS ET ERGONOMIE GÉNÉRALE VALIDÉES PUBLIQUEMENT` — Édition inline auteur-only, tombstone `Message supprimé`, restauration dans les 500 derniers, purge définitive du seul contenu supprimé sorti de cette fenêtre, actions absolues hover/focus/tactile, éditeur entièrement visible et patch autoritatif sont acquis. Le complément candidat ajoute seulement Entrée pour sauvegarder, Ctrl/Meta+Entrée pour insérer une ligne, protection IME et annulation par clic extérieur. Confirmer une suppression ferme maintenant l'overlay avant toute attente réseau ; un rollback déterministe ne le rouvre pas. Ces micro-interactions seules restent à revalider, sans nouvelle décision ni migration.
- `R515 — IMPLÉMENTATION SUR main, À VALIDATION PUBLIQUE` — Historique complet séparé du fil live borné à 500, avec scroll keyset bidirectionnel par `submissionOrder`, recherche textuelle serveur excluant tout tombstone et navigation vers le premier message à partir d'une date locale, ou le dernier antérieur en fallback. Les GET restent participant-only, lisibles en mode bloqué/read-only et ne marquent jamais lu. Cette implémentation n'est pas déclarée validée publiquement et attend le contrôle de déploiement puis le test public propriétaire.
