...
- `CANDIDAT R896 — RECHERCHE VALIDÉE PUBLIQUEMENT, DEUX MICRO-AJUSTEMENTS À REVALIDER` — Le vertical MP conserve un cache éphémère par conversation et des onglets permanents discrets `Conversations | Archives`; chaque libellé est désormais centré dans sa moitié. Après choix d'un destinataire, l'aide vaut exactement `Écrivez le premier message.`. La recherche dédiée à 50 ms, son choix en un clic et sa protection contre les réponses obsolètes sont validés publiquement. Les autres comportements déjà validés restent acquis.
- `CANDIDAT R897 — À VALIDER PUBLIQUEMENT APRÈS CORRECTION` — Pendant le verrou long R894, le composer affiche exactement `Spam, veuillez attendre...`. Les optimistics restent immédiats, mais les nouveaux POST d'une instance Chat sont transmis dans leur ordre local, sans chevauchement et avec au moins 750 ms entre leurs démarrages. Une erreur ne casse pas la file et un retry exact conserve clé/ligne sans devenir une nouvelle intention de pacing. Sur réseau lent, le verrou attend que le troisième transport soit résolu et reste compatible avec son timestamp serveur. Le backend capture cet instant après le verrou Player, sans changer `submissionOrder`, R895 ni les gardes serveur.
...
- `IMPLÉMENTATION R504 / R513 / R514, ERGONOMIE FINALE À REVALIDER` — Édition inline auteur-only, tombstone `Message supprimé`, restauration dans les 500 derniers et purge définitive du seul contenu supprimé sorti de cette fenêtre restent matérialisés sans nouvelle décision ni migration. Le correctif `review` remplace le `⋯` par des actions absolues hover/focus/tactile, rend l'éditeur entier visible, place la confirmation de suppression en overlay et adopte immédiatement le patch serveur edit/delete/restore. La règle des 500 et la purge ne changent pas.
