...
- `VALIDÉ R772` — Les quatre slots Team conservent le même gabarit quelle que soit la composition 0..4. Une carte occupée enchaîne directement son portrait puis son bloc identité, sans bande vide structurelle. Un slot vide n'affiche en permanence que son numéro et un `+` centré : toute la carte est activable au clic ou au clavier, et le bouton visuel `Ajouter` apparaît uniquement au survol ou au focus. Sur desktop standard 1920×1080 et 2560×1440, bandeau, quatre slots et panneau Passifs doivent tenir sans scroll vertical normal lorsque le viewport le permet.
- `VALIDÉ R773` — `Voir les passifs` ouvre le référentiel `Les sept passifs` dans un dialog au-dessus de Team, fermé par croix, backdrop ou Escape, sans déplier ni allonger la page. Le panneau contextuel reste compact et conserve `Aucun passif actif` ainsi que l'accès au référentiel lorsqu'aucun effet n'est dérivé.
