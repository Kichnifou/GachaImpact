...
- `TECHNIQUE R781` — `server/src/domain/team/team-passives.ts` porte une source centrale fortement typée des paramètres futurs des sept passifs Gacha et une dérivation machine-readable depuis les éléments de la Team, avec cap de deux stacks, sans parsing des descriptions. Cette dérivation est pure : elle n'exécute aucun RNG et ne mute aucune ressource, Pity ou XP. Elle n'est pas encore appelée par le moteur Pull ; les sept effets seront raccordés ensemble dans un lot ultérieur, après création de la primitive serveur centrale de gain d'XP requise par Cryo.
