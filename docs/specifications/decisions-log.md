...
- `VALIDÉ R797` — Une mutation qui augmente réellement le niveau publie un unique feedback global agrégé : `Niveau supérieur !` pour +1 ou `N niveaux gagnés !`, suivi d'une seule ligne de récompenses machine-readable totalisées, sans recalcul économique frontend. Il apparaît en overlay non bloquant, reste environ cinq secondes et s'accompagne d'un `+N` et d'un glow Profil d'environ deux secondes. Le chargement initial n'en produit jamais ; les événements distincts sont mis en file. Une montée Cryo/Gacha attend le disclosure anti-spoil de Progression. `prefers-reduced-motion` conserve uniquement les fades/glows statiques nécessaires.
- `VALIDÉ R798` — Lors de l'implémentation réelle du Sac, sa catégorie `Monnaies` devient `Ressources`. La carte Moras entière ouvre Banque et peut afficher discrètement `Accéder à la Banque`. Cette décision ne justifie aucune modification du mock Sac avant le lot dédié.
