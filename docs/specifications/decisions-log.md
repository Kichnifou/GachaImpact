...
- `VALIDÉ R736` — Langage visuel commun des cartes personnage : trois contextes sont distingués. La variante **Showcase / Team**, dont la carte de l'écran Équipe validée publiquement est la référence absolue, place le badge élémentaire en haut à gauche, une éventuelle métadonnée contextuelle en haut à droite, un portrait sans espace parasite supérieur dans une zone clairement délimitée, puis le nom, les étoiles et, si le contexte le prévoit, une ligne Niveau à gauche / Cx à droite. Le picker Invocation 5★ doit être visuellement dérivé de ce modèle exact, et pas seulement partager un DOM général, une teinte ou une bordure : badge (composant, taille, forme, placement et distance aux bords), zone portrait, séparation portrait/texte, fond, bordure, rayon, nom, étoiles et paddings internes sont communs. Ses seules différences autorisées sont un gabarit légèrement réduit, l'absence de numéro de slot, Niveau et constellation, ainsi que l'interaction de sélection. L'ordre d'assets du picker reste `iconPath` → `fullbodyPath` → `wishPath` → `splashPath`. L'Équipe active de la sidebar conserve son adaptation compacte de la même famille Showcase. La variante **Collection compacte**, référencée par Box / Personnages, est conçue pour la densité et ses cartes ne doivent pas être agrandies.
- `VALIDÉ R737` — Présentation des six 4★ de bannière : ils sont absents de toute sélection ou changement de cible 5★, y compris la première sélection sans cible ; aucun conteneur ni espace ne leur est réservé dans le picker. Ils restent affichés **uniquement dans le hero Invocation normal lorsqu'une cible est sélectionnée**, sur une seule ligne de six, desktop comme mobile. Ces cartes sont directement dérivées de la primitive visuelle `CharacterCard` de Box validée publiquement : même carte de base, portrait prioritaire `iconPath`, badge élémentaire et placement, séparation portrait/texte, bordure, surface élémentaire, nom et `★★★★`. Leur variante compacte n'affiche aucune donnée de possession ou de catalogue : ni Niveau, constellation, rôle, type, arme ou région.
