...
- `VALIDÉ R875` — Mentions `@pseudo` assistées et mises en évidence, sans notification persistante ; joueur bloqué exclu des suggestions et d'une mention réelle même si sa chaîne est saisie. R885 précise la résolution manuelle autoritative.
...
- `VALIDÉ R885` — Un `@pseudo` entier saisi directement crée une mention réelle après résolution autoritative du Player par le serveur, insensible à la casse et aux accents selon Social. Les doublons sont dédupliqués ; pseudo inexistant, partiel ou bloqué reste du texte ordinaire. Les IDs client et les commandes ne créent pas de mention sociale arbitraire.
