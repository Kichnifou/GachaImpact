...
- `VALIDÉ COMME DÉCISION PRODUIT ; IMPLÉMENTATION CANDIDATE REVIEW` — La migration additive 044 conserve un JSONB nullable sur chaque rotation. À la génération de la bannière suivante, la même transaction fige la rotation de vote source, l'instant, tous les candidats 5★ alors éligibles avec leur nombre de votes, y compris zéro, et le choix/source du slot communautaire. Aucun pool n'est figé au début de la semaine. Aucun backfill fictif des anciennes rotations : leur détail de vote est explicitement indisponible.
