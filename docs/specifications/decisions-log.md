...
- `TECHNIQUE R789` — Un dépôt/retrait est une seule transaction `SERIALIZABLE` verrouillée par Player. `MAX` est résolu depuis le solde verrouillé côté serveur, l'opération possède la même idempotence persistante que les autres mutations et le mouvement du portefeuille passe par une primitive interne dédiée qui ne modifie ni `totalMorasEarned` ni `totalMorasSpent`.
- `TECHNIQUE R790` — L'intérêt Banque est traité par journée métier `Europe/Paris`, séquentiellement et avec floor afin de préserver la composition. Un scheduler en processus exécute le catch-up au démarrage puis planifie chaque prochain minuit en tenant compte du DST. La paire Player/journée possède une unicité DB et une opération système idempotente : restart ou concurrence ne peuvent créditer deux fois la même journée.
