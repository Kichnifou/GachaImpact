...
- `RÉVISÉ R861 / CANDIDAT 0.95` — Chaque gain de points Concours affiche temporairement `+N` à droite du score du participant bénéficiaire, y compris `+0` pour un Risque raté, les tours Bot/automatiques et le soutien. Le serveur projette dans l’ordre chronologique une fenêtre bornée aux huit événements de score récents et chaque client déduplique puis met en file tous les nouveaux `eventId`, afin que l’acteur et les autres viewers voient le même feedback sans calculer un delta de score. La première fenêtre d’une partie ou après reprise d’un onglet caché sert de baseline ; l’overlay ne modifie pas la géométrie et respecte `prefers-reduced-motion`.
- `CANDIDAT R862` — Dans un Concours RUNNING, les actions contextuelles vivent dans un overlay hover/focus des cartes participant sans modifier leur géométrie : Basic/Risque uniquement sur la carte du viewer dont c’est le tour, Soutenir sur les quatre cartes pour le spectateur sélectionné. Les contrôles restent de vrais boutons accessibles, deviennent directement visibles sans hover sur tactile et remplacent entièrement la grande région d’action inférieure ; le bandeau conserve seulement un statut compact de hauteur stable.
- `RÉVISÉ R593 / CANDIDAT R863` — Le résultat final est projeté sur l’écran Concours courant strictement pendant les cinq minutes qui suivent son `finishedAt` autoritatif. À cinq minutes révolues, l’écran redevient neutre si aucun Concours n’est actif. Le FINISHED reste conservé dans l’Historique et aucun délai ne bloque la création immédiate du prochain lobby par un joueur éligible.
- `TECHNIQUE CANDIDAT R864` — `PlayerItem.quantity` reste l’unique solde autoritatif d’un objet. `ItemAcquisition` est un ledger append-only de faits d’acquisition (Player, définition, quantité positive, source/provenance et date), sans backfill artificiel ; son historique personnel est chargé uniquement dans la fiche, par pages de vingt, et ne sert jamais à recalculer le stock courant.
