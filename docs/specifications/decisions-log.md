...
- `RÉVISÉ R365 / R844 / R853` — Le bouton `Accéder` de la carte Expedition reste disponible dans les états À faire, RUNNING et READY et redirige vers la vraie Box. Le cas `IDLE` déjà effectué aujourd'hui affiche `✅ Terminé` sans action.
...
- `VALIDÉ R844` — Dans `Quotidiennes > Aperçu`, une activité réelle terminée affiche `✅ Terminé`, son détail contextuel, puis `Obtenu : <résultat réel>` ; une absence de gain s’écrit `Obtenu : Rien`. Daily Reward, Roue et Défi lisent leurs DTO autoritatifs et ne rendent plus aucune action `Accéder` ou `Récupérer` une fois terminés, sans changer la hauteur externe de leur carte.
...
- `RÉVISÉ R365 / R844 / CANDIDAT R853` — La carte Expedition conserve `Accéder` dans les états À faire, RUNNING et READY. Dans le seul état `IDLE` dont le départ courant est déjà consommé, elle affiche `✅ Terminé` sans action et sans résultat de récompense fictif.
