...
- `VALIDÉ R866` — Toute action UI qui explicite une conséquence irréversible sur la participation quotidienne demande une confirmation projet sombre `Non | Oui` avant mutation : lancement effectif, départ d’un participant après lancement et annulation RUNNING par l’organisateur. Quitter un lobby, quitter comme spectateur et annuler un lobby restent directs ; fermer, `Escape` ou backdrop équivalent à `Non`, et le serveur demeure l’autorité métier/idempotente.
