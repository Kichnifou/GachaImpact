...
- `VALIDÉ R772` — Les quatre slots Team conservent le même gabarit quelle que soit la composition 0..4. Un slot vide n'affiche en permanence que son numéro et un `+` centré : toute la carte est activable au clic ou au clavier, et l'action discrète `Ajouter` apparaît uniquement au survol ou au focus. Sur desktop standard 1920×1080 et 2560×1440, bandeau, quatre slots et panneau Passifs doivent tenir sans scroll vertical normal lorsque le viewport le permet.
- `VALIDÉ R773` — `Voir les sept passifs` ouvre un dialog au-dessus de Team, fermé par croix, backdrop ou Escape, sans déplier ni allonger la page. Le panneau contextuel reste compact et conserve `Aucun passif actif` ainsi que l'accès au référentiel lorsqu'aucun effet n'est dérivé.
...
- `VALIDÉ R777` — La direction du drag des Teams est horizontale dans le bandeau réel et remplace la direction verticale envisagée par R232. Un drop directement sur une carte échange les deux positions ; un drop entre deux cartes insère la Team et décale l'ordre intermédiaire. Dans les deux cas, l'UUID et l'état actif restent attachés à la même Team.
