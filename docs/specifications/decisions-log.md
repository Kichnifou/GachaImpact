...
- `TECHNIQUE R800` — Le rôle communautaire `MODERATOR` n'accorde aucun outil de test. Dans l'écran actuel, `ADMIN` est affiché `Super` et porte `moderationAccess`, `selfResourceTools`, `superTools`, `canSelectPlayers` et `canManageTesters`; `TESTER` est affiché `Testeur` et porte uniquement `moderationAccess` et `selfResourceTools`. L'autorisation dépend exclusivement des attributions actives persistées, jamais d'un pseudo, du frontend ou d'une URL.
...
- `VALIDÉ R806` — Une Stella confirmée produit un feedback `+1` temporaire, sans place réservée ni layout shift, à côté de la constellation lorsqu'elle augmente ou dans la seule carte de statistique C6 désignée par le résultat serveur structuré. Le passage C5 → C6 cible la constellation et affiche les cinq statistiques initialisées, sans attribuer de faux `+1` à l'une d'elles ; une Stella déjà C6 ne produit aucun `+1` de constellation.
