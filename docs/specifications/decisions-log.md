...
- `RÉVISÉ R891 — À REVALIDER PUBLIQUEMENT` — Les actions d'un message Chat d'autrui suivent `Signaler | Masquer | Copier | Message privé | Mentionner | Répondre` ; l'accès MP n'est plus accolé au pseudo. Desktop le représente par une icône seule accessible et tactile conserve le libellé `Message privé`. Celles de son propre message restent `Copier | Répondre | Supprimer`.
...
- `CANDIDAT R896 — À REVALIDER PUBLIQUEMENT SUR LES ÉLÉMENTS MODIFIÉS` — Le vertical MP conserve un cache éphémère par conversation et des onglets permanents `Conversations | Archives`; ces onglets deviennent discrets. Un clic manuel MP revient à la liste, tandis qu'un deep-link remplace immédiatement toute recherche ou cible périmée. La recherche de destinataire utilise une projection légère dédiée, après 50 ms, avec choix en un clic et protection contre les réponses obsolètes. L'envoi optimistic garde le même rendu visuel que le message confirmé, sans heure ni statut interne. Une seule ligne hors bulle porte `Envoi...`, `Envoyé`, `Lu ✓` ou l'âge relatif du dernier message sortant sans réponse. Les autres comportements déjà validés restent acquis.
- `CANDIDAT R897 — À VALIDER PUBLIQUEMENT` — Pendant le seul verrou long R894, jusqu'à quatre secondes après le troisième envoi rapproché, le composer Chat devient non modifiable et affiche exactement `Spam, veuillez attendre...`; Entrée et Envoyer restent sans effet. Le délai simple de 750 ms demeure silencieux et laisse la saisie modifiable. Un rejet déterministe recalcule le verrou depuis les tentatives réellement conservées.
...
- `IMPLÉMENTATION SUR MAIN R504 / R513 / R514` — Édition inline auteur-only, tombstone `Message supprimé`, restauration dans les 500 derniers et purge définitive du seul contenu supprimé sorti de cette fenêtre sont matérialisés sans nouvelle décision ni migration. La validation publique reste à faire.
