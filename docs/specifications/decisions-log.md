...
- `VALIDÉ R828` — `Configuration > Menu` conserve les flèches accessibles et ajoute un drag-and-drop à aperçu local. Un drop déclenche exactement une sauvegarde ; un échec restaure l’ordre précédent et conserve les identifiants masqués. Configuration demeure toujours visible. Confidentialité et Apparence restent désactivés.
...
- `VALIDÉ R833` — Dans `Configuration > Menu`, un drop entre deux lignes signifie insertion et un drop au centre d’une ligne signifie échange. Les intentions ont des previews distinctes ; aucun mouvement ne sauvegarde, un drop valide sauvegarde exactement une fois et tout échec restaure le dernier ordre confirmé avec ses éléments masqués. Les flèches restent disponibles.
...
- `VALIDÉ R838` — Primos ouvre Boutique, Moras ouvre Banque et seules les particules de l’élément personnel ouvrent la conversion. Sac expose ces actions dans `Tout` et `Ressources`, chaque carte réservant le même slot inférieur même vide ; la sidebar ouvre la même modale de conversion sur l’écran courant et réutilise la même intention idempotente.
