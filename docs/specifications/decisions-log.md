...
- `VALIDÉ R861` — Chaque gain de points Concours affiche temporairement `+N` à droite du score du participant bénéficiaire, y compris `+0` pour un Risque raté, les tours Bot/automatiques et le soutien. Le feedback est déclenché une seule fois par identifiant d’événement serveur, n’est pas rejoué au premier rendu d’une partie déjà active, reste positionné en overlay sans modifier la géométrie des cartes et respecte `prefers-reduced-motion`.
