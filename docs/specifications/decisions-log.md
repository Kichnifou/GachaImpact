...
- `TECHNIQUE R800` — Le rôle communautaire `MODERATOR` n'accorde aucun droit de modification de l'économie personnelle. Les outils de test utilisent la capacité serveur `SELF_TEST_TOOLS`, portée initialement par le rôle technique `TESTER`; `ADMIN` l'obtient implicitement. L'autorisation dépend exclusivement des attributions actives persistées, jamais d'un pseudo, du frontend ou d'une URL.
- `TECHNIQUE R801` — `Kichnifou` cumule `MODERATOR` et `TESTER` dans l'environnement DEV après résolution unique d'un Player ACTIVE lié à une WebIdentity ACTIVE. Cette attribution est une donnée d'environnement révocable et ne constitue jamais une condition applicative fondée sur son nom.
- `TECHNIQUE R802` — Les mutations de self-test sont exclusivement self-only, transactionnelles, idempotentes et reliées chacune à une `BusinessOperation` et une `AdminAuditEntry`. Leur idempotence est garantie de bout en bout : après une erreur réseau/serveur ambiguë, le frontend conserve par Player l'intention et sa clé pour le retry du même type d'action et du même payload; toute intention différente reste bloquée jusqu'à résolution, afin d'éviter tout double delta ou double audit. Un succès ou une erreur déterministe libère l'intention, et le sign-out les efface. Les ajustements de ressources créent aussi un `ResourceMovement` de source `ADMIN`, sans modifier les statistiques ordinaires gagnées/dépensées; le set XP ne simule aucun gain, récompense ou message.
...
- `TECHNIQUE R804` — Le frontend n'affiche le bouton et l'écran Modération que d'après le DTO de permissions serveur. Une route `#moderation` forcée ne révèle aucun outil sans capacité, et toutes les routes privilégiées contrôlent à nouveau l'autorisation côté backend avant toute lecture ou mutation.
