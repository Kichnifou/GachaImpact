...
- `TECHNIQUE R802` — Les mutations de self-test sont exclusivement self-only, transactionnelles, idempotentes et reliées chacune à une `BusinessOperation` et une `AdminAuditEntry`. Les ajustements de ressources créent aussi un `ResourceMovement` de source `ADMIN`, sans modifier les statistiques ordinaires gagnées/dépensées; le set XP ne simule aucun gain, récompense ou message.
