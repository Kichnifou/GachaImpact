...
- `RÉVISÉ R891 — À REVALIDER PUBLIQUEMENT` — Les actions d'un message Chat d'autrui suivent `Signaler | Masquer | Copier | MP | Mentionner | Répondre` sur desktop comme sur tactile ; l'accès MP n'est plus accolé au pseudo. Celles de son propre message restent `Copier | Répondre | Supprimer`.
...
- `RÉVISÉ R895 — À REVALIDER PUBLIQUEMENT` — `submissionOrder` reste l'autorité persistante backend pour snapshots, historique, fenêtres, pagination, non-lus et curseurs. Le rendu live Chat et MP ne déplace jamais rétroactivement une ligne déjà affichée : une inconnue est ajoutée lors de sa livraison, une ligne connue ou un optimistic est remplacé en place, sans buffering. L'historique ancien reste préfixé avec compensation du viewport.
- `CANDIDAT R896 — À REVALIDER PUBLIQUEMENT` — Le vertical MP conserve un cache éphémère par conversation et des onglets permanents `Conversations | Archives`; un clic manuel MP revient à la liste, un deep-link ouvre sa cible. L'envoi optimistic n'affiche qu'une bulle, sans heure ni statut interne. Une seule ligne hors bulle porte `Envoi...`, `Envoyé`, `Lu ✓` ou l'âge relatif du dernier message sortant sans réponse. Badges, scroll d'envoi et toggle d'accusés réagissent localement avant revalidation serveur.
