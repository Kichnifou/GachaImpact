...
- `TECHNIQUE R786` — Le frontend rafraîchit Progression avec Resources et Gacha après un Pull, mais ces états restent dans le buffer de présentation jusqu'au disclosure déjà autorisé. Aucun feedback passif n'apparaît pendant les deux secondes de Focus personnage ; les effets réellement déclenchés peuvent apparaître en phase Ready, au récapitulatif x10 et dans l'Historique. Dans le récapitulatif compact, un gain passif n'est présenté qu'une seule fois : les crédits Anémo et le bundle Dendro déjà représentés par `passiveEffects` ne sont pas répétés depuis `bonusRewards`, sans masquer les remboursements C6 ni les récompenses de niveau XP.
