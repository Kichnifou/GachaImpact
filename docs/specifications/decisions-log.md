...
- `ABANDONNÉ R841` — R833 n’est plus applicable à `Configuration > Menu` : insertion, échange, previews et toute intention future de drag y sont supprimés. Les règles DnD restent valides dans Team et dans les composants dont la spécification les exige explicitement.
...
- `RÉVISÉ R843` — Primos ouvre Boutique, Moras ouvre Banque et seules les particules de l’élément personnel ouvrent la conversion. Sac expose ces actions dans `Tout` et `Ressources` en les intégrant à la géométrie compacte historique dont Moras est la référence ; une carte sans action ne réserve aucun slot inférieur vide. Dans la sidebar, Conversion occupe l’espace interne bas droite du panneau Particules sans en modifier les dimensions et ouvre la même modale sur l’écran courant avec la même intention idempotente.
...
- `VALIDÉ R843` — Les liens contextuels d’une carte Ressource s’intègrent à sa géométrie compacte existante ; ils ne justifient ni hauteur minimale artificielle, ni footer vide universel. La carte Moras pré-0.83 est la référence physique pour Primos et les particules personnelles, tandis que les autres particules ne portent aucune action.
