...
- `RÉVISÉ R894 — DURÉE À REVALIDER PUBLIQUEMENT` — Le Chat global impose 750 ms entre nouveaux envois acceptés ; trois envois en moins de quatre secondes verrouillent désormais exactement trois secondes après le troisième, au lieu de quatre. Les refus locaux et `CHAT_PACING_LIMIT` sont des no-op silencieux qui préservent draft/réponse/mentions et ne prolongent pas le verrou ; un retry exact garde sa clé et ne compte pas comme nouvel envoi. Seule cette durée révisée est rouverte ; le filet serveur dix messages/dix secondes demeure.
...
- `R509 / R510 / R517 — IMPLÉMENTATION SUR MAIN, VALIDATION PUBLIQUE REQUISE` — Un participant peut prévisualiser puis signaler un message MP actif de l'autre participant depuis le fil ou R515. Le serveur gèle la cible et au plus dix lignes avant/dix après, masque tout contenu supprimé, exige le fingerprint SHA-256 exact au POST et refuse en 409 un aperçu devenu obsolète. La modération `MODERATOR`/`ADMIN` consulte uniquement ces dossiers gelés dans Communauté ; aucun browse de conversation, motif inventé, statut, sanction ou notification au signalé n'est ajouté. La validation publique propriétaire reste requise.
