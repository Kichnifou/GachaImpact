...
- `APPLICATION TECHNIQUE R301 — LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT ; LOT 3 APPROUVÉ PAR REVIEW INDÉPENDANTE ET PROMU SUR main, NON ENCORE VALIDÉ PUBLIQUEMENT` — Pour les Players standalone provisionnés avant la migration 041, la baseline physique est 0 afin que le rattrapage retrouve leurs compteurs V1 autoritatifs accumulés depuis le provisionnement. Les migrations n’accordent aucune récompense : le rattrapage économique passe exactement une fois par `PermanentMissionService` et Economy lors d’une activation métier réelle. La reprise legacy ambiguë reste régie séparément par R328.
