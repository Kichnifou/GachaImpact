...
`BankTransaction` conserve le type, le montant, les soldes Banque avant/après, les soldes wallet avant/après pour un transfert, la date métier pour un intérêt et l'unique `BusinessOperation` source. Le ledger accepte actuellement `DEPOSIT`, `WITHDRAWAL` et `INTEREST`; son ordre récent est `createdAt DESC`, puis identifiant stable. Un intérêt nul n'est pas une opération économique et ne produit donc aucune ligne de ledger.

État de lecture 0.73 : l'écran principal demande au plus les cinq opérations les plus récentes. L'historique complet réutilise le même ledger, reste strictement personnel et est paginé côté serveur par dix opérations newest-first ; aucune nouvelle table ni duplication d'historique n'est introduite.

Le scheduler serveur exécute un catch-up au démarrage puis planifie le prochain minuit `Europe/Paris` avec un calcul tenant compte du DST. Chaque journée manquée est appliquée dans l'ordre avec `floor(balance × 3 / 100)`. Pour un gain strictement positif, l'unicité persistante `(playerId, businessDate)` limitée aux intérêts et l'idempotency key système empêchent tout double crédit. Pour un gain nul, `lastInterestDate` avance sans `BusinessOperation`, `BankTransaction`, statistique ni historique. Un transfert est statistiquement neutre ; un intérêt positif crédite uniquement la Banque et `totalMorasEarned`. Une intention de transfert ambiguë reste stable côté session et son `requestedAmount` (`entier` ou `max`) est lié à la clé persistée côté serveur. Dans l'UI web 0.73, `MAX` remplit seulement le champ depuis le snapshot affiché et la confirmation envoie un entier ; le contrat serveur `max` reste disponible pour les futurs canaux communs.
...
La projection personnelle canonique expose B/A/S avec objectif, progression, seuil, état, récompense et rang. Avant `zUnlockedAt`, elle ne renvoie que `{ status: LOCKED }` pour Z, sans définition ni récompense. Le Lot 4 validé publiquement la sérialise par la route personnelle authentifiée `/api/v1/me/missions` après le catch-up R301 exactly-once éventuel. Le candidat Lot 5 réutilise cette projection pour `!mission` et ajoute une projection Profil tierce lazy, contrôlée par Privacy `MISSIONS` et strictement read-only ; aucun modèle ou table supplémentaire n’est créé. Les notifications et la migration legacy restent absentes.
...
- source : INTERNAL_CHAT / TWITCH / SYSTEM
...
- `createdAt`, utilisé par le pacing, est capturé seulement après le verrou Player ; sa chronologie sérialisée reste distincte de la réservation de `submissionOrder` et ne peut pas être antérieure au dernier message persistant du même Player traité avant lui ;
...
- `replyToMessageId`, auto-référence nullable restrictive vers un message persistant de la même conversation
...
R515 ajoute une projection Historique distincte et éphémère, sans nouvelle table : page chronologique `submissionOrder ASC`, curseurs `olderCursor`/`newerCursor`, recherche active case-insensitive par conversation et ancre date `{ messageId, submissionOrder, createdAt }`. Chaque ligne historique ajoute `canRestore`, calculé à partir d'une unique borne des 500 par réponse : auteur courant, tombstone, contenu serveur encore présent, non purgé et position toujours dans la fenêtre. Le contenu d'un tombstone reste toujours nul dans le DTO. Recherche et date sont résolues en PostgreSQL, jamais par scan Node, et aucun GET historique ne touche les curseurs de lecture.

R504/R513/R514 sont matérialisées sans migration : seul l'auteur peut éditer, supprimer pour les deux participants ou restaurer dans la fenêtre courante. Une édition remplace le contenu sur la même ligne et renseigne `editedAt`. Une suppression conserve contenu serveur, ID, ordre et date mais renseigne `deletedAt`; toute projection player-facing garde alors `content = null`, y compris à l'auteur. Une restauration remet `deletedAt` à null et renseigne `restoredAt`. La réponse commune de mutation expose uniquement `message: { id, content, editedAt, deletedAt, restoredAt }`; elle est stockée dans le résultat de `BusinessOperation`, donc un replay exact restitue le même patch. Restore peut exposer à l'auteur le contenu redevenu actif, jamais un contenu encore supprimé. Lors d'un nouvel envoi, les seuls contenus déjà supprimés passés hors des 500 derniers deviennent définitivement nuls et reçoivent `contentPurgedAt`; aucune ligne active n'est purgée. Chaque mutation possède sa propre `BusinessOperation` idempotente sans créer de message ni de non-lu.

R898 est matérialisée par la migration additive 043, sans backfill : les lignes antérieures gardent `replyToMessageId = null`. Seul le POST d'un message dans une conversation existante accepte cette cible optionnelle ; l'initiation d'une conversation n'en accepte pas. La cible doit être persistante, active, non purgée et appartenir à la même conversation au moment de l'écriture. Elle peut appartenir à l'un ou l'autre participant. L'empreinte idempotente inclut la cible. Les DTO list/recent/history/search projettent `replyToMessageId` et un `replyPreview` dynamique : contenu courant actif, sinon `Message supprimé`. Aucune copie du texte cible n'est persistée dans `direct_messages`.
...
Le preview participant-only montré au joueur contient seulement le message précédent immédiat, la cible active de l'autre participant et le message suivant immédiat, en `submissionOrder ASC`. L'API explique qu'un contexte plus large sera transmis. En parallèle, elle construit jusqu'à dix messages précédents, la cible et dix suivants pour le fingerprint et la preuve serveur gelée. Un tombstone de contexte conserve `content = null`. Chaque ligne fige aussi ses `replyToMessageId` et `replyPreview` tels qu'observés à cet instant. La confirmation reconstruit cet ensemble complet dans une transaction sérialisable ; un fingerprint différent produit `DIRECT_MESSAGE_REPORT_PREVIEW_STALE` sans dossier et exige une nouvelle confirmation humaine. Après création, édition, suppression/purge et messages futurs ne modifient jamais les deux JSON enregistrés.
