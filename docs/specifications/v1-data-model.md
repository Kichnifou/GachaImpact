...
Le preview participant-only montré au joueur contient seulement le message précédent immédiat, la cible active de l'autre participant et le message suivant immédiat, en `submissionOrder ASC`. L'API explique qu'un contexte plus large sera transmis. En parallèle, elle construit jusqu'à dix messages précédents, la cible et dix suivants pour le fingerprint et la preuve serveur gelée. Un tombstone de contexte conserve `content = null`. La confirmation reconstruit cet ensemble complet dans une transaction sérialisable ; un fingerprint différent produit `DIRECT_MESSAGE_REPORT_PREVIEW_STALE` sans dossier et exige une nouvelle confirmation humaine. Après création, édition, suppression/purge et messages futurs ne modifient jamais les deux JSON enregistrés.
