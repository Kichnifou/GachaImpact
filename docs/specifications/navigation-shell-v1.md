...
- Événement est une surface réelle organisée dans l’ordre `Inscription | Jeux | Shop | Classement`. Inscription, ouvert par défaut, porte le hero Festival, le statut et le résumé personnel de l’édition. Jeux reste visible mais désactivé avant l’inscription, puis devient interactif sans navigation automatique après le join serveur. Si un changement d’édition rend le Player non inscrit alors que Jeux est sélectionné, la vue revient automatiquement sur Inscription. Jeux possède une seconde barre de trois sous-onglets aux identités thématiques du Festival : Jeu A et Jeu B sont actifs et fonctionnels (Récolte et Grenier en septembre) ; Jeu C, Shop et Classement restent des repères désactivés sans badge de roadmap ni faux contenu. Jeu B présente les 32 combinaisons globales, les codes testés/restants, le quota personnel et le découvreur après résolution.
- Concours est une surface réelle ; son contrat métier reste propriétaire de son contenu.

Quotidiennes possède `Aperçu | Roue | Défi`. Aperçu liste toujours, dans cet ordre, le catalogue quotidien décidé : `Récompense quotidienne | Roue | Défi | Combat | Boss | Expédition | Amitié | Événement`. Les domaines implémentés, dont Event Lot 3 — Jeu B, utilisent leur état serveur réel ; les autres restent visibles avec un état neutre et honnête, sans progression, compteur ni donnée fictive. Le hub ne réimplémente jamais leur logique métier. La carte Event délègue sa disponibilité à un agrégateur de présentation central : son CTA est visible si l’inscription reste possible ou, après inscription, si Jeu A possède encore une fenêtre active/future non réussie **ou** si Jeu B n'est pas résolu et conserve des essais personnels ainsi qu'au moins une combinaison non testée. Si A et B n'offrent plus d'action, le CTA disparaît ; un nouveau snapshot de business date le rétablit selon les nouveaux états. Jeu C non implémenté ne compte pas.
