...
Social (`#social`) ouvre **Amis** par défaut, avec les trois onglets réels **Amis / Demandes / Joueurs**. Amis présente progression, cœur individuel, `Envoyer un cœur à tous` séparé des filtres et retrait ; son dernier tri est conservé dans les préférences Player. Demandes filtre les demandes reçues/envoyées et expose accepter/refuser/annuler, avec les demandes reçues prioritaires. Joueurs conserve recherche normalisée, Élément et pagination serveur de vingt, ajoute les filtres réels de présence et de relation (`Tous`, `Amis`, `Envoyées`, `À accepter`, `À ajouter`, `Moi`) ainsi que l'action contextuelle stable `Toi` / `Envoyée` / `Ami` / `Accepter` / `Ajouter`. Une présence privée n'est jamais inventée Hors ligne. Identité, présence et actions occupent des zones stables. Aucune huitième tuile principale ni onglet Messages.

Le profil (`#profile`) conserve sa cible en mémoire de session ; un rechargement ouvre le profil personnel. Rubriques réelles : Aperçu, Team active, Box, Collection et Statistiques. Aperçu contient l'identité agrandie, le statut autorisé avec pastille entre le nom et le niveau, l'icône Élément et la dernière activité séparée ; Team active reste uniquement dans son onglet. Fallback exact : « Aucune activité récente. » Consultation en lecture seule selon permissions serveur, privé distinct de vide. Le profil personnel propose Configuration > Confidentialité, présentée en lignes séparées avec selects alignés.

Le panneau Joueurs connectés et le compteur Chat partagent la présence visible : En ligne et Absent seulement, sans valeur fictive. Chaque ligne conserve l'accès Profil et expose l'état social utile `Toi` / `Ajouter` / `Envoyée` / `Ami` / `Accepter` ; « Voir tous les joueurs → » ouvre explicitement **Social > Joueurs**, même si Social était déjà ouvert sur Amis. La sidebar ouvre le profil personnel. Le profil d'un autre joueur expose les mêmes transitions pertinentes, le cœur et le retrait ; le profil personnel n'affiche aucune action vers soi. Une notification de demande reçue ouvre **Social > Demandes**. Les messages du Chat restent un prototype sans clic de profil ni envoi réel.
