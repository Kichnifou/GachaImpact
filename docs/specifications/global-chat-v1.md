...
## Mutations MP candidates — R504, R513 et R514

- Sur `review`, un message propre actif expose discrètement `Modifier` et `Supprimer` via `⋯`; un message reçu n'expose aucune de ces actions. Modifier ouvre un textarea inline multiligne, garde la même bulle et affiche `Modifié` après succès sans remplacer le statut de livraison.
