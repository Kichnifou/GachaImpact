...
## Mutations MP sur main — R504, R513 et R514

- Sur `main`, un message propre actif expose discrètement `Modifier` et `Supprimer` via `⋯`; un message reçu n'expose aucune de ces actions. Modifier ouvre un textarea inline multiligne, garde la même bulle et affiche `Modifié` après succès sans remplacer le statut de livraison.
- Supprimer demande `Supprimer ce message ?`, conserve la ligne et affiche `Message supprimé` aux deux participants. Le dernier tombstone n'affiche plus `Envoyé`/`Lu` et son aperçu de liste reste `Message supprimé`.
- Seul l'auteur peut restaurer son tombstone tant qu'il appartient aux 500 derniers et que son contenu n'a pas été purgé. La restauration reprend le même ID, ordre et emplacement. Il n'existe aucune suppression de conversation, route d'historique complet, recherche/date, signalement ou administration MP dans ce lot promu ; sa validation publique reste portée par le Master.
