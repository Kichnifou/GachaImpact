...
Version : candidat review de polish public Chat/MP avec réconciliation optimiste MP corrigée
Date : 2026-09-23
Statut : POLISH CHAT/MP CANDIDAT SUR REVIEW, À REVIEW INDÉPENDANTE PUIS REVALIDATION PUBLIQUE
...
- **Candidat polish public Chat/MP sur `review`, base `a359e88`.** Le panneau droit reste le propriétaire unique `Chat | MP`. R893 révisée et R894 sont désormais **VALIDÉES PUBLIQUEMENT**. R895 est révisée : `submissionOrder` reste l'autorité backend des snapshots, fenêtres, pages, non-lus et curseurs, mais le frontend live ne déplace plus une ligne déjà affichée ; inconnues ajoutées à la livraison, lignes connues et optimistic remplacés en place. Le premier MP d'un nouveau fil apparaît immédiatement dans un fil provisoire, adopte ensuite l'état serveur PENDING/ACCEPTED, et restaure le brouillon avec la même clé de retry en cas d'erreur déterministe. Une projection serveur fraîche observée par polling reste prioritaire sur un accusé POST tardif. Chat poll à 350 ms start-to-start lorsqu'il est ouvert et visible, sans overlap, et garde le bas dans le même layout si le lecteur y était déjà.
...
- **Chat global — état public réconcilié.** R890, R892, R893 révisée et R894 restent validées publiquement. R891 est révisée et doit être revalidée : aucun petit bouton près du pseudo ; l'ordre autrui devient `Signaler, Masquer, Copier, MP, Mentionner, Répondre`, propre inchangé. R895 révisée doit également être revalidée publiquement.
- **MP — candidat R896 à revalider.** `Conversations | Archives` sont deux onglets permanents et un clic manuel MP revient à la liste, tandis que les deep-links ouvrent leur cible. Avatar identitaire commun, cache éphémère par conversation, seed immédiat, optimistic unique réconcilié par `clientIntentKey`, projection fraîche gagnante, bulles sans heure ni statut, ligne globale `Envoi... | Envoyé | Lu ✓ | relatif`, badge/scroll/toggle accusés optimistes et polling 500/1 250/5 000 ms sans overlap composent ce polish.
...
- Base DEV : migrations 036–039 appliquées via Prisma ; **39 migrations Prisma** suivies. La migration 039 et `submissionOrder` restent immuables ; ce candidat n'ajoute **aucune migration 040**. Le DTO MP projette seulement à l'auteur la clé d'intention déjà portée par `BusinessOperation`. RLS et droits restent inchangés ; aucun Player, seed économique, import legacy ou réglage Privacy n'est modifié.
- Validation technique de ce candidat : **751 tests frontend / 85 fichiers** réussis, dont le ciblage final **25/25** de `DirectMessagePanel`. Build frontend réussi ; le lint frontend ne conserve que les deux avertissements `ContestScreen` préexistants et Vite son avertissement de chunk. Le backend n'est pas modifié dans ce correctif : les preuves précédentes restent **389 tests backend / 54 fichiers** et **8/8 scénarios PostgreSQL MP**, sans nouvelle exécution revendiquée. Le vrai `GameShell`, les vrais panneaux Chat/MP et le CSS de production, alimentés par des réponses synthétiques sans persistance, avaient été inspectés dans le lot précédent à **2560×1440, 1920×1080, 1366×768 et 390×844** : pseudo Chat sans MP inline et action MP dans l'overlay, liste et onglets MP, recherche avec avatar `A`, fil avec chevron/menu/composer, message long sans débordement, bulles sans heure, scrollbar masquée au bas et statuts exclusifs `Envoi...`, `Envoyé`, `Lu ✓` et relatif. Captures et harnais temporaires ont été retirés du lot. Ces preuves ne constituent ni déploiement ni validation publique.
...
- Le lot Chat player-facing antérieur reste validé publiquement. R890, R892, R893 révisée et R894 sont validées publiquement. Les nouvelles versions de R891 et R895, ainsi que le polish MP R896, attendent une revalidation publique après le cycle review/promotion/déploiement approprié.
...
- Domaine actif : **polish Chat/MP**. Le socle backend/DB et le vertical initial restent sur `main`; ce nouveau candidat vit uniquement sur `review`. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R896, sans transfert depuis Event Game C.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai commit poussé sur `origin/review`**. Aucune promotion `main` n'appartient à cette mission. Après approbation seulement viendront promotion dédiée, déploiements et checklist publique ciblée R891 révisée, R895 révisée et MP R896. Les fonctions avancées édition, suppression/restauration, historique complet, recherche/date, signalement/admin MP, médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportées. `PAID_INFRA_APPROVED = false`.
