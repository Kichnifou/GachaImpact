...
Version : Missions permanentes — Lot 5 validé publiquement ; derniers correctifs MP approuvés et promus, test public final attendu ; workflow Codex actif
Date : 2026-09-25
Statut : LOTS MISSIONS 1 À 5 PROMUS ET VALIDÉS PUBLIQUEMENT ; R898 MP ET SÉCURISATION DES FINS D'ENVOI PROMUS, CŒUR DES RÉPONSES OBSERVÉ PUBLIQUEMENT MAIS CYCLE MP NON ENCORE VALIDÉ ; POLISH MP ET DEUX DERNIERS CORRECTIFS APPROUVÉS ET PROMUS, TEST PUBLIC FINAL ATTENDU ; WORKFLOW CODEX PROMU ET ACTIF ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Polish MP et deux derniers correctifs approuvés et promus ; cycle complet non encore validé.** Dans une conversation existante, l'envoi actif libère immédiatement le composer et son focus. Une seule intention suivante peut attendre, texte et réponse visibles dans un composer grisé ; elle garde sa clé UUID et ne démarre qu'après succès du précédent. Pendant cette attente, les actions Répondre sont désactivées et la cible capturée ne peut pas changer. Un échec libère cette intention comme brouillon éditable, garde l'envoi échoué récupérable avec sa clé et ne lance aucun envoi automatique. La navigation invalide la queue et la rend récupérable uniquement à l'origine. Le propriétaire confirme sur téléphone réel le vertical MP, l'ouverture sur les messages récents, le scroll tactile haut/bas et le composer visible ; il confirme aussi M1→M2 queued, la réponse et sa cible gelée, les actions sur messages longs, la confirmation Supprimer auto-scrollée et le comportement général desktop/mobile. Le dernier correctif restaure le focus après réactivation réelle du composer lors de queued→active, pendant que M2 reste réseau-pending, sans voler un focus volontaire ; les bulles retrouvent leur largeur naturelle sans réserve permanente pour les actions. Ces deux points attendent encore le dernier test public propriétaire avant clôture du cycle.
...
- Domaine actif : **VÉRIFICATION POST-PROMOTION DES DERNIERS CORRECTIFS MP ; WORKFLOW CODEX ACTIF ; LOT 5 MISSIONS PROMU ET VALIDÉ PUBLIQUEMENT**. Les Lots 1 à 5, pagination/loaders et le correctif responsive Boutique `7106f39` sont validés publiquement ; le bootstrap demeure stable et publiquement acceptable avec sa latence résiduelle non bloquante. R898, la sécurisation MP, le polish et ses deux derniers correctifs sont promus ; plusieurs comportements du polish sont confirmés publiquement, mais les deux corrections et le cycle MP complet ne le sont pas. La migration 043 appartient seulement au premier correctif MP ; DEV et le dépôt restent à 43 migrations, sans 044.
- Prochaine étape exacte : **ChatGPT vérifie `main == review`, Cloudflare Pages, Railway et `/health`, puis le propriétaire teste publiquement le focus immédiat après M2 queued→active et la géométrie naturelle des bulles avec actions overlay sur desktop/mobile**. Si ces deux points passent, clôturer publiquement le cycle MP, puis suivre la procédure de passation ChatGPT/Codex avant le prochain domaine. Aucun succès de déploiement ni validation publique de ces corrections n'est encore revendiqué. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
