...
Version : Chat global promu ; ultime micro-correctif de détails Composer promu, retest public à faire ; MP encore non physique
Date : 2026-09-22
Statut : CHAT GLOBAL PRESQUE VALIDÉ ; ULTIME MICRO-CORRECTIF COMPOSER PROMU, VÉRIFICATIONS PUBLIQUES ET VALIDATION FINALE EN ATTENTE
...
- Le premier lot Chat global player-facing, le lot réactivité/UX, le mini-lot final de polish, le dernier lot UX Composer et ses micro-correctifs sont **implémentés, testés, reviewés indépendamment et promus sur `main`**. Le dernier état public testé `263bb3be2288e9838ccea3f92c543f6c544b874a` a confirmé la hauteur du composer, son cadre bleuté arrondi et son focus, la suppression de l'espace fin supérieur, la taille à 11 px et les couleurs par Élément des pseudos PLAYER, sans nouveau défaut sur le reste du Chat. Il a relevé cinq derniers détails : coins du cadre hors focus trop fins, texte et croix du bandeau Réponse trop petits, chevron trop bas, placeholder et texte saisi trop hauts. Le micro-correctif `b16a1769bafb44b95b2e5d2e0d15e1b795f310e0` les implémente, ses tests sont réussis et sa review indépendante est approuvée ; il est promu sur `main`. Son contour hors focus est uniforme à 2 px, le focus reste renforcé, le bandeau Réponse et sa citation ainsi que sa croix sont agrandis, le chevron est recentré et le texte du champ est légèrement redescendu. Cloudflare, Railway et le healthcheck du nouveau HEAD restent à vérifier, puis le retest public final reste à faire par Axel. Le Chat global est **presque validé**, sans clôture publique définitive ; les MP restent non physiques. Twitch, Realtime/WebSocket/SSE demeurent absents ; `PAID_INFRA_APPROVED = false`. La conversation Codex actuelle continue en CONTINUITÉ.
...
Le [contrat Chat global V1](../specifications/global-chat-v1.md) porte les décisions validées R872–R887. Le panneau desktop repliable accueille le Chat global réel ; l’onglet MP prévu reste non physique. Les fondations serveur/DB, les commandes et leurs réponses publiques, puis le `ChatPanel` connecté et son transport HTTP incrémental ont été implémentés et reviewés indépendamment. La chaîne player-facing et le lot réactivité/UX sont promus sur `main`. Axel a effectué le second retest public : les parcours de base essayés fonctionnent, et les retours UX et fonctionnels ont été corrigés dans le lot promu. Les déploiements Cloudflare/Railway, le healthcheck et le retest public final de réactivité/UX restent à vérifier ; le Chat global n’est pas encore validé publiquement définitivement. Twitch et son pont optionnel restent reportés ; Realtime est absent.
