...
Version : fondations serveur/DB MP corrigées R505/R511/R516 candidates sur review ; micro-polish Chat R890–R893 candidat
Date : 2026-09-23
Statut : FONDATIONS MP BACKEND/DB PHYSIQUES SUR DEV, SANS UI ; CHAT GLOBAL RESTE LE SEUL CHAT PLAYER-FACING
...
- **Candidat courant — fondations MP et micro-polish Chat.** Les migrations 036–037 matérialisent et durcissent conversation canonique à deux, participants, demandes et messages MP privés ; `DirectMessageService` et ses routes HTTP portent permissions PUBLIC/FRIENDS/PRIVATE, blocage Social, idempotence, anti-rafale, non-lus, lecture/accusés séparés, archivage et fenêtre normale de 500. La correction de review applique R505 : désactiver les accusés fige leur curseur partagé sans effacer les lectures déjà communiquées ; leur réactivation seule ne le fait pas avancer. R511/R516 utilisent le refus neutre unique `Ce message ne peut pas être envoyé.` pour les causes sociales privées. Aucun onglet, badge, composer ou historique UI MP n'est ajouté. Le Chat global borne désormais toute projection joueur aux 200 derniers messages de la génération, recharge le récent au dépliage et applique l'ordre d'actions R891 avec confirmation Signaler adjacente R892. Ce candidat n'est pas encore validé publiquement par Axel.
...
- Le lot Chat player-facing antérieur reste validé publiquement ; le micro-polish R890–R893 et les fondations MP 036–037 forment le candidat review courant.
...
- Prochaine étape exacte : review indépendante du candidat publié sur `review`, puis test public d'Axel limité au micro-polish Chat. Le lot MP reste invisible et se valide techniquement par ses routes/tests/contraintes. Une mission UI MP séparée ne commence qu'après validation explicite ; elle ne doit pas ajouter implicitement édition, suppression/restauration, historique, recherche, date, signalement, `!mp`, Twitch ou Realtime. `PAID_INFRA_APPROVED = false`.
