...
Version : vertical MP player-facing et correctifs Chat promus sur main ; validation publique ciblée à poursuivre
Date : 2026-09-23
Statut : UI MP PRÉSENTE SUR MAIN, TESTÉE TECHNIQUEMENT ET VISUELLEMENT EN LOCAL, À VALIDATION PUBLIQUE CIBLÉE
...
- Domaine actif : **Messages privés (MP)**. Le socle backend/DB, l'UI player-facing, R893 révisée, R894, R895 et les corrections de review Chat sont physiques sur main ; la validation publique de R893 révisée, R894, R895 et MP reste à obtenir. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R895, sans transfert depuis Event Game C.
- Prochaine étape exacte : **vérifier après promotion que `main == review`, puis Railway, `Application ready`, `/health` et le déploiement frontend disponible ; fournir ensuite à Axel une checklist publique ciblée UI MP, R893 révisée, R894 et R895.** Axel valide ou remonte les défauts ; seulement après validation publique viendra le checkpoint documentaire de clôture du premier vertical MP. Les fonctions avancées édition, suppression/restauration, historique complet, recherche/date, signalement/admin MP, médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportées. `PAID_INFRA_APPROVED = false`.
