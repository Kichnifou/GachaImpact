...
Version : Statistiques générales approuvées en review et promues sur main ; déploiements et test public à vérifier
Date : 2026-09-25
Statut : STATISTIQUES GÉNÉRALES APPROUVÉES EN REVIEW ET PROMUES SUR MAIN, DÉPLOIEMENTS ET TEST PUBLIC NON ENCORE VÉRIFIÉS ; LOTS MISSIONS 1 À 5 PROMUS ET VALIDÉS PUBLIQUEMENT ; CYCLE MESSAGES PRIVÉS, R898, SÉCURISATION ASYNC/FOCUS/QUEUE ET RESPONSIVE MOBILE PROMUS ET VALIDÉS PUBLIQUEMENT ; AUCUN DÉFAUT MP CONNU RESTANT DANS CE CYCLE ; WORKFLOW CODEX PROMU ET ACTIF ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Statistiques générales approuvées en review et promues sur `main` dans ce checkpoint.** `Profil > Statistiques` présente Progression, Gacha, Économie et Activités depuis une projection serveur en lecture seule des sept tables propriétaires. Les 23 métriques, dont le taux de 5★ dérivé sans perte de précision, suivent `GENERAL_STATISTICS` ; `!infos` reste compact. Aucune migration Prisma supplémentaire. Railway, Cloudflare Pages et le healthcheck restent à vérifier après le push `main` ; aucun test public propriétaire n'a encore été effectué pour ce lot. Les Lots Missions 1 à 5 et le cycle Messages privés restent promus et validés publiquement. Top / classements globaux est le domaine suivant dans la roadmap, à ouvrir seulement après validation publique des Statistiques. Restent non implémentés les notifications de complétion Missions player-facing, Twitch et la migration legacy R328.
- **Contrôles du lot Statistiques avant promotion.** Tests ciblés frontend **8/8** et backend **110/110**, suites complètes non-DB frontend **843/843** et backend **410/410**, suite PostgreSQL Social/Profil **9/9**, builds frontend/backend et `verify:quick` **5/5** exécutés localement sur le candidat approuvé `631da2827f69fdf7f059edc01b4031387f92f008`. Un harnais temporaire avec le vrai `ProfileScreen`, les CSS de production et la géométrie du shell a été vérifié sous Edge à **1920×1080, 1366×768 et 390×844** : quatre groupes présents, `Non disponible` visible dans le DOM, aucune valeur coupée ni débordement horizontal mesuré. Ce contrôle local ne remplace pas le test visuel/public d'Axel dans le `GameShell` complet. Le lot est promu par Git, sans preuve de déploiement ni validation publique à ce stade.
...
- Domaine actif : **STATISTIQUES GÉNÉRALES — APPROUVÉES EN REVIEW ET PROMUES SUR MAIN ; VALIDATION PUBLIQUE EN ATTENTE**. La projection dédiée et l'onglet Profil en quatre groupes sont dans le checkpoint Git promu. Toutes les valeurs `bigint` traversent l'API en chaînes décimales ; le taux de 5★ est calculé en `bigint` et indisponible à zéro Pull. La confidentialité `GENERAL_STATISTICS` est contrôlée avant la lecture des compteurs. `!infos` conserve seulement Pulls et victoires Combat. La dernière migration reste `20260925100000_043_add_direct_message_replies` ; DEV et dépôt comptent **43 migrations Prisma**, sans 044 ni migration pour ce lot.
- Prochaine étape exacte : **vérifier les déploiements Railway et Cloudflare Pages ainsi que le healthcheck, puis fournir et réaliser le test public propriétaire des Statistiques générales**. Attendre cette validation publique avant d'ouvrir Top / classements globaux. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
...
Pour la prochaine étape exacte, consulter le bloc **Reprise rapide — état vivant** au début du Master. Batch A, Batch B et le cycle MP sont historiques et validés publiquement ; le Calendrier de Noël naturel reste à observer en décembre. L'historique Event transversal, Twitch/chat et la migration legacy ne sont pas développés dans ce checkpoint documentaire.
