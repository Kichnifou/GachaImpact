...
Version : Batch C — polish public Batch B + Amitié, candidat review
Date : 2026-09-20
Statut : BATCH A ET BATCH B PROMUS ET VALIDÉS PUBLIQUEMENT ; BATCH C CANDIDAT REVIEW, NON PROMU
...
- Base DEV : **29 migrations Prisma** à jour ; dernière additive `20260920210000_029_add_friendship_workflows` appliquée via Prisma. Migrations 001–028 intactes. Demandes, cœurs et compteur individuel cumulé sont physiques ; aucune donnée legacy importée, aucun reset Privacy.
- Le déploiement public reste celui du Batch B `d66576de952893b17851721fa0d5e9eb6aac85d8`, précédemment vérifié Railway SUCCESS/healthcheck 200. **Batch C n'est ni promu, ni déployé, ni validé publiquement.** Aucune action Railway/Cloudflare dans ce lot ; `PAID_INFRA_APPROVED = false`.
- Batch C candidat sur `review` : Amitié complète selon l'audit existant (demandes, réciproque, retrait/réactivation, cœurs Paris, niveaux, récompenses, compteur individuel, concurrence/idempotence), Social **Amis/Demandes/Joueurs**, Quotidiennes réelle et polish public réalisés. Catalogue `Voté ✓`, filtres de présence, première bordure, Profil, panneau connecté et Confidentialité sortent du backlog actif ; le claim Expédition possédait déjà le bon libellé, désormais couvert par test.
...
- **Prochaine étape immédiate : review indépendante ChatGPT/propriétaire du vrai commit GitHub de Batch C.** Après approbation seulement : mission de promotion main, déploiements puis validation publique. Les échanges de particules deviennent le domaine actif uniquement après clôture publique correcte de Batch C ; ensuite Chat global + MP, puis la suite de [implementation-order-v1.md](../roadmap/implementation-order-v1.md). R633/R634 restent différées vers Historique global.
...
- Prisma deploy/status réussis : **29 migrations à jour**, 29 checksums compatibles (5 exacts, 24 équivalents LF/CRLF Windows), aucun registre modifié manuellement. Contrôle final : trois nouvelles tables publiques vides, aucun schéma `batch_test_*` résiduel, contraintes/index/FK/RLS/grants relus. Les tests écrivent réellement des fixtures synthétiques dans des schémas privés UUID puis les suppriment exactement ; aucune suite DB publique partagée relancée.
...
- Warnings connus : deux warnings lint préexistants ContestScreen et taille du bundle Vite ; journaux frontend ECONNREFUSED localhost:3000 sans échec de test ; dépréciation pg observée lors des DB tests. Advisor Supabase : `function_search_path_mutable` sur le nouveau trigger **SECURITY INVOKER** (search_path du schéma appelant, EXECUTE navigateur révoqué), avertissement à examiner en review, migration appliquée non réécrite. Avertissements préexistants `rls_auto_enable` SECURITY DEFINER exécutable et protection Auth contre mots de passe compromis désactivée ; aucun correctif hors scope effectué.
