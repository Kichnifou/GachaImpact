...
Domaine actif :
**Team / Équipe / Passifs — premier lot réel déployé et validé publiquement ; Team Management 0.70 reste en attente de validation publique propriétaire. Le lot 0.71 raccorde localement sur `review` les sept passifs de la Team active au moteur Pull, avec primitive XP centrale, snapshot persistant, retry idempotent et feedback Invocation différé ; review GitHub puis validation publique propriétaire restent nécessaires.**
...
- lot Team Management 0.70 : **DÉPLOYÉ, VALIDATION PUBLIQUE PROPRIÉTAIRE RESTANTE** ; le correctif local 0.71 réserve structurellement la place du bouton `Voir les passifs` avec quatre passifs sur desktop 1920×1080, sans dégrader le rendu 2560×1440 ni le mobile ;
- lot Team / Gacha 0.71 : **IMPLÉMENTÉ ET TESTÉ LOCALEMENT SUR `review`, REVIEW ET VALIDATION PUBLIQUE RESTANTES** ; les sept passifs sont activés ensemble depuis un snapshot de la Team active, sans nouvelle migration.
...
- les passifs Team sont documentés dans la modale et réellement appliqués par le serveur. Chaque `PullResult.snapshot` conserve le snapshot de Team et ses `passiveEffects` machine-readable ; l'API, le retry, l'Historique, la phase Ready et le récapitulatif x10 relisent ces effets sans les attribuer causalement à Hydro ;
...
La Team autoritative et ses sept passifs modifient désormais le moteur Pull dans une seule passe cohérente, transactionnelle, snapshotée et auditable. La validation publique de ce raccord reste à effectuer après review et promotion.
...
Premier lot Team réel déployé et validé publiquement ; Team Management est déployé et son raccord Gacha 0.71 est implémenté et testé localement sur `review` :
...
- le bouton intrinsèque et aligné à gauche `Voir les passifs` ouvre le référentiel des sept passifs dans une modale fermable par croix, backdrop ou Escape, sans allonger l'écran Team ; un à trois passifs conservent leur disposition compacte, exactement quatre utilisent une grille desktop 2×2, et le mobile revient à une colonne. Le panneau conserve `Aucun passif actif` lorsqu'aucun effet n'est dérivé ;
...
**Faire reviewer le candidat Team/Gacha 0.71 uniquement sur `review` → corrections éventuelles → promotion fast-forward vers `main` seulement après approbation → validation publique propriétaire du bouton Passifs à 1920×1080, des sept effets en x1/x10, de leur feedback Ready/récapitulatif/Historique, de l'anti-spoil et de la persistance/retry → clôturer Team / Passifs puis reprendre l'ordre V1 dans `docs/roadmap/implementation-order-v1.md`.** `PAID_INFRA_APPROVED = false` reste inchangé.
