...
Version : Batch B — Social Foundations, validation publique et passation
Date : 2026-09-20
Statut : BATCH A ET BATCH B PROMUS ET VALIDÉS PUBLIQUEMENT ; PRÉPARATION DU BATCH C — POLISH PUBLIC + AMITIÉ
...
- Référence publique du Batch B promu : `d66576de952893b17851721fa0d5e9eb6aac85d8`. À l'issue de sa promotion, `main == review` sur ce SHA ; les checkpoints documentaires de passation sont ensuite publiés sur `review`. Batch B — Social Foundations est reviewé, corrigé, promu par fast-forward et validé publiquement. Batch A reste également promu. Noël naturel reste à observer en décembre.
...
- Batch B validé publiquement : Catalogue Votes et tri Votes fonctionnels, ancien gros bandeau supprimé, Menu Activités fonctionnel, Social/annuaire et profils consultables, présence réelle dans le panneau et compteur connecté réel, Configuration > Confidentialité avec Public / Amis uniquement / Privé fonctionnels. Cette validation porte le fonctionnement général ; le polish n'est pas déclaré terminé.
- Base DEV : **28 migrations Prisma**, dernière additive `20260920130000_028_add_social_presence` déjà appliquée et validée ; aucune migration 029. RLS active sur ses deux tables, sans grant navigateur. Aucun reset Privacy ni backfill des comptes. Migrations 001–027 intactes.
- Déploiement correspondant à `d66576de952893b17851721fa0d5e9eb6aac85d8` : Railway **SUCCESS**, Application ready et healthcheck HTTP 200 vérifiés. Le push `main` a déclenché les mécanismes automatiques ; aucune action Railway/Cloudflare manuelle n'appartient à ce checkpoint. `PAID_INFRA_APPROVED = false` reste inchangé.
- Prochaine étape exacte : **BATCH C — polish public Batch B + Amitié complète**. Le périmètre Amitié vient exclusivement de [l'audit Ami/Social](../legacy/14-ami-social-audit.md), notamment R451–R467 et R494–R500 : demandes, accepter/refuser/annuler/réciproque, retrait/réajout, amis, niveaux/cœurs quotidiens, Envoyer à tous, récompenses économiques, concurrence/idempotence, Quotidiennes Amitié et Social Amis/Demandes/Joueurs. `friendships` existe ; `friend_requests` et `friend_hearts` ne sont pas physiques, donc une migration 029 est probable mais non décidée ni créée.
- Backlog public immédiat du prochain lot : Catalogue/Votes affiche `Voté ✓` avec check doré et réduit la zone verticale votes/état ; Expédition affiche `Récupération…` après `Récupérer`, tandis que `Départ…` reste réservé au départ ; Social ajoute le filtre réel Tous / En ligne / Absent / Hors ligne et corrige la bordure haute de la première ligne au hover ; le panneau connecté devient plus aéré et son bouton « Voir tous les joueurs » mieux intégré ; Confidentialité gagne une présentation en lignes/séparateurs, libellés plus lisibles et selects clairement alignés.
- Profil, prochain polish : retirer Team active de l'Aperçu (son onglet existe), agrandir et aérer l'identité, placer le statut sous l'avatar avec pastille vert/jaune/gris selon En ligne/Absent/Hors ligne, puis séparer harmonieusement la dernière activité. Le fallback devient « Aucune activité récente. »
- Observation fonctionnelle à traiter dans Batch C après inspection : une action réelle (ex. message Festival envoyé par Céo) ne nourrit pas encore durablement la dernière activité du Profil. Concevoir une responsabilité transverse unique réutilisable (nom à choisir) pour les actions réelles initiées par le Player — interaction standalone, gameplay, Game C, puis futurs Ami/Chat — en excluant polling, heartbeat technique, scheduler, notifications, réponses automatiques, fin d'Expédition et tâches serveur. Ne pas disperser des hooks par domaine.
- Nettoyage DEV autorisé mais reporté : `Codex Event 2f5fb3ad` et `Sender5 090faef4` sont encore ACTIVE, sans Friendship ni session actuelle lors du contrôle read-only. Le prochain lot retrouvera leurs IDs, inspectera toutes les FK/dépendances et nettoiera précisément, sans DELETE naïf ni préfixe large. Ne jamais toucher Kichnifou, Mynonyme, MynonymeTest1/2/3, Céo, Mika ou Jean Julien.
- Après Batch C : 1. Amitié ; 2. échanges de particules ; 3. Chat global + MP ; 4. suite de [implementation-order-v1.md](../roadmap/implementation-order-v1.md). R633/R634 restent reportées vers Historique global. Lire aussi [le workflow](../process/implementation-workflow.md), [l'architecture Social](../architecture/backend-architecture-v1.md#social-foundations) et [la navigation](../specifications/navigation-shell-v1.md). Les checkpoints ci-dessous restent des preuves historiques.
