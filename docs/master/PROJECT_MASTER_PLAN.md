...
Version : Missions permanentes — Lots 1 à 3 validés publiquement ; Lot 4 approuvé et promu, à validation publique
Date : 2026-09-25
Statut : LOTS 1 À 3 ET PAGINATION/LOADERS PROMUS ET VALIDÉS PUBLIQUEMENT ; LOT 4 API/ÉCRAN MISSIONS APPROUVÉ PAR REVIEW INDÉPENDANTE ET PROMU SUR `main`, À VALIDATION PUBLIQUE ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : Missions permanentes B/A/S/Z — Lot 4 approuvé et promu sur `main`, à validation publique.** Les Lots 1 à 3 restent promus et validés publiquement, et tous les producteurs autoritatifs du catalogue permanent sont raccordés. Le Lot 4 ajoute la projection personnelle canonique, `GET /api/v1/me/missions` et le vrai écran `Activités > Missions` B/A/S/Z, sans alourdir le bootstrap initial. Restent non implémentés `!mission`, Missions sur Profil, la consommation tierce de la confidentialité Missions, les notifications de complétion player-facing, Twitch et la migration legacy R328.
...
- **Lot 4 Missions approuvé par review indépendante et promu sur `main`, non encore validé publiquement.** La query personnelle résout exclusivement le Player authentifié, exécute le catch-up R301 exactly-once dans sa transaction propriétaire si le marqueur est nul, puis projette l’état confirmé. La réponse sérialise les grands nombres en chaînes et réduit strictement Z verrouillé à `{ status: "LOCKED" }`. L’écran charge seulement à son ouverture, conserve les changements B/A/S/Z en mémoire, relit Ressources une fois uniquement si `catchUpApplied` est vrai et ne propose aucune action manuelle.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 4 APPROUVÉ ET PROMU SUR `main`, À VALIDATION PUBLIQUE**. Les Lots 1 à 3, pagination/loaders et le correctif responsive Boutique `7106f39` sont validés publiquement ; le bootstrap demeure stable et publiquement acceptable avec sa latence résiduelle non bloquante. L’API personnelle et le vrai écran Missions sont physiques sur `main`, le secret Z est appliqué côté serveur et aucune migration 043 n’est créée.
- Prochaine étape exacte : **vérification post-promotion par ChatGPT de l’égalité `main`/`review`, de Cloudflare Pages, de Railway, du log `Application ready` et de `/health`, puis test public du Lot 4 par le propriétaire**. Seulement après cette validation viendra le prochain lot Missions. Restent non implémentés : `!mission`, Missions sur Profil et consultation tierce selon confidentialité, notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
