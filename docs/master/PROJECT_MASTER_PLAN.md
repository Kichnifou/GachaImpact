...
Version : validations publiques Chat/MP, micro-polish éditeur et signalement MP sur main à revalider publiquement
Date : 2026-09-24
Statut : R894/R897/R515 VALIDÉS PUBLIQUEMENT, R509/R510/R517 SUR MAIN À VALIDATION PUBLIQUE
...
- **MP avancés — R504/R513/R514 validés publiquement.** Actions hover/tactile, édition Enter/Ctrl/Meta+Enter/IME/outside/Escape, suppression avec fermeture immédiate de sa confirmation, restauration, statuts, accusés Lu et scroll éditeur sont acquis. Le micro-polish sur main ferme désormais l'éditeur et affiche la bulle optimistic avant d'attendre le réseau après Entrée ou Sauvegarder ; il reste à revalider publiquement. La fenêtre/purge R504 reste strictement inchangée.
...
- **R509/R510/R517 — IMPLÉMENTATION sur main, à validation publique.** Les commits `0fad77083740b16e948ca42b3f8bc63ceb01e863` et `6f68dc49b137ef57ec60759c732c8b54a82a674f` portent le micro-polish éditeur puis le signalement volontaire depuis fil live ou R515 : aperçu serveur 10 avant/cible/10 suivants déjà existants, tombstones masqués, fingerprint SHA-256 et reconfirmation obligatoire en cas de stale. Le snapshot JSON est gelé dans `direct_message_reports`; aucun motif, statut, sanction, notification au signalé ou browse libre n'est ajouté.
- **Modération Communauté sur main, à validation publique.** `communityModeration` vaut vrai uniquement pour MODERATOR/ADMIN et ouvre liste paginée par vingt puis détail gelé. Un MODERATOR-only arrive directement sur Communauté sans recevoir Ressources/XP/Gacha/Stella/Testeur/Codes ; TESTER seul n'accède pas aux dossiers. Les routes de détail prennent seulement un `reportId` et ne reconstruisent jamais le contexte depuis `direct_messages`.
...
- Base DEV : migrations 036–040 appliquées via `prisma migrate deploy` ; **40 migrations Prisma** suivies. La migration 040 est additive et crée uniquement `direct_message_reports` avec quatre FK RESTRICT, unique reporter/message, index de liste, RLS et révocations `PUBLIC`/`anon`/`authenticated`. `prisma migrate dev` reste bloqué historiquement par P3006 sur la FK `moras` de la migration 010 dans le shadow database, sans mise en cause ni recréation de 040. Aucun Player, message, Notification, seed économique, import legacy ou réglage Privacy n'est modifié.
- Validation technique des commits sur main A + R515 + R509/R510/R517 : frontend **779/779 sur 85 fichiers**, dont `ChatPanel` **63/63**, `DirectMessagePanel` **51/51** et `ModerationScreen` **11/11** ; backend **392/392 sur 55 fichiers** ; routes report **2/2** ; PostgreSQL report **5/5** et PostgreSQL Chat **26/26** isolés. Builds frontend/backend et typecheck serveur réussissent ; le lint conserve seulement deux avertissements préexistants dans `ContestScreen`, et le diff-check est propre. La campagne PostgreSQL générale parallèle a produit **262/289** avec 27 échecs : le client `pg` partagé a croisé résultats et fixtures entre fichiers, tandis que le smoke global conserve aussi une dette antérieure de listes exhaustives arrêtées avant plusieurs migrations déjà présentes ; aucun de ces échecs n'est masqué comme réussi. Le layout `GameShell` avec les CSS de production a été contrôlé à **2560×1440, 1920×1080, 1366×768 et 390×844** ; les mesures navigateur donnent `scrollWidth = innerWidth` aux quatre tailles. Cela ne constitue ni un déploiement ni une validation publique.
...
- Le lot Chat player-facing est validé publiquement pour R890 à R897 dans leur état révisé. R504/R513/R514 et R515 sont validées publiquement ; seule la fermeture immédiate de l'éditeur après validation est sur main et reste à revalider publiquement.
...
- Domaine actif : **micro-polish éditeur et signalement/modération privée R509/R510/R517 sur main, en attente de déploiement vérifié puis de validation publique**.
- Prochaine étape exacte : **vérifier les déploiements automatiques et le healthcheck, puis fournir le test public propriétaire ciblé**. Le micro-polish éditeur et R509/R510/R517 ne sont pas déclarés publiquement validés. Médias, `!mp`, sanctions, Twitch et Realtime/WebSocket/SSE restent reportés. `PAID_INFRA_APPROVED = false`.
