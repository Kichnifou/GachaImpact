...
- Le premier lot Chat global player-facing est **implémenté, reviewé, promu et déployé** sur `main`. Le checkpoint public actuel est `7ed10b475e8f50ad71a4bac20be7c1a87f8d7516`. Le second retest public a confirmé que la majorité des correctifs précédents fonctionne ; `!help`, `!banque` et `!pull` ont été observées fonctionnelles. Le candidat sur `review` corrige encore la réactivité de réception et les retours UX constatés. La migration additive 035 est appliquée sur DEV : **35 migrations Prisma** sont suivies ; aucun MP général n'est physique. Le Chat global n'est **pas encore validé publiquement définitivement** : ce candidat doit être contrôlé puis retesté. MP, Twitch et Realtime demeurent non physiques ; `PAID_INFRA_APPROVED = false`. La conversation Codex actuelle continue en CONTINUITÉ.
