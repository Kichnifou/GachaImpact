...
- Domaine actif : **TwitchIdentity / pilote Kichnifou candidat `review`, sans validation publique**. Axel a confirmé publiquement les derniers correctifs Modération, Sidebar et Chat (alignement/cadrage et disparition des lueurs). La validation visuelle R912 et Menu reste distincte, et R911 doit encore être observé lors d'une rotation naturelle. DEV compte **49 migrations Prisma appliquées** ; 049 est enregistrée et ses quatre tables backend ont RLS active, sans lecture `anon` ou `authenticated`.
- Le code candidat lie une identité Twitch par OIDC vérifié et permet la prévisualisation de la copie ignorée des 17 JSON. Le dry-run Kichnifou en lecture seule sur le hash `1852d7141a121c335c5928a8265c20e840e5c5dd20ccee12b054b99f780806ba` trouve **14 domaines personnels physiques importables, 9 domaines globaux ou interjoueurs différés, 2 domaines sans cible et 0 ambiguïté bloquante** ; [la matrice](../architecture/twitch-pilot-mapping.md) en donne les sources et règles. **Aucun import du Player public n'a été lancé**, aucune liaison Twitch publique n'a été créée, et la validation publique du pilote reste à faire. La source Streamer.bot vivante demeure autoritaire jusqu'au cutover, même si ses fichiers changent après la copie figée. Prochaine étape : finaliser les contrôles du candidat, review indépendante, promotion dédiée et test d'Axel. `PAID_INFRA_APPROVED = false`.
