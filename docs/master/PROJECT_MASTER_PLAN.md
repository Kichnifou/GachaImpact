...
Version : Statistiques générales validées publiquement ; R907 et Classements candidats en review
Date : 2026-09-25
Statut : STATISTIQUES GÉNÉRALES VALIDÉES PUBLIQUEMENT PAR LE PROPRIÉTAIRE ; CORRECTIF CHAT R907 ET TOP / CLASSEMENTS GLOBAUX CANDIDATS EN REVIEW, SANS VALIDATION PUBLIQUE DE CES DEUX LOTS ; LOTS MISSIONS 1 À 5 ET CYCLE MESSAGES PRIVÉS PROMUS ET VALIDÉS PUBLIQUEMENT ; WORKFLOW CODEX PROMU ET ACTIF
...
- **Statistiques générales promues sur `main` et validées publiquement par le propriétaire.** `Profil > Statistiques` présente Progression, Gacha, Économie et Activités depuis une projection serveur en lecture seule des sept tables propriétaires. Les 23 métriques, dont le taux de 5★ dérivé sans perte de précision, suivent `GENERAL_STATISTICS` ; `!infos` reste compact. Le propriétaire confirme les quatre groupes, le taux Gacha, les valeurs Économie/Activités, la confidentialité `GENERAL_STATISTICS` et le rendu mobile. Pendant cette validation, `!infos @Pseudo` a révélé un défaut de résolution des références Player ; R907 le corrige dans un lot Chat dédié. Aucune migration Prisma supplémentaire. Les Lots Missions 1 à 5 et le cycle Messages privés restent promus et validés publiquement. Top / classements globaux a ensuite été ouvert. Restent non implémentés les notifications de complétion Missions player-facing, Twitch et la migration legacy R328.
- **Top / Classements globaux V1 candidat en review.** Le registre serveur couvre 32 métriques en cinq catégories ; `RankingService` relit les sources canoniques, filtre strictement les permissions Public et les valeurs positives, puis calcule les rangs compétition avant une pagination de 20. Le taux 5★ est classé par fraction exacte avec 100 Pulls minimum ; le patrimoine Moras additionne portefeuille et Banque seulement si les deux sont Public. La review indépendante a approuvé l'architecture et demandé quatre corrections : une Banque non matérialisée compte pour zéro sans provisioning, Pity s'affiche `X/90` sans modifier son score, l'écran ne montre plus un snapshot ancien après erreur, et un test couvre les ex æquo de part et d'autre d'une page. `GET /api/v1/rankings` authentifié alimente l'écran Classements du Menu global et `!top` utilise le même service pour le Top 5 et le résumé personnel. Aucune migration, récompense, saison, table ni cache Ranking. Nouvelle review indépendante et test public encore attendus.
- **Contrôles du candidat Classements corrigé.** Backend non DB **423/423**, frontend non DB **849/849** avec un worker, tests PostgreSQL isolés Ranking **5/5**, ciblés Ranking/Chat **119/119** et écran Ranking **5/5**, builds frontend/backend et `verify:quick` exécutés. La suite frontend parallèle du lot précédent avait reproduit le timeout intermittent préexistant de ChatPanel ; ChatPanel isolé **63/63**, puis suite complète avec un worker **849/849**. Le contrôle visuel local antérieur tenté avec un harnais Edge incomplet ne permet pas de conclure sur le `GameShell` réel ; la validation visuelle/mobile publique reste à faire. Aucun test de déploiement Railway/Cloudflare ni healthcheck n'est revendiqué pour ce candidat.
...
- Domaine actif : **TOP / CLASSEMENTS GLOBAUX — CANDIDAT REVIEW**. Statistiques générales sont validées publiquement par le propriétaire. R907 est publié séparément sur `review` au commit `5b01110` et normalise les seuls arguments Chat qui désignent un Player. Le candidat Classements suit R714–R727, avec écran, API et `!top` physiques ; ni R907 ni Classements ne sont encore validés publiquement. La dernière migration reste `20260925100000_043_add_direct_message_replies` ; DEV et dépôt comptent **43 migrations Prisma**.
- Prochaine étape exacte : **review indépendante du commit Classements sur GitHub, puis promotion dédiée et test public par le propriétaire après approbation**. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
