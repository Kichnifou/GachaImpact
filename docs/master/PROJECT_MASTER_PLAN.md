...
Version : correctif R897 et finition des mutations MP promus sur main
Date : 2026-09-23
Statut : CORRECTIF R897/MP SUR MAIN, DÉPLOIEMENTS ET VALIDATION PUBLIQUE CIBLÉE À VÉRIFIER
...
- **Correctif R897/MP promu sur `main`.** Le commit `d9c9be42d5de5cd8128f73e5e2edbcfbae9bbe43` garde l'optimistic immédiat mais sérialise les nouveaux POST Chat dans l'ordre local, avec au moins 750 ms entre leurs démarrages ; le verrou `Spam, veuillez attendre...` attend aussi la compatibilité réelle du transport lent. R891 et R895 restent **VALIDÉES PUBLIQUEMENT** et ne changent pas. R897 reste **À VALIDATION PUBLIQUE** après vérification des déploiements et correction de la disparition des premiers messages d'une rafale.
...
- **Chat global — état public réconcilié.** R890, R891 révisée, R892, R893 révisée, R894 et R895 révisée sont validées publiquement. Seul R897 attend son nouveau contrôle public. Le serveur capture désormais l'instant de pacing après le verrou Player, sans changer `submissionOrder`, R895 ni les protections 750 ms/rafale/10 sur 10 secondes.
- **MP — R896.** La recherche destinataire est validée publiquement. Les deux seuls micro-ajustements de ce correctif sont la copie exacte `Écrivez le premier message.` et le centrage horizontal de chaque onglet `Conversations | Archives` ; ils restent à valider publiquement.
- **MP avancés — R504/R513/R514 présents, ergonomie finale à revalider.** L'auteur modifie et supprime via une barre absolue `✎ | ×`, ou restaure via `↶`, sans bouton `⋯` par message ni déplacement de bulle. L'éditeur est rendu entièrement visible avec focus et caret final ; la confirmation de suppression flotte sous la bulle. Les mutations edit/delete/restore renvoient leur patch autoritatif et le cache l'adopte immédiatement, avec restauration optimistic lorsque le contenu supprimé est encore disponible en mémoire. La fenêtre/purge R504 reste strictement inchangée. Aucun Historique complet, recherche/date, signalement ou accès admin MP n'est ajouté.
- **Historique complet MP R515 uniquement audité.** Aucun code R515 n'est présent. Le prochain lot dédié devra séparer cette vue du fil courant borné à 500, paginer par `submissionOrder`, ajouter scroll continu, recherche serveur et navigation par date, exclure les messages supprimés de la recherche et étudier les index avec `EXPLAIN` avant toute migration éventuelle.
...
- Base DEV : migrations 036–039 appliquées via Prisma ; **39 migrations Prisma** suivies. La migration 039 et `submissionOrder` restent immuables ; ce correctif n'ajoute **aucune migration 040**. Le DTO de mutation MP renvoie seulement `id`, `content`, `editedAt`, `deletedAt` et `restoredAt`; un tombstone conserve toujours `content = null`. RLS et droits restent inchangés ; aucun Player, seed économique, import legacy ou réglage Privacy n'est modifié.
- Validation technique du candidat `d9c9be4` : frontend **760/760 sur 85 fichiers**, dont `ChatPanel` **62/62** et `DirectMessagePanel` **34/34** ; backend **389/389 sur 54 fichiers** ; routes MP **2/2** ; PostgreSQL MP **12/12**. La passe PostgreSQL Chat atteint **24/26** : le scénario R897 modifié réussit, tandis que deux anciens scénarios Banque/Conversion expirent sur leur timeout fixe de 5 s avec le pool distant, sans preuve d'une régression produit R897. Builds frontend/backend, typecheck serveur, lint et diff-check réussissent ; le lint conserve seulement deux avertissements préexistants dans `ContestScreen`. Le vrai `GameShell` et les CSS de production ont été contrôlés aux quatre résolutions, sans que cela constitue un déploiement ni une validation publique.
...
- Le lot Chat player-facing est validé publiquement pour R890, R891 révisée, R892, R893 révisée, R894 et R895 révisée. R897 corrigée et les seuls micro-ajustements R896 attendent leur validation publique. R504/R513/R514 sont présents mais leur ergonomie finale doit être revalidée après ce correctif.
...
- Domaine actif : **vérification du déploiement du correctif R897/MP présent sur `main`, puis validation publique ciblée**, sans déclarer ces étapes acquises par la seule promotion Git.
- Prochaine étape exacte : **ChatGPT vérifie `main == review`, Railway `SUCCESS`, `Application ready`, `/health` et le frontend si possible ; Axel teste ensuite la rafale Chat 1/2/3 et le verrou lent, le hint et les tabs R896, puis les actions, l'édition visible, la confirmation de suppression et Restore MP**. Après réussite, un checkpoint consigne la validation publique ; seulement ensuite vient le lot R515 Historique complet/recherche/date. R891 et R895 ne sont pas rouvertes. Signalement/admin MP, médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportés. `PAID_INFRA_APPROVED = false`.
