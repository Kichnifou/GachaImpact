...
Version : Missions permanentes — Lots 1 à 3 validés publiquement ; Lot 4 candidat sur review
Date : 2026-09-25
Statut : LOTS 1 À 3 ET PAGINATION/LOADERS PROMUS ET VALIDÉS PUBLIQUEMENT ; LOT 4 API/ÉCRAN MISSIONS CANDIDAT SUR `review` ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 4 CANDIDAT SUR `review`**. Les Lots 1 à 3, pagination/loaders et le correctif responsive Boutique `7106f39` sont validés publiquement ; le bootstrap demeure stable et publiquement acceptable avec sa latence résiduelle non bloquante. L’API personnelle et le vrai écran Missions sont physiques dans le candidat, le secret Z est appliqué côté serveur et aucune migration 043 n’est créée.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai SHA Lot 4**. Une promotion vers `main` exigera ensuite une mission dédiée. Restent non implémentés : `!mission`, Missions sur Profil et consultation tierce selon confidentialité, notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
