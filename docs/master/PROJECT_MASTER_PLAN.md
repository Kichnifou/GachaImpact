...
Version : correctifs publics et mutations MP promus sur main
Date : 2026-09-23
Statut : LOTS A ET B CHAT/MP PROMUS SUR MAIN, VALIDATIONS PUBLIQUES CIBLÉES EN ATTENTE
...
- **Lot A micro-corrections Chat/MP promu sur `main`.** R895 est désormais **VALIDÉE PUBLIQUEMENT**. R891 reste à revalider sur son action Message privé : icône seule accessible et overlay compact sur desktop, texte `Message privé` sur tactile. R897 rend non modifiable le composer uniquement pendant le verrou long quatre secondes, avec `Spam, veuillez attendre...`; les 750 ms restent silencieuses et éditables.
...
- **Chat global — état public réconcilié.** R890, R892, R893 révisée, R894 et R895 révisée sont validées publiquement. Seuls les éléments modifiés de R891 et le nouveau comportement R897 attendent leur contrôle public.
- **MP — finition R896 sur `main`, à revalider sur les éléments modifiés.** Onglets discrets, recherche destinataire légère à 50 ms protégée contre les réponses obsolètes, choix en un clic, deep-link nettoyant immédiatement l'ancien état et bulle optimistic sans opacité grise complètent le vertical. Les comportements déjà validés — cache, ordonnancement live, statuts, lecture, scroll et polling — ne sont pas rouverts.
- **MP avancés — R504/R513/R514 sur `main`, à valider publiquement.** L'auteur peut modifier inline, supprimer pour tous sans retirer la ligne puis restaurer tant que le message reste dans les 500 derniers. Le DTO masque le contenu d'un tombstone aux deux participants. Une suppression conserve le contenu serveur avec 499 messages plus récents, mais le purge immédiatement et atomiquement avec 500 messages plus récents ; l'ajout d'un MP conserve en parallèle la purge des tombstones qui sortent ensuite de la fenêtre. Menu propre `⋯`, confirmation compacte, mention `Modifié`, aperçu `Message supprimé`, optimistic/rollback/protection stale et retry ambigu avec même clé sont physiques. Aucun Historique complet, recherche/date, signalement ou accès admin MP n'est ajouté.
...
- Base DEV : migrations 036–039 appliquées via Prisma ; **39 migrations Prisma** suivies. La migration 039 et `submissionOrder` restent immuables ; ce lot promu n'ajoute **aucune migration 040**. Le DTO MP projette seulement à l'auteur la clé d'intention déjà portée par `BusinessOperation`. RLS et droits restent inchangés ; aucun Player, seed économique, import legacy ou réglage Privacy n'est modifié.
- Validation technique du lot promu : preuves frontend inchangées à **757 tests / 85 fichiers**, dont `DirectMessagePanel` **31/31** ; le correctif R504 ne modifie aucun fichier frontend et ne relance pas ces suites. Le backend complet passe **389/389 tests sur 54 fichiers**, les routes MP **2/2** et PostgreSQL MP isolé **12/12**. Le nouveau scénario bulk verrouille sans envoi ultérieur le rang 500 restaurable, le rang 501 immédiatement purgé/non restaurable et le replay idempotent sans changement de `contentPurgedAt`. Build et typecheck backend ainsi que diff-check réussissent. La validation visuelle précédente du vrai `GameShell` aux quatre résolutions reste acquise et ne constitue ni déploiement ni validation publique.
...
- Le lot Chat player-facing est validé publiquement pour R890, R892, R893 révisée, R894 et R895 révisée. R891 attend une revalidation limitée à son action Message privé ; R897 et les seuls éléments R896 modifiés attendent leur validation publique. Les mutations MP avancées sont sur main et attendent elles aussi leur validation publique.
...
- Domaine actif : **validation publique ciblée du Chat/MP promu sur `main`**. Le lot A porte les micro-corrections publiques ; le lot B porte l'édition, la suppression et la restauration MP sans migration.
- Prochaine étape exacte : **ChatGPT vérifie `main == review`, Railway, `Application ready`, `/health` et la disponibilité frontend lorsque possible, puis Axel effectue le test public ciblé** : R891 icône/overlay et R897 spam lock ; onglets/recherche/sélection/deep-link/optimistic MP ; Modifier, `Modifié`, Supprimer, `Message supprimé` et Restaurer. Seulement après retour propriétaire, un checkpoint documentaire de validation publique est créé. Historique complet, recherche/date dans l'historique, signalement/admin MP, médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportés. `PAID_INFRA_APPROVED = false`.
