...
Version : Missions permanentes — Lot 5 validé publiquement ; polish MP testé publiquement, deux correctifs candidats ; workflow Codex actif
Date : 2026-09-25
Statut : LOTS MISSIONS 1 À 5 PROMUS ET VALIDÉS PUBLIQUEMENT ; R898 MP ET SÉCURISATION DES FINS D'ENVOI PROMUS, CŒUR DES RÉPONSES OBSERVÉ PUBLIQUEMENT MAIS CYCLE MP NON ENCORE VALIDÉ ; POLISH MP PROMU ET TESTÉ PUBLIQUEMENT, DEUX CORRECTIFS CANDIDATS SUR REVIEW ; WORKFLOW CODEX PROMU ET ACTIF ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : derniers correctifs candidats du polish MP après test public.** Les Lots 1 à 5 sont promus et validés publiquement ; le propriétaire confirme premier rang incomplet, non-terminées d'abord, navigation Missions, `!mission` et Missions Profil. Le Lot 5 active aussi Privacy `MISSIONS`. Restent non implémentés les notifications de complétion player-facing, Twitch et la migration legacy R328.
- **R898 MP approuvée et promue ; cœur des réponses observé publiquement, cycle MP non encore validé.** Le propriétaire confirme réponse à autrui, réponse à soi et aperçu visible en production. La migration additive 043, présente sur `main` et appliquée sur DEV, porte les réponses persistantes dans une même conversation ; liste, fil récent, historique, recherche et preuves de signalement projettent leur contexte selon le contrat dynamique/figé.
- **Sécurisation frontend des fins d'envoi MP approuvée et promue, non encore validée publiquement.** Une génération de session invalide les effets tardifs sur brouillon, réponse, navigation et focus après changement de conversation, de vue, d'onglet ou de Player, y compris lors d'un aller-retour. Un échec tardif reste récupérable explicitement dans sa conversation d'origine avec sa clé de retry ; une initiation tardive ne sélectionne plus son résultat après sortie du parcours. Aucun changement backend ni migration supplémentaire.
- **Polish MP promu et testé publiquement ; cycle complet non encore validé.** Dans une conversation existante, l'envoi actif libère immédiatement le composer et son focus. Une seule intention suivante peut attendre, texte et réponse visibles dans un composer grisé ; elle garde sa clé UUID et ne démarre qu'après succès du précédent. Pendant cette attente, les actions Répondre sont désactivées et la cible capturée ne peut pas changer. Un échec libère cette intention comme brouillon éditable, garde l'envoi échoué récupérable avec sa clé et ne lance aucun envoi automatique. La navigation invalide la queue et la rend récupérable uniquement à l'origine. Le propriétaire confirme sur téléphone réel le vertical MP, l'ouverture sur les messages récents, le scroll tactile haut/bas et le composer visible ; il confirme aussi M1→M2 queued, la réponse et sa cible gelée, les actions sur messages longs, la confirmation Supprimer auto-scrollée et le comportement général desktop/mobile. Deux défauts subsistent avant clôture : focus absent à la promotion queued→active pendant l'envoi suivant et espace vide permanent dans les bulles. Leurs corrections sont candidates sur `review`, sans validation publique à ce stade.
...
- Base DEV : migrations 036–043 appliquées via `prisma migrate deploy` ; **43 migrations Prisma** suivies. La 041 initialise les lignes Missions sans économie ; la 042 ajoute uniquement le marqueur nullable de rattrapage R301, sans exécuter ce rattrapage ni toucher un wallet ; la 043 ajoute seulement la relation nullable de réponse MP, sa FK restrictive et son index. `prisma migrate dev` reste historiquement inadapté à cette base partagée en raison du P3006 de la migration 010 dans le shadow database ; le workflow retenu reste `migrate deploy` puis `migrate status`.
...
- Validation automatisée de la révision R884 : 378 tests backend sur 51 fichiers, typecheck, build et lint ciblé réussis. Suites PostgreSQL exécutées séparément et réussies : Chat (13), Gacha (23), Roue (1), Combat quotidien (8), Boss (8), Social (8), Échanges (21), Expédition (9), Event de base (8), Boutique Event (6), Classement Event (1), Jeu B (7), Jeu C (7, dont le passage réel Chat → EventService), Calendrier Event (5). La suite PostgreSQL Concours complète n'a pas été relancée dans ce lot : au checkpoint précédent sur DEV partagée, elle ne passait pas (11/25), avec un scénario de salon reproduit seul et deux Concours terminés préexistants contrariant l'attente d'historique isolé. Seule la frontière de lecture `!concours` est vérifiée ici ; le domaine Concours n'est pas déclaré validé. **Historique R884 :** aucune migration nouvelle n'était alors ajoutée et DEV comptait 33 migrations. **État courant :** les migrations Missions 041–042 et MP 043 sont appliquées, avec 43 migrations suivies sur DEV.
- Le cycle Chat/MP antérieur est **CLÔTURÉ PUBLIQUEMENT** : R890–R897 et R504–R517 sont acquis. R898 et la sécurisation des fins d'envoi MP sont approuvées et promues ; les réponses à soi/autrui et leur aperçu sont observés publiquement pour R898. Le polish MP est promu et testé publiquement sur desktop/mobile, dont téléphone réel ; ses deux derniers défauts font l'objet de correctifs candidats sur `review`. Le cycle MP complet reste non validé jusqu'au dernier test propriétaire après review et promotion de ces correctifs.
...
- Domaine actif : **DERNIERS CORRECTIFS CANDIDATS DU POLISH MP ; WORKFLOW CODEX ACTIF ; LOT 5 MISSIONS PROMU ET VALIDÉ PUBLIQUEMENT**. Les Lots 1 à 5, pagination/loaders et le correctif responsive Boutique `7106f39` sont validés publiquement ; le bootstrap demeure stable et publiquement acceptable avec sa latence résiduelle non bloquante. R898, la sécurisation MP et le polish sont promus ; plusieurs comportements du polish sont confirmés publiquement, mais ses deux correctifs candidats et le cycle MP complet ne le sont pas. La migration 043 appartient seulement au premier correctif MP ; DEV et le dépôt restent à 43 migrations, sans 044.
- Prochaine étape exacte : **review indépendante ChatGPT du SHA du dernier correctif MP candidat sur `review`**. Après approbation, promotion et dernier test public propriétaire des deux corrections, clôturer le cycle MP complet puis suivre la procédure de passation ChatGPT/Codex avant le prochain domaine. Aucun succès de déploiement ni validation publique de ces corrections n'est revendiqué. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
