...
Version : Missions permanentes — Lot 2 validé publiquement ; stabilisation pagination/bootstrap promue, en attente de validation publique
Date : 2026-09-24
Statut : LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT ; STABILISATION TRANSVERSE APPROUVÉE PAR REVIEW INDÉPENDANTE ET PROMUE SUR `main`, À VALIDATION PUBLIQUE ; LOT 3 NON COMMENCÉ
...
- **Domaine actif : Missions permanentes B/A/S/Z — Lots 1 et 2 promus et validés publiquement.** Chat/XP, Pulls Gacha, possessions 4★/5★, C6 Pull/Stella, niveau Player, Moras gagnées, particules principales générées et intérêts Banque sont raccordés dans leurs transactions propriétaires. Expedition, Combat et Social/cœurs restent explicitement au Lot 3 non commencé ; aucun écran/API final, `!mission`, Profil, notification player-facing, Twitch ou import legacy n’est ajouté.
...
- **Stabilisation transverse pagination/bootstrap approuvée par review indépendante et promue sur `main`.** Les callbacks de lecture transmis par `AppBootstrap` sont stables et les composants auto-chargeants ne traitent plus une nouvelle identité de fonction comme un signal métier. Le bootstrap conserve les lectures réellement indépendantes en parallèle, sérialise les GET de provisioning/réconciliation/verrou Player, exécute Notifications avant la lecture Expedition autonome et retente une seule fois les erreurs réseau/5xx, jamais les 4xx. Ce lot ne change aucun gameplay Missions et n’ajoute aucune migration ; il reste à validation publique ciblée.
- **Validation technique de la stabilisation.** Frontend 794/794 sur 87 fichiers, tests ciblés pagination/bootstrap, typecheck, build Vite et lint réussissent ; le lint conserve seulement les deux avertissements préexistants de `ContestScreen`. Les suites backend/DB et leurs builds ne sont pas relancés car aucun fichier backend, Prisma ou migration n’est modifié. L’inspection Chromium authentifiée et la validation publique restent à effectuer ; les tests DOM interactifs prouvent déjà l’absence de requête supplémentaire lors de rerenders parents répétés.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT**. La migration 042 est présente sur `main` et déjà appliquée/suivie sur DEV. La stabilisation transverse pagination/loaders/bootstrap est promue sur `main`, mais pas encore validée publiquement. Le Lot 3 n’est pas commencé.
- Prochaine étape exacte : **vérification post-promotion ChatGPT** : confirmer `main == review`, les déploiements Cloudflare Pages et Railway, `Application ready` et `/health`; puis mener une validation publique ciblée de la pagination Banque/Gacha, du comportement pendant Expedition `RUNNING`, du reload complet et de l’absence de nouveaux P2028 dans Railway. Ces preuves restent à acquérir. Restent au Lot 3 non commencé : Expedition, Combat, Social/cœurs et leur restitution spécifique. Restent aux lots ultérieurs : route/UI Missions, `!mission`, Profil, notifications player-facing, Twitch et reprise legacy R328. `PAID_INFRA_APPROVED = false`.
