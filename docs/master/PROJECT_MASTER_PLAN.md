...
Version : promotion Missions permanentes — Lot 1 socle backend/DB
Date : 2026-09-24
Statut : MISSIONS PERMANENTES LOT 1 APPROUVÉ PAR REVIEW INDÉPENDANTE ; PROMU SUR `main` ; NON VALIDÉ PUBLIQUEMENT
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 1 SOCLE BACKEND/DB PROMU**. La cible produit R299–R339 reste documentée ; l’état physique 041 et le service central sont présents sur `main`, mais aucune validation publique fonctionnelle n’est encore acquise.
- Prochaine étape exacte : **vérification ChatGPT de la promotion réelle** : confirmer `main == review`, les déploiements Railway/Cloudflare et le healthcheck, puis fournir une checklist publique proportionnée au diff. Le Lot 2 n’est pas commencé ni autorisé avant cette vérification et l’arbitrage explicite du propriétaire. Restent reportés : raccords producteurs, rattrapage économique standalone via le moteur, route/projection player-facing, UI Missions, `!mission`, Profil, restitution/notifications et Twitch. `PAID_INFRA_APPROVED = false`.
