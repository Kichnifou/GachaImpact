...
Version : Missions permanentes — Lot 3, optimisation bootstrap et micro-polish Boutique promus sur `main`, en attente de validation publique
Date : 2026-09-25
Statut : LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT ; PAGINATION/LOADERS VALIDÉS PUBLIQUEMENT ; LOT 3, OPTIMISATION BOOTSTRAP ET MICRO-POLISH BOUTIQUE APPROUVÉS PAR REVIEW INDÉPENDANTE ET PROMUS SUR `main`, NON ENCORE VALIDÉS PUBLIQUEMENT
...
- **Domaine actif : Missions permanentes B/A/S/Z — Lot 3 approuvé par review indépendante et promu sur `main`, non encore validé publiquement.** Les Lots 1 et 2 restent promus et validés publiquement. Le Lot 3 raccorde désormais Expedition, Combat quotidien et Social/cœurs, dont les métriques Z Amitié parfaite et victoires manuelles, dans leurs transactions propriétaires : tous les producteurs autoritatifs du catalogue permanent B/A/S/Z sont donc physiquement raccordés. Aucun écran/API Missions final, `!mission`, Profil, notification player-facing, Twitch ou import legacy n’est ajouté.
...
- **Pagination/loaders validés publiquement ; bootstrap fiable mais performance encore ouverte sur `main`.** Le propriétaire confirme la stabilité des historiques Banque/Gacha et l’absence de clignotement des boutons. Cloudflare Pages et Railway étaient `SUCCESS`, `Application ready` et `/health` HTTP 200 confirmés. Aucun nouveau P2028 de bootstrap utilisateur n’a été observé sur le nouveau conteneur ; le P2028 vu pendant le déploiement appartenait à l’ancien conteneur en SIGTERM. Avant le présent candidat, Notifications mesurait publiquement environ 3,6–4,0 s, avec un pic vers 6,3 s.
- **Optimisation bootstrap promue, non validée publiquement.** Notifications retourne aussi la projection Expedition qu’il vient de réconcilier ; le frontend conserve un fallback vers le GET autonome pendant un déploiement roulant. Après Notifications, la voie à verrou Player reste ordonnée `Expedition` de fallback → Teams → Event, tandis que Défi quotidien, Combat quotidien, Boss mensuel et Concours s’exécutent en parallèle. Dans Notifications, Expedition précède toujours les réconciliations ; Codes cadeaux peut ensuite progresser en parallèle d’une voie Event où messages puis cycle de vie restent séquentiels. Le timer Expedition READY recharge seulement Notifications. Aucun timeout n’est augmenté et les endpoints individuels restent autonomes.
- **Mesures locales comparables du bootstrap promu.** Sur le même processus local et la même base DEV, quatre lectures Notifications avant modification donnaient 869–934 ms et après modification 557–723 ms ; ce profil exclut HTTP/auth/réseau et ne remplace pas la mesure publique. Un harnais déterministe utilisant les mêmes délais par reader donne 1 688–1 718 ms avec l’ancien ordonnanceur contre 964–975 ms avec le nouveau. La validation publique connexion/Ctrl+F5 et la surveillance Railway restent requises.
- **Lot 3 Missions promu, non validé publiquement.** Expedition réconcilie une fois `EXPEDITIONS_COMPLETED` et, selon la récompense, Moras ou particules après le claim ; Combat réconcilie victoire, victoire manuelle seulement en MANUAL et Moras après la tentative gagnante ; Social agrège les cœurs sortants en une passe sender et réconcilie Amitié parfaite pour les deux participants lors du passage réel à 1000, sans catch-up du destinataire passif. Les canaux UI/INTERNAL_CHAT et les opérations déclenchantes réelles sont conservés. Catch-up avant mutation, crédits Economy sans récursion, replay et rollback restent dans la transaction propriétaire.
- **Micro-polish Boutique promu, non validé publiquement.** Le libellé `Achat unitaire` du Ticket est supprimé ; le prix et le bouton réutilisent la géométrie normale des contrôles des deux cartes, sans prix, probabilité ni règle d’achat modifiés. La validation visuelle publique reste à faire.
- **Contrôles du Lot 3 approuvé.** Frontend 796/796 sur 87 fichiers et backend 396/396 sur 56 fichiers réussissent. PostgreSQL réussit fichier par fichier : Missions 11/11, Expedition 11/11, Combat 9/9, Amitié 16/16 et Social 8/8. Typecheck backend, builds frontend/backend, lint, diff-check, Prisma validate/status et contrôle des fixtures réussissent ; 42 migrations restent suivies et appliquées. Le lint conserve uniquement les deux avertissements préexistants de `ContestScreen`. Aucun moteur de navigateur mesurable n’étant installé localement, l’alignement Boutique est couvert par DOM/CSS et reste à vérifier visuellement après déploiement.
...
- **Backend/DB MP 036–040 sur main.** Les migrations 036–039 portent conversations, participants, demandes, messages, lecture partagée et ordre serveur ; la 040 ajoute uniquement le dossier privé `direct_message_reports`. `DirectMessageService` projette `canSend` et `blockedByMe` depuis les mêmes règles PUBLIC/FRIENDS/PRIVATE/blocages que l'envoi. Le blocage archive physiquement les deux participants ; le déblocage partagé retire seulement le blocage de l'acteur, ne restaure ni amitié ni archives. Un nouvel envoi autorisé désarchive les deux côtés. Le socle Missions 041 est désormais également présent sur `main`, sans modifier ces comportements Chat/MP.
...
- Base DEV : migrations 036–042 appliquées via `prisma migrate deploy` ; **42 migrations Prisma** suivies. La 041 initialise les lignes Missions sans économie ; la 042 ajoute uniquement le marqueur nullable de rattrapage R301, sans exécuter ce rattrapage ni toucher un wallet. `prisma migrate dev` reste historiquement inadapté à cette base partagée en raison du P3006 de la migration 010 dans le shadow database ; le workflow retenu reste `migrate deploy` puis `migrate status`.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 3 APPROUVÉ PAR REVIEW INDÉPENDANTE ET PROMU SUR `main`, NON ENCORE VALIDÉ PUBLIQUEMENT**. Lots 1/2 et pagination/loaders restent validés publiquement. Tous les producteurs autoritatifs B/A/S/Z sont raccordés ; aucune migration 043 n’est créée. Le bootstrap optimisé et le micro-polish Ticket sont promus, sans validation publique encore acquise.
- Prochaine étape exacte : **vérification post-promotion ChatGPT** : confirmer `main == review`, Cloudflare Pages, Railway, `Application ready`, `/health`, logs/P2028 et les mesures publiques connexion/Ctrl+F5/Notifications. Puis le propriétaire validera connexion normale, Ctrl+F5, alignement Ticket, smoke Expedition, Combat et Social/cœur. Restent aux lots ultérieurs : route/UI Missions, `!mission`, Profil, notifications player-facing, Twitch et reprise legacy R328. `PAID_INFRA_APPROVED = false`.
...
Pour la prochaine étape exacte, consulter le bloc **Reprise rapide — état vivant** au début du Master. Batch A et Batch B Social Foundations sont promus et validés publiquement ; le Calendrier de Noël naturel reste à observer en décembre. Le prochain lot est Batch C : polish public puis Amitié complète. L'historique Event transversal, Twitch/chat et la migration legacy restent hors de Batch B et ne sont pas développés dans ce checkpoint documentaire.
