...
Version : Chat global + MP ; fondations Chat global promues, orchestrateur de commandes candidat sur `review`
Date : 2026-09-22
Statut : LOT 1 CHAT GLOBAL PROMU ET DÉPLOYÉ ; ORCHESTRATEUR COMMANDES EN REVIEW
...
- Base DEV : **33 migrations Prisma** à jour ; `032` demeure intacte et `20260922080000_033_fix_global_chat_content_check` corrige additivement sa contrainte de contenu interne. Les migrations sont appliquées via Prisma et vérifiées par checksum, `prisma migrate status`, contrainte, RLS et grants. Migrations 001–031 intactes. Aucun seed économique, import legacy ou reset Privacy.
...
- Domaine actif : **Chat global + messages privés**. Le cadrage produit Chat global reste validé dans [R872–R883](../specifications/global-chat-v1.md) et le [journal](../specifications/decisions-log.md). Le Lot 1 promu matérialise les tables Chat global/read state et `GlobalChatService` : envoi joueur idempotent, totalMessages, XP/countedMessages selon élément standalone valide et cooldown, activité, anti-rafale, réponses relationnelles, lecture cursorisée, non-lus, marquage monotone et suppression auteur. La correction garantit qu’un Player valide à 0 XP reçoit son premier gain ; le cas défensif sans élément ne reçoit pas d’XP. L'orchestrateur serveur candidat persiste les réponses publiques `GAME_RESULT` (en plusieurs parties atomiques au-delà de 500 caractères), réutilise les services métier et leur idempotence, et expose le registre à `!help`.
- Commandes physiquement branchées : `help`, `element` (choix défensif), `banniere`, `select`, `vote`, `pity`, `pull` (1..10), `obtention`, `stella`, `passifs`, `roue`, `ami`, `echanger`, `infos`, `liste`, `banque`, `convertir`, `sac`, `coffre`, `shop`, `code`, `event` (inscription, état, boutique, classement, conversions, Collection, calendrier, jeux A/B/C), `expedition` (état, départ, retour), `concours` (salon, participation, spectateur, tours, soutien), `combat` (quotidien, Auto, matrice, statistiques, Boss et attaques), `quotis`. `box` et `team` restent des consultations synthétiques, sans leurs sous-commandes legacy ; `!echanger annuler` sans pseudo exige une demande unique car `TradeService` ne propose pas d'annulation globale. `top` attend un classement global physique ; `legende` attend un service adapté ; `mission` et `faveur` attendent leur domaine. `giveaway` et `wish` restent Twitch uniquement. Le `ChatPanel` reste mocké ; aucune API Chat navigateur, aucun MP physique ni pont Twitch/Realtime. Les réponses privées de commandes, annonces globales automatiques et extension générale UI → Chat restent reportées au polish final ; R633/R634 restent différées vers Historique global.
- Validation automatisée du candidat corrigé : 362 tests backend, typecheck, build et lint ciblé réussis ; suites PostgreSQL exécutées séparément et réussies pour Chat (13), Gacha (23), Roue (1), Combat quotidien (8), Boss (8), Social (8), Échanges (21), Expédition (9), Event de base (8), Boutique Event (6), Classement Event (1), Jeu B (7), Jeu C (6) et Calendrier Event (5). La suite PostgreSQL Concours sur DEV partagée a été exécutée mais ne passe pas (11/25) : un scénario de salon échoue aussi seul, et deux Concours terminés préexistants contredisent l'attente d'historique isolé ; aucun code propriétaire Concours n'a été modifié dans cette correction. Le résultat de cette suite n'est pas présenté comme validé. Aucune migration nouvelle ; `prisma migrate status` confirme les 33 migrations appliquées sur DEV.
- Prochaine étape : review indépendante et promotion dédiée du candidat orchestrateur corrigé, puis activation API/transport et frontend du Chat global avec revalidation immédiate des projections UI affectées par chaque commande mutationnelle. Ce futur lot branchera le Chat réel, les non-lus, réponses, mentions, suppression et signalement. Aucun parcours Chat player-facing n'est déclaré commencé ou validé publiquement.
