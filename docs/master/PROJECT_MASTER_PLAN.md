...
Domaine actif :
**Team / Équipe / Passifs — le raccord des sept passifs au moteur Pull est déployé et ses comportements métier critiques ont été validés publiquement par le propriétaire. Le polish public final 0.71 corrige localement sur `review` le panneau Team à quatre passifs, la lecture des niveaux actifs dans Détail, l’alignement des feedbacks, l’Historique, le feedback C6 individuel et le vocabulaire `Primos` ; ces corrections restent à reviewer puis à revalider publiquement.**
...
- lot Team Management 0.70 : **DÉPLOYÉ** ; le polish local 0.71 réserve désormais structurellement la place du bouton `Voir les passifs` avec quatre passifs sur desktop 1920×1080, sans dégrader le rendu 2560×1440 ni le mobile ; ce correctif précis reste à revalider publiquement ;
- lot Team / Gacha 0.71 : **DÉPLOYÉ ET VALIDÉ PUBLIQUEMENT POUR SES COMPORTEMENTS MÉTIER CRITIQUES** ; Focus sans passif, anti-spoil, Pyro, Géo, Cryo, Électro, Anémo, Dendro, Hydro silencieux/non causal, récompenses C6/XP, coexistence multi-passifs x10, changement de Team et non-régressions générales ont été validés par le propriétaire. Le polish frontend de ce lot reste à reviewer puis à revalider publiquement.
...
La Team autoritative et ses sept passifs modifient désormais le moteur Pull dans une seule passe cohérente, transactionnelle, snapshotée et auditable. Les comportements métier critiques de ce raccord sont validés publiquement ; seuls les correctifs de polish frontend 0.71 restent à reviewer puis à revalider publiquement.
...
Premier lot Team réel déployé et validé publiquement ; Team Management et son raccord Gacha 0.71 sont déployés, et les comportements métier critiques des sept passifs sont validés publiquement. Le polish frontend final reste local sur `review` :
...
- l'UI affiche les effets réellement déclenchés seulement après Focus et dans le récapitulatif x10 ; l'Historique conserve sa progression traditionnelle sans feedback passif. Les lignes icône + texte sont centrées comme un bloc commun. Le récapitulatif compact déduplique les crédits Anémo et le bundle Dendro déjà représentés par leur feedback passif, tout en conservant les remboursements C6 et récompenses de niveau XP ; un proc Électro plafonné sans gain réel reste masqué. Une progression C6 de type statistique apparaît aussi au centre de la révélation individuelle uniquement en phase Ready, au-dessus du hint, sans régresser son rendu x10. Hydro demeure un modificateur enregistré mais n'est jamais présenté comme cause d'un 5★. Progression, ressources et Pity finales suivent le disclosure anti-spoil existant. Aucune migration supplémentaire n'a été nécessaire ;
- le libellé court player-facing de la monnaie d'Invocation est `Primos` dans l'interface normale ; les contrats techniques, clés API/JSON et champs de données conservent `primogems`.
...
**Faire reviewer le candidat de polish public final 0.71 uniquement sur `review` → corrections éventuelles → promotion fast-forward vers `main` seulement après approbation → revalidation publique propriétaire du bouton Team à quatre passifs en 1920×1080, des niveaux actifs dans Détail, de l’alignement des feedbacks Ready/x10, de l’Historique sans passifs avec pagination visible, du gain C6 individuel post-Focus et du vocabulaire `Primos` → clôturer Team / Passifs puis reprendre l’ordre V1 dans `docs/roadmap/implementation-order-v1.md`.** `PAID_INFRA_APPROVED = false` reste inchangé.
