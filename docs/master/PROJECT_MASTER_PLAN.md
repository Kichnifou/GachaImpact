...
Domaine actif :
**Team / Équipe / Passifs — premier lot réel déployé et validé publiquement ; Team Management 0.70 est déployé et reste en attente de validation publique propriétaire. Une passe ciblée de finition est implémentée sur `review` : proportions de cartes et slots vides stabilisées, quatre passifs en 2×2, bouton de référentiel compact, feedback de permutation renforcé et contrat technique des futurs effets Gacha préparé sans raccord au Pull.**
...
- lot Team Management 0.70 : **DÉPLOYÉ, VALIDATION PUBLIQUE PROPRIÉTAIRE RESTANTE** ; sa passe ciblée de finition est implémentée sur `review`, sans nouvelle migration ni raccord des passifs au moteur Pull.
...
Premier lot Team réel déployé et validé publiquement ; Team Management est déployé et son polish UX/visuel 0.70 est implémenté sur `review` :
...
- sur desktop standard, Team tient sans scroll vertical normal et les trois colonnes utilisent la hauteur disponible ; la sidebar conserve ses panneaux naturels et son dernier panneau Récompense quotidienne absorbe seul l'espace restant sans étirer son contenu. Le bouton crayon de 29 px reste centré sur le titre. Mobile et petits viewports conservent leur scroll naturel, une hauteur raisonnable pour les slots vides et leurs contrôles de réorganisation de secours ;
- raccord Gacha différé : le moteur Pull ne consomme encore aucun stack Team. La configuration machine-readable prépare Pyro/Hydro/Cryo/Electro/Anemo/Geo/Dendro avec leurs valeurs R75–R84 et cap 2, mais la prochaine passe Team doit d'abord fournir le gain d'XP serveur central requis par Cryo, puis raccorder et snapshotter ensemble les sept effets, sans activation partielle, modification anticipée des probabilités ni calcul frontend.
...
**Faire reviewer le candidat de finition Team uniquement sur `review` → corrections éventuelles → promotion vers `main` seulement après approbation → validation publique propriétaire des gabarits 0..4, du cadrage générique des portraits, du 2×2 à quatre passifs, du bouton compact, du feedback `Échanger`, du crayon et du responsive → passe Team de raccord simultané des sept passifs au Pull après ajout de la primitive XP serveur requise par Cryo.** `PAID_INFRA_APPROVED = false` reste inchangé.
