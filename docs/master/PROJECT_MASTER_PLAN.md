...
Version : micro-corrections publiques Chat/MP candidates sur review
Date : 2026-09-23
Statut : LOT A CHAT/MP CANDIDAT REVIEW, NON PROMU
...
- Domaine actif : **deux lots séquentiels Chat/MP sur `review`**. Le lot A porte les micro-corrections publiques ci-dessus ; le lot B suivant porte l'édition, la suppression et la restauration MP sans migration. Le socle validé sur `main` reste inchangé tant qu'aucune mission de promotion dédiée n'est autorisée.
- Prochaine étape exacte : terminer et publier séparément les lots A puis B sur `review`, faire contrôler les vrais commits GitHub, puis seulement une mission dédiée de promotion/déploiement et une validation publique ciblée. Historique complet, recherche/date dans l'historique, signalement/admin MP, médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportés. `PAID_INFRA_APPROVED = false`.
