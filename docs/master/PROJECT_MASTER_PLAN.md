...
Version : Event — micro-polish des Jeux et de la frise approuvé pour promotion
Date : 2026-09-21
Statut : CANDIDAT EVENT REVIEWÉ ET APPROUVÉ ; PROMOTION MAIN AUTORISÉE ; TEST PUBLIC REQUIS
...
- État Event : Lots 1 à 6 clôturés. Le checkpoint `3b47662` a promu les corrections Panier, dont la synchronisation et la réinitialisation du destinataire aux frontières métier. Le candidat `1daa64d9e98d5d40758bafd018c24307f0306e41` retire les grands titres qui répètent Récolte/Grenier/Panier et permet de rétracter la frise des paliers, sans modifier les règles métier. La review indépendante ChatGPT l'a approuvé et la promotion dédiée est autorisée dans cette mission. Le commit parallèle Story `1a0fca5` reste intact dans l'historique.
- Domaine actif : **micro-polish final Event → Jeux**. Récolte, Grenier et Panier gardent leur petit libellé de contexte sans répéter en grand le nom de l'onglet. La frise des paliers se rétracte et se rouvre ; le total de points reste visible et la préférence est conservée localement par joueur. Validation technique acquise : **657 tests frontend / 82 fichiers**, dont **51 tests Event ciblés** ; build, lint et diff-check réussis, avec les deux warnings ContestScreen préexistants. Ce micro-polish attend uniquement sa validation publique après déploiement.
...
- Le checkpoint `3b47662` est sur `main` ; le candidat Event approuvé attend promotion puis validation publique. `PAID_INFRA_APPROVED = false`.
