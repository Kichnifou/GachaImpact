...
Version : Missions permanentes — Lot 2 validé publiquement ; stabilisation pagination/bootstrap candidate
Date : 2026-09-24
Statut : LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT ; STABILISATION TRANSVERSE CANDIDATE SUR `review` ; LOT 3 NON COMMENCÉ
...
- **Stabilisation transverse pagination/bootstrap candidate sur `review`.** Les callbacks de lecture transmis par `AppBootstrap` sont stables et les composants auto-chargeants ne traitent plus une nouvelle identité de fonction comme un signal métier. Le bootstrap conserve les lectures réellement indépendantes en parallèle, sérialise les GET de provisioning/réconciliation/verrou Player, exécute Notifications avant la lecture Expedition autonome et retente une seule fois les erreurs réseau/5xx, jamais les 4xx. Ce candidat ne change aucun gameplay Missions et n’ajoute aucune migration.
- **Validation locale de la stabilisation.** Frontend 794/794 sur 87 fichiers, tests ciblés pagination/bootstrap, typecheck, build Vite et lint réussissent ; le lint conserve seulement les deux avertissements préexistants de `ContestScreen`. Les suites backend/DB et leurs builds ne sont pas relancés car aucun fichier backend, Prisma ou migration n’est modifié. L’inspection Chromium authentifiée reste à effectuer après review/promotion ; les tests DOM interactifs prouvent déjà l’absence de requête supplémentaire lors de rerenders parents répétés.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT**. La migration 042 est présente sur `main` et déjà appliquée/suivie sur DEV. Un lot transverse de stabilisation pagination/loaders/bootstrap est candidat sur `review`. Le Lot 3 n’est pas commencé.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai commit de stabilisation poussé sur `review`**, comparé à `main`, avant toute promotion. Restent au Lot 3 non commencé : Expedition, Combat, Social/cœurs et leur restitution spécifique. Restent aux lots ultérieurs : route/UI Missions, `!mission`, Profil, notifications player-facing, Twitch et reprise legacy R328. `PAID_INFRA_APPROVED = false`.
