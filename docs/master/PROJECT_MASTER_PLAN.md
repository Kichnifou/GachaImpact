...
Version : correctifs publics et mutations MP candidats sur review
Date : 2026-09-23
Statut : LOTS A ET B CHAT/MP CANDIDATS REVIEW, NON PROMUS
...
- **Chat global — état public réconcilié.** R890, R892, R893 révisée, R894 et R895 révisée sont validées publiquement. Seuls les éléments modifiés de R891 et le nouveau comportement R897 attendent leur contrôle public après review/promotion.
- **MP — finition R896 candidate sur `review`.** Onglets discrets, recherche destinataire légère à 50 ms protégée contre les réponses obsolètes, choix en un clic, deep-link nettoyant immédiatement l'ancien état et bulle optimistic sans opacité grise complètent le vertical. Les comportements déjà validés — cache, ordonnancement live, statuts, lecture, scroll et polling — ne sont pas rouverts.
- **MP avancés — R504/R513/R514 candidates sur `review`.** L'auteur peut modifier inline, supprimer pour tous sans retirer la ligne puis restaurer tant que le message reste dans les 500 derniers. Le DTO masque le contenu d'un tombstone aux deux participants ; l'ajout d'un MP purge en SQL les seuls tombstones sortis de la fenêtre. Menu propre `⋯`, confirmation compacte, mention `Modifié`, aperçu `Message supprimé`, optimistic/rollback/protection stale et retry ambigu avec même clé sont physiques. Aucun Historique complet, recherche/date, signalement ou accès admin MP n'est ajouté.
...
- Validation technique de ce candidat : **757 tests frontend / 85 fichiers** et **389 tests backend / 54 fichiers** réussis ; ciblages finaux `DirectMessagePanel` **31/31**, routes MP **2/2** et PostgreSQL MP isolé **11/11**, dont édition/suppression/restauration/idempotence et purge au-delà de 500. Builds frontend/backend, typecheck backend et diff-check réussis ; le lint frontend ne conserve que les deux avertissements `ContestScreen` préexistants et Vite son avertissement de chunk. Le vrai `GameShell`, les vrais panneaux Chat/MP et le CSS de production, alimentés par des réponses synthétiques sans persistance, ont été inspectés à **2560×1440, 1920×1080, 1366×768 et 390×844** : overlay Chat compact sans collision, verrou long et placeholder, tabs, cible MP en un clic, optimistic non grisé, menu auteur, édition, `Modifié`, confirmation de suppression, tombstone et restauration tactile. Les neuf états mesurés n'ont aucun overflow horizontal ; captures et harnais temporaires ont été retirés du lot. Ces preuves ne constituent ni déploiement ni validation publique.
...
- Le lot Chat player-facing est validé publiquement pour R890, R892, R893 révisée, R894 et R895 révisée. R891 attend une revalidation limitée à son action Message privé ; R897 et les seuls éléments R896 modifiés dans ce candidat attendent leur validation publique après review et promotion.
...
- Domaine actif : **deux lots séquentiels Chat/MP candidats sur `review`**. Le lot A porte les micro-corrections publiques ; le lot B porte l'édition, la suppression et la restauration MP sans migration. Le socle validé sur `main` reste inchangé tant qu'aucune mission de promotion dédiée n'est autorisée.
- Prochaine étape exacte : **review indépendante ChatGPT des deux vrais commits candidats sur GitHub**, puis seulement une mission dédiée de promotion/déploiement et une validation publique ciblée. Historique complet, recherche/date dans l'historique, signalement/admin MP, médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportés. `PAID_INFRA_APPROVED = false`.
