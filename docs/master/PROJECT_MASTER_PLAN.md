...
Version : lifecycle pacing Chat finalisé sur le candidat player-facing ; validation publique R890–R892 réconciliée
Date : 2026-09-23
Statut : UI MP CANDIDATE SUR REVIEW, TESTÉE TECHNIQUEMENT ET VISUELLEMENT EN LOCAL, NON VALIDÉE PUBLIQUEMENT
...
- **Candidat review MP player-facing corrigé après `42e1122c1ffe397e526b33ce0302e90d830ef5f1`.** Le panneau droit reste le propriétaire unique `Chat | MP` et tout le vertical MP de `37da8c4` est conservé. Le lifecycle local R894 compte uniquement les messages réellement acceptés : toute `ApiError` déterministe `< 500` et un `!clear` confirmé retirent seulement leur tentative par clé ; une erreur réseau ambiguë la conserve et le retry exact n'ajoute aucun timestamp. Le pacing serveur reste temporel via `createdAt`, indépendamment de `submissionOrder`, et `updates` livre toutes les lignes inconnues de la fenêtre canonique de 200. R895 conserve l'ordre serveur persistant réservé avant les traitements, sans horloge client ni délai d'attente, et une arrivée A tardive se replace immédiatement avant B. R893 place toujours le snapshot Chat au bas avant le premier paint de réouverture. Le candidat n'est pas promu sur main.
- **Backend MP durci pour l'UI.** `DirectMessageService` projette `canSend` et `blockedByMe` depuis les mêmes règles PUBLIC/FRIENDS/PRIVATE/blocages que l'envoi. Le blocage archive physiquement les deux participants ; le déblocage partagé retire seulement le blocage de l'acteur, ne restaure ni amitié ni archives. Un nouvel envoi autorisé désarchive les deux côtés. La migration additive `20260923092102_038_add_direct_message_shared_read_at` ajoute l'instant réel du dernier curseur partagé ; il n'avance qu'avec le curseur et les accusés actifs, sans divulgation rétroactive après OFF puis ON.
- **Chat global — état public réconcilié.** Le retour explicite d'Axel valide publiquement R890 (fenêtre 200), R891 (ordre `Signaler, Masquer, Copier, Mentionner, Répondre`) et R892 (confirmation Signaler sous les actions). La version révisée de R893 avec repositionnement avant paint, R894, R895 et l'UI MP restent à validation publique. L'accès MP reste attaché à l'identité Player hors de cette rangée.
...
- Base DEV : migrations 036–039 appliquées via Prisma ; **39 migrations Prisma** suivies et `migrate status` à jour. Checksum 039 : `bbc626d14f40f950874a69be7adf475768731e3e89fd57df6feacd4c449cd666`. Le backfill déterministe suit `(created_at, id)` ; messages, curseurs de lecture et dernier MP portent l'ordre canonique. RLS reste active et les tables/séquences n'accordent aucun droit `PUBLIC`/`anon`/`authenticated`. Aucun Player, seed économique, import legacy ou réglage Privacy n'a été modifié.
- Validation technique du candidat corrigé : **732 tests frontend / 85 fichiers**, **389 tests backend / 54 fichiers** et **34 scénarios PostgreSQL ciblés** réussis (**26 Chat + 8 MP**). `ChatPanel` ciblé passe à **60/60**. Les preuves nouvelles couvrent les refus déterministes non-pacing sans prolongation des 750 ms ni fausse rafale, leur retour tardif sans effacer une tentative plus récente, la conservation d'une erreur réseau ambiguë et l'exclusion de `!clear`. Les preuves acquises couvrent aussi la course R894+R895 où A réserve avant B mais termine après, la rafale selon les trois temps serveur, le rattrapage exact de 150 inconnus, l'arrivée tardive antérieure, les changements connus et la limite 200. Le backend, PostgreSQL, Prisma et les migrations ne sont pas modifiés par ce correctif ; leurs validations de `fe8c039` restent acquises, dont `migrate status` DEV à 39 migrations. Build frontend réussi. Le lint ne conserve que les deux avertissements `ContestScreen` préexistants ; Vite conserve son avertissement de chunk. Le vrai `GameShell`, le vrai `ChatPanel` et le CSS de production, alimentés par des messages synthétiques sans persistance, ont été inspectés à **2560×1440, 1920×1080, 1366×768 et 390×844** lors du lot précédent : ordre et densité des messages, réponse citée, mention, onglets, composeur et pile mobile restent lisibles sans recouvrement ni overflow horizontal. Captures et harnais ont été retirés du lot. Ces preuves ne constituent ni déploiement ni validation publique.
...
- Domaine actif : **Messages privés (MP)**. Le socle backend/DB, le candidat UI, son correctif R893–R895 et les corrections de review Chat sont physiques sur `review` ; la validation publique de R893 révisée, R894, R895 et MP reste absente. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R895, sans transfert depuis Event Game C.
- Prochaine étape exacte : **review indépendante ChatGPT du candidat review MP corrigé**. Après approbation seulement viendront promotion dédiée sur `main`, vérifications GitHub/déploiements/healthcheck puis checklist et test public Axel. Les fonctions avancées édition, suppression/restauration, historique complet, recherche/date, signalement/admin MP, `!mp`, Twitch et Realtime restent reportées. `PAID_INFRA_APPROVED = false`.
