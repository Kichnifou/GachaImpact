...
Version : Missions permanentes — Lot 3 candidat sur `review` ; optimisation bootstrap et micro-polish Boutique candidats
Date : 2026-09-25
Statut : LOTS 1 ET 2 PROMUS ET VALIDÉS PUBLIQUEMENT ; PAGINATION/LOADERS VALIDÉS PUBLIQUEMENT ; LOT 3, OPTIMISATION BOOTSTRAP ET MICRO-POLISH BOUTIQUE CANDIDATS SUR `review`, NON PROMUS ET NON VALIDÉS PUBLIQUEMENT
...
- **Domaine actif : Missions permanentes B/A/S/Z — Lot 3 candidat sur `review`.** Les Lots 1 et 2 restent promus et validés publiquement. Le candidat raccorde désormais Expedition, Combat quotidien et Social/cœurs, dont les métriques Z Amitié parfaite et victoires manuelles, dans leurs transactions propriétaires. Aucun écran/API Missions final, `!mission`, Profil, notification player-facing, Twitch ou import legacy n’est ajouté.
...
- **Optimisation bootstrap candidate, non validée publiquement.** Notifications retourne aussi la projection Expedition qu’il vient de réconcilier ; le frontend conserve un fallback vers le GET autonome pendant un déploiement roulant. Après Notifications, la voie à verrou Player reste ordonnée `Expedition` de fallback → Teams → Event, tandis que Défi quotidien, Combat quotidien, Boss mensuel et Concours s’exécutent en parallèle. Dans Notifications, Expedition précède toujours les réconciliations ; Codes cadeaux peut ensuite progresser en parallèle d’une voie Event où messages puis cycle de vie restent séquentiels. Le timer Expedition READY recharge seulement Notifications. Aucun timeout n’est augmenté et les endpoints individuels restent autonomes.
- **Mesures locales comparables du candidat bootstrap.** Sur le même processus local et la même base DEV, quatre lectures Notifications avant modification donnaient 869–934 ms et après modification 557–723 ms ; ce profil exclut HTTP/auth/réseau et ne remplace pas la mesure publique. Un harnais déterministe utilisant les mêmes délais par reader donne 1 688–1 718 ms avec l’ancien ordonnanceur contre 964–975 ms avec le nouveau. La validation publique connexion/Ctrl+F5 et la surveillance Railway restent requises.
- **Lot 3 Missions candidat.** Expedition réconcilie une fois `EXPEDITIONS_COMPLETED` et, selon la récompense, Moras ou particules après le claim ; Combat réconcilie victoire, victoire manuelle seulement en MANUAL et Moras après la tentative gagnante ; Social agrège les cœurs sortants en une passe sender et réconcilie Amitié parfaite pour les deux participants lors du passage réel à 1000, sans catch-up du destinataire passif. Les canaux UI/INTERNAL_CHAT et les opérations déclenchantes réelles sont conservés. Catch-up avant mutation, crédits Economy sans récursion, replay et rollback restent dans la transaction propriétaire.
- **Micro-polish Boutique candidat.** Le libellé `Achat unitaire` du Ticket est supprimé ; le prix et le bouton réutilisent la géométrie normale des contrôles des deux cartes, sans prix, probabilité ni règle d’achat modifiés. La validation visuelle publique reste à faire.
- **Contrôles du candidat Lot 3.** Frontend 796/796 sur 87 fichiers et backend 396/396 sur 56 fichiers réussissent. PostgreSQL réussit fichier par fichier : Missions 11/11, Expedition 11/11, Combat 9/9, Amitié 16/16 et Social 8/8. Typecheck backend, builds frontend/backend, lint, diff-check, Prisma validate/status et contrôle des fixtures réussissent ; 42 migrations restent suivies et appliquées. Le lint conserve uniquement les deux avertissements préexistants de `ContestScreen`. Aucun moteur de navigateur mesurable n’étant installé localement, l’alignement Boutique est couvert par DOM/CSS et reste à vérifier visuellement après promotion.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 3 CANDIDAT SUR `review`**. Lots 1/2 et pagination/loaders restent validés publiquement. Expedition, Combat et Social/cœurs sont raccordés dans le candidat, sans migration 043. Le nouveau bootstrap et le micro-polish Ticket ne sont ni promus ni validés publiquement.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai SHA `review`**. Après approbation seulement, une mission distincte pourra promouvoir par fast-forward puis vérifier déploiements, connexion/Ctrl+F5, Notifications, Railway/P2028 et alignement visuel Boutique. Restent aux lots ultérieurs : route/UI Missions, `!mission`, Profil, notifications player-facing, Twitch et reprise legacy R328. `PAID_INFRA_APPROVED = false`.
