...
Version : fondations serveur/DB MP corrigées R505/R511/R516 promues sur main ; micro-polish Chat R890–R893 sur main, à valider publiquement
Date : 2026-09-23
Statut : FONDATIONS MP BACKEND/DB PHYSIQUES SUR MAIN, SANS UI ; CHAT GLOBAL RESTE LE SEUL CHAT PLAYER-FACING
...
- **Fondations MP et micro-polish Chat promus sur main.** Les migrations 036–037 matérialisent et durcissent conversation canonique à deux, participants, demandes et messages MP privés ; `DirectMessageService` et ses routes HTTP portent permissions PUBLIC/FRIENDS/PRIVATE, blocage Social, idempotence, anti-rafale, non-lus, lecture/accusés séparés, archivage et fenêtre normale de 500. R505 est appliquée : désactiver les accusés fige leur curseur partagé sans effacer les lectures déjà communiquées ; leur réactivation seule ne le fait pas avancer. R511/R516 utilisent le refus neutre unique `Ce message ne peut pas être envoyé.` pour les causes sociales privées. Aucun onglet, badge, composer ou historique UI MP n'est ajouté. Le Chat global borne toute projection joueur aux 200 derniers messages de la génération, recharge le récent au dépliage et applique l'ordre d'actions R891 avec confirmation Signaler adjacente R892. Les déploiements de ce checkpoint ne sont pas encore vérifiés ; le micro-polish Chat R890–R893 reste à valider publiquement par Axel.
- **Chat global — VALIDÉ PUBLIQUEMENT ET CLÔTURÉ.** Le dernier HEAD fonctionnel validé publiquement `68b8adc3bece298fe3f1e4be10a358860c563eba`, dont le dernier micro-polish fonctionnel est `b7d2774d45fe8400347c02dd25176624596a2c35`, a été validé explicitement par Axel : chevron d'envoi repositionné, texte/placeholder du composer repositionnés et séparateur sous Joueurs connectés sont acquis, comme le reste du cycle player-facing. ChatGPT a vérifié `main == review`, divergence `0 / 0`, Cloudflare Pages `SUCCESS`, Railway `SUCCESS`, `Application ready` et `/health` HTTP 200. Les erreurs HTTP transitoires observées pendant certains redémarrages Railway se sont stabilisées et ne constituent pas un défaut produit ouvert. Aucun correctif Chat global player-facing ne reste ouvert. Le checkpoint documentaire de passation est promu sur `main` ; il ne constitue pas un nouveau lot fonctionnel à valider. Twitch, Realtime/WebSocket/SSE demeurent absents.
...
- Base DEV : migrations additives `20260922225444_036_add_direct_message_foundations` puis `20260922232731_037_harden_direct_message_history` appliquées via Prisma ; **37 migrations Prisma** sont suivies, checksums respectifs `b558b434cf399dfed0e2983a7bded7d4b69d4af788b65fd26e2151d6262690fa` et `331237a50d8b904fded1b6a6d1a51418f5a3ba5f04d4b38f1b2fac3493553c4f`. La 037 rend restrictives les FK enfant → conversation sans toucher aux données. Les quatre tables MP sont vides après migration, RLS active, sans grant `PUBLIC`/`anon`/`authenticated`. Les migrations 032–035 du Chat global restent intactes. Aucun seed économique, import legacy ou reset Privacy.
- Validation technique du lot promu : **708 tests frontend** acquis sur `d50b507`, **389 tests backend** et **29 scénarios PostgreSQL Chat + MP** (22 + 7) réussis ; génération Prisma, typecheck et builds frontend/backend réussis. Le lint réussit avec les deux avertissements `ContestScreen` préexistants ; Vite conserve son avertissement de chunk. Après la review, le test PostgreSQL MP couvre explicitement le gel et la reprise du curseur partagé ainsi que le wording neutre dans les deux sens de blocage. Le `GameShell` complet avec les CSS de production a été inspecté à **1920×1080, 1366×768 et 390×844** : messages propre/autrui, contenu court et multiligne, actions, confirmation, menu tactile, absence de clipping/overflow/recouvrement compositeur et retour réel au bas après repli/réouverture sont vérifiés. Aucun changement frontend ou CSS n'a été nécessaire. Ces preuves techniques ne constituent ni une vérification de déploiement ni une validation publique.
...
- Domaine actif : **Messages privés (MP)**. Le socle backend/DB est désormais physique ; l'interface MP reste entièrement non physique. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R893, sans modification de l'audit Social ni transfert depuis Event Game C.
- Prochaine étape exacte : ChatGPT vérifie `main == review`, Cloudflare Pages, Railway, `Application ready` et le healthcheck, puis fournit immédiatement la checklist « À tester en public » du micro-polish Chat. Axel teste publiquement ce micro-polish ; seulement après réussite, sa validation publique est documentée et le prochain vertical UI MP peut être ouvert par mission distincte. Le lot MP reste invisible et se valide techniquement par ses routes/tests/contraintes. Cette future mission UI MP ne doit pas ajouter implicitement édition, suppression/restauration, historique, recherche, date, signalement, `!mp`, Twitch ou Realtime. `PAID_INFRA_APPROVED = false`.
