...
Domaine actif :
**Team / Équipe / Passifs — le raccord des sept passifs au moteur Pull est déployé et ses comportements métier critiques ont été validés publiquement par le propriétaire. Le panneau Team à quatre passifs, l’Historique sans feedback passif et sa pagination, le feedback C6 individuel et la mise en évidence fonctionnelle des niveaux actifs dans Détail sont également validés publiquement. Le candidat correctif 0.71 restant sur `review` cible uniquement le centrage contextuel des feedbacks, le layout sans asset parasite ni scroll desktop de Détail > Passifs et le vocabulaire serveur player-facing `Primos` ; ces derniers points restent à reviewer puis à revalider publiquement.**
...
La Team autoritative et ses sept passifs modifient désormais le moteur Pull dans une seule passe cohérente, transactionnelle, snapshotée et auditable. Les comportements métier critiques ainsi que le panneau Team, l’Historique, sa pagination, le feedback C6 individuel et la lecture des niveaux actifs sont validés publiquement ; seuls le centrage des feedbacks et le layout de Détail > Passifs du candidat correctif 0.71 restent à reviewer puis à revalider publiquement.
...
**Faire reviewer le candidat correctif final 0.71 uniquement sur `review` → corrections éventuelles → promotion fast-forward vers `main` seulement après approbation → revalidation publique propriétaire du centrage contextuel des feedbacks passifs, du layout sans asset parasite ni scrollbar desktop de Détail > Passifs et du vocabulaire `Primos` dans les descriptions servies par le backend → clôturer Team / Passifs puis reprendre l’ordre V1 dans `docs/roadmap/implementation-order-v1.md`.** Le panneau Team à quatre passifs, les niveaux actifs, l’Historique sans passifs, sa pagination, le gain C6 individuel et les comportements métier sont déjà validés publiquement. `PAID_INFRA_APPROVED = false` reste inchangé.
