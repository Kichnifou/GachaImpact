...
Version : Chat global promu ; mini-lot final de polish UX promu, vérifications publiques à faire ; MP encore non physique
Date : 2026-09-22
Statut : CHAT GLOBAL PRESQUE VALIDÉ ; MINI-LOT POLISH UX PROMU, VÉRIFICATIONS PUBLIQUES ET VALIDATION FINALE EN ATTENTE
...
- Le premier lot Chat global player-facing, le lot réactivité/UX et le mini-lot final de polish sont **implémentés, reviewés indépendamment et promus sur `main`**. Le second retest public a confirmé que la majorité des correctifs précédents fonctionne ; `!help`, `!banque` et `!pull` ont été observées fonctionnelles. Le dernier checkpoint public vérifié avant cette promotion reste `3716b0b911e945abfb9bcecf33b9d2250a3eae1e` : Cloudflare Pages `SUCCESS` (check-run `106820942244`) et Railway `SUCCESS` (déploiement `01858098-7fa0-4d14-b431-be4eb68a6d33`). Pour le nouveau HEAD, Cloudflare, Railway et le healthcheck restent à vérifier ; le dernier test public des nouveaux polish reste à faire par Axel. La migration additive 035 est appliquée sur DEV : **35 migrations Prisma** sont suivies. Le Chat global est **presque validé**, sans clôture publique définitive ; les MP restent non physiques. Twitch, Realtime/WebSocket/SSE demeurent absents ; `PAID_INFRA_APPROVED = false`. La conversation Codex actuelle continue en CONTINUITÉ.
