...
- Prochaine étape exacte : **review indépendante ChatGPT du nouveau SHA candidat sur `review`**, incluant le correctif queue/reply et le vertical MP mobile. Ne pas promouvoir `main` dans cette mission. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
