...
Version : Batch C — correctifs post-test public Social / Profil, candidat review
Date : 2026-09-20
Statut : BATCH C PROMU ET TESTÉ PUBLIQUEMENT ; CORRECTIFS SOCIAL / PROFIL CANDIDATS SUR REVIEW, NON PROMUS
...
- Référence publique courante : Batch C promu jusqu'à `ebb4c2e7f8f76053a7f2a8cb8f7f57e2af055660`, point de départ commun de `main` et `review` pour le retour public. Son cœur Amitié est validé en conditions publiques ; le lot de correction Social / Profil issu de ce test est candidat sur `review` et n'est pas promu. Batch B — Social Foundations et Batch A restent également promus. Noël naturel reste à observer en décembre.
...
- Base DEV : **30 migrations Prisma** à jour ; dernière additive `20260920220000_030_harden_friend_heart_trigger_search_path` appliquée via Prisma. Migrations 001–029 intactes. Demandes, cœurs et compteur individuel cumulé sont physiques ; aucune donnée legacy importée, aucun reset Privacy.
- Le déploiement et le test public de Batch C ont confirmé demandes, acceptation, amitié, cœurs, Quotidiennes et retrait/réajout. Le retour a identifié des défauts ciblés d'alignement, de filtres, de notification, de rafraîchissement et d'actions contextuelles ; leur correction est le candidat `f3f7624669f14582939cb42dad7f90e4c17f6dbf` sur `review`. Aucune action Railway/Cloudflare dans ce lot ; `PAID_INFRA_APPROVED = false`.
- Correctifs post-test public : Social conserve **Amis / Demandes / Joueurs** avec colonnes stables, filtre relationnel serveur, demandes reçues prioritaires et surlignées, feedback temporaire cloisonné et bouton global explicite. Profil et Joueurs connectés consomment le même contrôleur Amitié et exposent les actions contextuelles. Une demande crée une notification existante typée vers Demandes, résolue avec la demande ; aucun cœur ne notifie.
...
- **Prochaine étape immédiate : review indépendante du candidat de correctifs post-test public sur `review`.** Après approbation seulement : mission dédiée de promotion vers `main`, déploiement puis nouveau contrôle public Social / Profil. Les échanges de particules deviennent le domaine actif après clôture publique correcte de ces correctifs ; ensuite Chat global + MP, puis la suite de [implementation-order-v1.md](../roadmap/implementation-order-v1.md). R633/R634 restent différées vers Historique global.
- Lire [le workflow](../process/implementation-workflow.md), [l'architecture Amitié](../architecture/backend-architecture-v1.md#amitié-et-activité-player--batch-c), [la navigation](../specifications/navigation-shell-v1.md) et le checkpoint candidat ci-dessous. Les checkpoints Batch A/B demeurent des preuves historiques.

---

# 0. RÈGLE D'OR DU PROJET

## Checkpoint candidat — Correctifs post-test public Batch C, 2026-09-20

- Commit fonctionnel `f3f7624669f14582939cb42dad7f90e4c17f6dbf`, parent `ebb4c2e7f8f76053a7f2a8cb8f7f57e2af055660`. Le lot part du checkpoint promu et testé publiquement où `origin/main == origin/review`, sans rebase, migration, action DB publique, déploiement ni push `main`.
...
- Le contrôleur Amitié du GameShell est partagé par Social, Profil, Joueurs connectés et Quotidiennes. Chaque mutation attend sa réponse puis recharge immédiatement le snapshot commun ; l'annuaire réagit à cette révision sans attendre son polling. Les succès sont associés à leur surface, effacés après quatre secondes et lors d'un changement d'onglet, d'action, de tri, de filtre ou de sortie. Le message individuel conserve la phrase Ami et seulement les gains `+5 Primos` ; l'envoi global conserve son décompte compact.
- Profil place la présence entre le nom et le niveau, affiche l'asset Élément et propose sur un autre profil ajouter, demande envoyée, accepter/refuser, cœur et retrait selon l'état. Aucune action vers soi n'est affichée. Joueurs connectés conserve l'ouverture du profil et `Voir tous les joueurs →`, avec une colonne d'action compacte utilisant le même état relationnel.
...
## Checkpoint candidat — Batch C Amitié / polish public, 2026-09-20
