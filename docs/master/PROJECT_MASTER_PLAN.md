...
Version : candidat Statistiques générales sur review ; Missions et Messages privés validés publiquement ; workflow Codex actif
Date : 2026-09-25
Statut : STATISTIQUES GÉNÉRALES CANDIDAT REVIEW, NON PROMU ; LOTS MISSIONS 1 À 5 PROMUS ET VALIDÉS PUBLIQUEMENT ; CYCLE MESSAGES PRIVÉS, R898, SÉCURISATION ASYNC/FOCUS/QUEUE ET RESPONSIVE MOBILE PROMUS ET VALIDÉS PUBLIQUEMENT ; AUCUN DÉFAUT MP CONNU RESTANT DANS CE CYCLE ; WORKFLOW CODEX PROMU ET ACTIF ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : Statistiques générales, candidat sur `review`, en attente de review indépendante.** `Profil > Statistiques` présente Progression, Gacha, Économie et Activités depuis une projection serveur en lecture seule des sept tables propriétaires. Les 23 métriques, dont le taux de 5★ dérivé sans perte de précision, suivent `GENERAL_STATISTICS` ; `!infos` reste compact. Aucune migration Prisma. Les Lots Missions 1 à 5 et le cycle Messages privés restent promus et validés publiquement. Top / classements globaux est le domaine suivant dans la roadmap, sans développement dans ce lot. Restent non implémentés les notifications de complétion Missions player-facing, Twitch et la migration legacy R328.
- **Contrôles du candidat Statistiques.** Tests ciblés frontend **8/8** et backend **110/110**, suites complètes non-DB frontend **843/843** et backend **410/410**, suite PostgreSQL Social/Profil **9/9**, builds frontend/backend et `verify:quick` **5/5** exécutés localement. Un harnais temporaire avec le vrai `ProfileScreen`, les CSS de production et la géométrie du shell a été vérifié sous Edge à **1920×1080, 1366×768 et 390×844** : quatre groupes présents, `Non disponible` visible dans le DOM, aucune valeur coupée ni débordement horizontal mesuré. Ce contrôle local ne remplace pas le test visuel/public d'Axel dans le `GameShell` complet. Ce candidat n'est ni promu sur `main`, ni déployé, ni validé publiquement.
...
- Domaine actif : **STATISTIQUES GÉNÉRALES — CANDIDAT REVIEW ; MISSIONS ET CYCLE MP VALIDÉS PUBLIQUEMENT**. La projection dédiée et l'onglet Profil en quatre groupes sont physiques sur `review`. Toutes les valeurs `bigint` traversent l'API en chaînes décimales ; le taux de 5★ est calculé en `bigint` et indisponible à zéro Pull. La confidentialité `GENERAL_STATISTICS` est contrôlée avant la lecture des compteurs. `!infos` conserve seulement Pulls et victoires Combat. La migration 043 reste la dernière ; DEV et dépôt comptent 43 migrations, sans 044.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai commit Statistiques sur GitHub, puis corrections éventuelles sur `review`**. Après approbation explicite, une mission distincte pourra promouvoir sur `main` ; déploiement et validation publique restent ultérieurs. Le domaine suivant sera Top / classements globaux. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
