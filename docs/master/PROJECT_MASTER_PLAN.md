...
- **Event et Échanges validés publiquement ; régression visuelle Modération signalée.** Les sélecteurs Event et Échanges conservent avatar et pseudo groupés à gauche ; le test public les valide. Dans la recherche rapide Modération, l'avatar et le texte étaient deux enfants flex distincts et `justify-content: space-between` écartait le pseudo au milieu. Le candidat sur `review` regroupe l'identité avec `PlayerIdentityInline` et laisse le badge TESTEUR à droite. Il corrige aussi le cadrage des avatars Sidebar et Chat en supprimant le padding natif des boutons et les lueurs des avatars, sans toucher aux dimensions extérieures ni aux assets. Ces corrections attendent la review indépendante puis la revalidation publique d'Axel.
...
- Domaine actif : **correctifs visuels post-test public sur `review`**. La notification Apparence et sa navigation, les avatars possédés, le fonctionnement de l'avatar élémentaire, Event et Échanges sont validés publiquement. L'alignement Modération, le cadrage Sidebar et Chat et la suppression de leurs lueurs sont implémentés en candidat ; review indépendante et revalidation publique restent nécessaires. La revalidation visuelle publique R912 et Menu reste en attente. R911 reste à observer lors d'une future rotation naturelle. DEV compte **48 migrations Prisma**, dont 045 à 048 pour Apparence.
- Prochaine étape exacte : **faire examiner le candidat visuel Modération/Sidebar/Chat en review indépendante, puis le promouvoir seulement sur mission dédiée et demander à Axel de revalider ces surfaces après déploiement**. TwitchIdentity reste l'étape suivante de roadmap et n'est pas commencée. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
