...
Version : R907 validé publiquement ; correctifs Classements et Historique global en candidats review
Date : 2026-09-25
Statut : R907 VALIDÉ PUBLIQUEMENT ; CLASSEMENTS MÉTIER TESTÉS PUBLIQUEMENT, CORRECTIFS POST-TEST APPROUVÉS CÔTÉ CODE MAIS NON REVALIDÉS PUBLIQUEMENT ; R911 ET HISTORIQUE GLOBAL EN CANDIDATS REVIEW
...
- **Candidat `review` Historique global / R911.** Le commit A `e98374e` des correctifs Classements est approuvé indépendamment côté code, sans revalidation publique. Le lot B ajoute l'écran unique `#history` avec Invocations, Bannières, Banque filtrable, Boutique et Event. `GET /api/v1/me/history` ne lit que Bannières et Event ; les trois autres onglets réutilisent leurs routes métier. La migration additive `20260925160000_044_add_banner_generation_vote_snapshot` est présente et appliquée sur DEV : **44 migrations**. Le JSONB nullable fige les candidats 5★ éligibles et leurs votes, y compris zéro, dans la transaction de création de chaque future rotation ; les anciennes restent à `null` sans reconstruction. Les éditions Event terminées sont lues depuis leurs tables métier et exposent le Top final public et le détail personnel du propriétaire. Aucune promotion, aucun déploiement ni validation publique de ce lot ne sont revendiqués. Prochaine étape : review indépendante du vrai commit GitHub.
...
- **Contrôles du candidat Historique / R911.** Backend complet **428/428**, frontend complet **853/853** avec un worker. PostgreSQL isolé History/Banner votes **6/6**, Banque **9/9** et Boutique **9/9** après correction de l'ordre de nettoyage de son fixture Missions. Builds frontend/backend, lint racine (seuls les avertissements déjà connus de ContestScreen et RankingsScreen), lint serveur et `verify:quick` **5/5** réussis. Le harnais visuel temporaire avec le vrai `HistoryScreen` et les CSS de production montre les cinq onglets, les snapshots présent/indisponible et le Top Event ; mesures sans débordement horizontal à **390×844, 1366×768, 1920×1080 et 2560×1440**. Ce contrôle n'est pas une validation dans une session authentifiée du vrai `GameShell` ; aucune validation publique n'est revendiquée.
...
- Domaine actif : **Historique global et R911 en candidat `review`**. R907 est validé publiquement ; les correctifs Classements du commit A sont approuvés côté code et attendent une nouvelle validation publique. DEV compte **44 migrations Prisma**, dont la 044 spécialisée Gacha ; `main` conserve son état public actuel.
- Prochaine étape exacte : **review indépendante du vrai commit GitHub du lot Historique**, avant toute promotion, déploiement ou validation publique. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
