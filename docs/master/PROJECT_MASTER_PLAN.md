...
Version : Chat global promu ; dernier lot UX Composer promu, vérifications publiques à faire ; MP encore non physique
Date : 2026-09-22
Statut : CHAT GLOBAL PRESQUE VALIDÉ ; LOT UX COMPOSER PROMU, VÉRIFICATIONS PUBLIQUES ET VALIDATION FINALE EN ATTENTE
...
- Le premier lot Chat global player-facing, le lot réactivité/UX, le mini-lot final de polish, son micro-polish visuel et le dernier lot UX Composer sont **implémentés, testés, reviewés indépendamment et promus sur `main`**. Le second retest public a confirmé que la majorité des correctifs précédents fonctionne ; `!help`, `!banque` et `!pull` ont été observées fonctionnelles. Le dernier checkpoint public vérifié avant cette promotion reste `efc599aafb4f6af2d454189f9fdf68f8d8ab1dc0` : Cloudflare Pages `SUCCESS` et Railway `SUCCESS`. Pour le nouveau HEAD, Cloudflare, Railway et le healthcheck restent à vérifier ; la validation publique finale du lot UX Composer reste à faire par Axel. La migration additive 035 est appliquée sur DEV : **35 migrations Prisma** sont suivies. Le Chat global est **presque validé**, sans clôture publique définitive ; les MP restent non physiques. Twitch, Realtime/WebSocket/SSE demeurent absents ; `PAID_INFRA_APPROVED = false`. La conversation Codex actuelle continue en CONTINUITÉ.
