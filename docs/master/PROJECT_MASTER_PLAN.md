...
Version : candidat Missions permanentes — Lot 2 producteurs et rattrapage standalone
Date : 2026-09-24
Statut : LOT 1 PROMU ET VALIDÉ PUBLIQUEMENT ; LOT 2 CANDIDAT SUR `review`, NON PROMU ET NON VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : Missions permanentes B/A/S/Z — Lot 2 candidat sur `review`.** Chat/XP, Pulls Gacha, possessions 4★/5★, C6 Pull/Stella, niveau Player, Moras gagnées, particules principales générées et intérêts Banque sont raccordés dans leurs transactions propriétaires. Expedition, Combat et Social/cœurs restent explicitement au Lot 3 ; aucun écran/API final, `!mission`, Profil, notification player-facing, Twitch ou import legacy n’est ajouté.
...
- **Rattrapage R301 prêt, jamais lancé en bulk pendant le développement.** À la première activation normale d’un standalone préexistant, une opération SYSTEM dédiée exécute le vrai moteur avant la mutation courante, lie les récompenses historiques à cette opération, puis écrit le marqueur dans la même transaction. Verrou Player, marqueur durable et opérations stables assurent l’exactly-once ; rollback annule ensemble marqueur et récompenses. Les nouveaux Players sont marqués dès leur provisionnement. Aucun UUID réel n’a été réconcilié manuellement ni crédité par un script.
- **Contrôles du candidat Lot 2.** Backend 394/394, PostgreSQL Missions 11/11, Chat 27/27, Gacha 23/23, Stella 6/6 et Banque 8/8 ; typecheck, build serveur, lint et statut Prisma réussissent. Le lint conserve uniquement les deux avertissements préexistants de `ContestScreen`. Après nettoyage exact des fixtures, DEV contient 10 Players, 10 états et 310 progressions, avec 0 complétion, 0 progression non nulle, 0 opération/récompense/catch-up Mission, 0 mouvement Mission, 0 marqueur R301 et 0 fixture du lot : aucun vrai Player DEV n’a été crédité ou rattrapé.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 2 CANDIDAT SUR `review`**. Le Lot 1 est sur `main` et validé publiquement ; le code et la migration 042 du Lot 2 restent uniquement candidats tant qu’ils ne sont pas reviewés, promus, déployés puis testés publiquement.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai commit Lot 2**. Ne pas promouvoir implicitement `main`. Restent au Lot 3 : Expedition, Combat, Social/cœurs et leur restitution spécifique. Restent aux lots ultérieurs : route/UI Missions, `!mission`, Profil, notifications player-facing, Twitch et reprise legacy R328. `PAID_INFRA_APPROVED = false`.
