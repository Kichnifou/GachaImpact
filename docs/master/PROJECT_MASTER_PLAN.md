...
Date : 2026-09-26
Statut : R907, CLASSEMENTS ET HISTORIQUE GLOBAL VALIDÉS PUBLIQUEMENT SUR LE PLAN FONCTIONNEL ; PARCOURS APPARENCE, EVENT ET ÉCHANGES VALIDÉS PUBLIQUEMENT ; CORRECTIFS VISUELS MODÉRATION, SIDEBAR ET CHAT APPROUVÉS EN REVIEW INDÉPENDANTE ET PROMUS SUR MAIN, REVALIDATION PUBLIQUE EN ATTENTE ; PROCHAINE ROTATION R911 À OBSERVER
...
- **Event et Échanges validés publiquement ; correctifs visuels Modération, Sidebar et Chat approuvés en review indépendante et promus sur `main`.** Les sélecteurs Event et Échanges conservent avatar et pseudo groupés à gauche ; le test public les valide. Dans la recherche rapide Modération, l'avatar et le texte étaient deux enfants flex distincts et `justify-content: space-between` écartait le pseudo au milieu. Le correctif regroupe l'identité avec `PlayerIdentityInline` et laisse le badge TESTEUR à droite. Il corrige aussi le cadrage des avatars Sidebar et Chat en supprimant le padding natif des boutons et les lueurs des avatars, sans toucher aux dimensions extérieures ni aux assets. Ces corrections attendent la revalidation publique d'Axel après déploiement ; aucun succès de déploiement n'est revendiqué ici.
...
- Domaine actif : **correctifs visuels post-test public approuvés en review indépendante et promus sur `main`**. La notification Apparence et sa navigation, les avatars possédés, le fonctionnement de l'avatar élémentaire, Event et Échanges sont validés publiquement. L'alignement Modération, le cadrage Sidebar et Chat et la suppression de leurs lueurs attendent encore le test public d'Axel. La revalidation visuelle publique R912 et Menu reste en attente. R911 reste à observer lors d'une future rotation naturelle. DEV compte **48 migrations Prisma**, dont 045 à 048 pour Apparence.
- Prochaine étape exacte : **vérifier les déploiements automatiques Railway/Cloudflare et le healthcheck, puis faire revalider publiquement par Axel Modération, Sidebar Character/élément et Chat Character/élément ainsi que l'absence de lueur sur ces avatars**. TwitchIdentity reste l'étape suivante de roadmap et n'est pas commencée. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
