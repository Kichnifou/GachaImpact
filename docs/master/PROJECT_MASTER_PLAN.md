...
Version : Chat global validé publiquement et clôturé ; reprise MP à préparer par bootstrap ; MP non physiques
Date : 2026-09-22
Statut : CHAT GLOBAL VALIDÉ PUBLIQUEMENT ET CLÔTURÉ ; MESSAGES PRIVÉS (MP) = PROCHAIN DOMAINE, NON PHYSIQUES
...
- Base DEV : migration additive `20260922120000_035_add_global_chat_generation` appliquée ; **35 migrations Prisma** sont suivies. Les migrations 032–035 du Chat global restent intactes et appliquées via Prisma. Aucun seed économique, import legacy ou reset Privacy.
...
- Domaine actif suivant : **Messages privés (MP)**. Les MP restent **non physiques** : aucune table, migration, route, service ni UI fonctionnelle MP ne peut être supposé avant inspection future du code, de Prisma, des migrations et des composants. Le cadrage déjà validé appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par [R883 du contrat Chat global](../specifications/global-chat-v1.md), le [journal des décisions](../specifications/decisions-log.md), la [navigation](../specifications/navigation-shell-v1.md), le [contrat UI](../specifications/ui-layout-contract-v1.md), le [modèle V1](../specifications/v1-data-model.md), [l'architecture backend](../architecture/backend-architecture-v1.md), le [schéma PostgreSQL](../architecture/postgresql-schema-v1.md) et la [référence des commandes](../commands/command-reference.md). Ne pas réauditer le legacy Social/MP depuis zéro.
- Prochaine étape exacte : ouvrir une **nouvelle conversation ChatGPT** consacrée aux MP. Elle applique le bootstrap d'[AGENTS.md](../../AGENTS.md), du [Guide ChatGPT](../../.chatgpt/CHATGPT_GUIDE.md), de ce Master, du [workflow](../process/implementation-workflow.md), de la [passation](../process/conversation-handoff.md), de [l'ordre V1](../roadmap/implementation-order-v1.md) et des sources MP ci-dessus ; elle vérifie les vrais HEAD `main`/`review`, reformule la reprise et **ne modifie rien** avant confirmation d'Axel. Après cette confirmation, ouvrir une **nouvelle conversation Codex** en `MODE : BOOTSTRAP`, faire relire les propriétaires MP et confronter la cible à l'état physique réel avant de découper le premier lot. `PAID_INFRA_APPROVED = false`.
