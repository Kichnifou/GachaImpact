...
- Domaine actif : **R907 + TOP / CLASSEMENTS GLOBAUX — PROMUS SUR MAIN, VALIDATION PUBLIQUE EN ATTENTE**. Statistiques générales sont déjà validées publiquement par le propriétaire. R907 a été approuvée en review indépendante et normalise les seuls arguments Chat qui désignent un Player ; Classements R714–R727 ont été approuvés en review indépendante, avec écran, API et `!top` physiques. Ni R907 ni Classements ne sont encore validés publiquement. La dernière migration reste `20260925100000_043_add_direct_message_replies` ; DEV et dépôt comptent **43 migrations Prisma**.
- Prochaine étape exacte : **vérifier les déploiements Railway et Cloudflare Pages, puis le healthcheck ; réaliser ensuite le test public R907 + Classements et attendre la validation du propriétaire avant d'ouvrir Historique global**. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
