...
- Prochaine étape exacte : **publier ce checkpoint documentaire sur `review`, le faire reviewer puis le promouvoir par une mission dédiée ; ouvrir une nouvelle conversation ChatGPT ; y exécuter une reprise Bootstrap strictement read-only selon le handoff ; vérifier `main`, `review` et leur divergence puis reformuler l'état ; attendre le choix explicite du propriétaire avant tout nouveau prompt Codex ; une fois le domaine confirmé, ouvrir une NOUVELLE conversation Codex en MODE BOOTSTRAP.** Restent explicitement reportés et non bloquants : médias/pièces jointes/GIF, `!mp`, Twitch/transport, Realtime/WebSocket/SSE, raisons de signalement, sanctions/avertissements/mute/ban, future notification au joueur signalé, réponses privées de commandes, annonces globales automatiques/UI→Chat, Historique global R633/R634 et autres sujets `FUTUR`. `PAID_INFRA_APPROVED = false`.
