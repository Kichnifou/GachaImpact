...
Version : Classements et Historique validés publiquement sur le plan fonctionnel ; R912 approuvé en review indépendante ; correctif Apparence candidat review
Date : 2026-09-26
Statut : R907, CLASSEMENTS ET HISTORIQUE GLOBAL VALIDÉS PUBLIQUEMENT SUR LE PLAN FONCTIONNEL ; R912 APPROUVÉ EN REVIEW, VALIDATION PUBLIQUE EN ATTENTE ; APPARENCE CORRECTIF REVIEW CANDIDAT ; PROCHAINE ROTATION R911 À OBSERVER
...
- **Apparence / avatars / titres : correctif candidat `review`.** Les migrations 045, `20260926100000_046_clear_disabled_equipped_avatars` et `20260926110000_047_harden_avatar_trigger_search_path` sont appliquées sur Supabase DEV : **47 migrations Prisma** ; 045 et 046 restent byte pour byte inchangées. 047 fixe le `search_path` de la fonction du trigger 046 sans modifier sa logique. Les deux tables Apparence conservent RLS et aucun droit navigateur direct ; catalogue réel et possessions restent vides. 046 retire la référence d'un AVATAR équipé à sa désactivation sans supprimer la possession et sans restauration automatique à sa réactivation ; TITLE n'est pas concerné. `effectiveAvatar` vérifie aussi l'activité avant d'exposer l'asset. `unlockCosmetic` exige `PLAYER_FACING` ou `SILENT_BACKFILL`, sans mode implicite ni producteur gameplay inventé. L'avatar élémentaire vient de `Player.elementKey`, sans faux unlock. `GET/PATCH /api/v1/me/appearance`, Profil propriétaire > Personnalisation et avatars des surfaces d'identité/sélection standalone sont candidats ; titre uniquement Profil. Aucune validation publique d'Apparence n'est acquise.

- **Validation publique et R912.** Le propriétaire confirme les parcours fonctionnels Classements (34 métriques, confidentialité, pagination, raccourci Profil, notification Trade) et Historique global (cinq catégories, filtres, deep-links, pagination et responsive). L'alignement des onglets sur `activity-inner-tabs` est approuvé par la review indépendante ; sa validation visuelle publique reste à faire. La rotation naturelle future doit encore confirmer qu'un nouveau snapshot complet R911 apparaît en production ; la logique R911 est techniquement validée.
- **Retouches UX candidates du 2026-09-26.** Le Menu n'expose plus la destination redondante Social : Amis ouvre toujours l'écran `social`, `#social` reste compatible et les anciennes préférences `social` sont nettoyées sans perdre les autres choix. Event présente l'onglet Général, un bandeau rétracté ouvrable depuis toute sa surface et au clavier, et un seul accès Historique discret dans sa carte Festival compactée. Aucune règle métier Event n'est modifiée.
...
- Domaine actif : **Apparence / avatars / titres, correctif candidat `review`**. R912 est publié sur `review` et approuvé par review indépendante ; Classements et Historique sont validés publiquement sur le plan fonctionnel, avec validation publique visuelle de R912 en attente. R911 reste à observer lors d'une future rotation naturelle. DEV compte **47 migrations Prisma**, dont 045 à 047 pour Apparence.
- Prochaine étape exacte : **attendre la review indépendante du hardening 047 puis le test public d'Axel pour Apparence**. TwitchIdentity reste l'étape suivante et n'est pas commencée. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
