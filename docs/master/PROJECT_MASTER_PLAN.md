...
Version : Chat global + MP ; fondations serveur Chat global approuvées, à promouvoir depuis `review`
Date : 2026-09-22
Statut : LOT 1 PHYSIQUE CHAT GLOBAL APPROUVÉ ; PROMOTION FAST-FORWARD VERS `main` À EFFECTUER
...
- Le checkpoint documentaire Chat global R872–R883, incluant la correction R460, part de `e662ac7b62f71abc2be6096705804de2cd08456c`. La chaîne Lot 1 `f334be4` puis `4d2a473` a été reviewée indépendamment et approuvée sur `review` : migrations 032 + 033 et `GlobalChatService` forment le socle serveur/DB. Après le push `main`, ChatGPT doit vérifier `main = review`, Cloudflare Pages, Railway, `Application ready` et `/health`. Le lot est invisible au joueur, donc aucune validation publique fonctionnelle n'est requise. Batch C / Social demeure validé ; Noël naturel reste à observer en décembre.
