...
Version : Chat global player-facing candidat sur review ; MP encore non physique
Date : 2026-09-22
Statut : PREMIER LOT PLAYER-FACING CHAT GLOBAL CANDIDAT SUR REVIEW ; REVIEW INDÉPENDANTE À FAIRE
...
- Candidat `review` du 2026-09-22, parent `ebe47e47238f0734d9478fd55f902cee71f821f2` : Chat global réel via sept routes authentifiées/no-store, `ChatPanel` connecté, polling HTTP visible (2,5 s ouvert / 5 s replié), non-lus serveur, historique progressif, réponses, mentions sélectionnées, Profil, suppression auteur, masquage local, signalement privé et URLs HTTP(S) sûres. L'envoi garde sa clé après échec réseau ambigu. Le serveur restitue `refreshScopes` selon les opérations métier confirmées ; le shell recharge les projections touchées sans rejouer une commande réussie. XP Chat, défi Messages dans la transaction et réponse publique GachaImpact de level-up sont branchés. La migration additive 034 porte mentions, reports et éligibilité du Défi Messages ; **34 migrations Prisma sont appliquées sur DEV**. Les tests locaux contrôlent routes, service DB et UI (383 serveur, 671 frontend, 16 PostgreSQL Chat) ; un rendu navigateur synthétique a été contrôlé aux trois tailles demandées, tandis que la validation visuelle et publique par Axel reste à faire. MP, Twitch et Realtime restent absents. Prochaine étape : review indépendante de ce candidat GitHub, puis mission dédiée de promotion/déploiement/test public ; corriger les retours avant le lot MP R501–R522.
...
- Le candidat player-facing et sa prochaine étape sont décrits dans la première ligne de cette reprise rapide ; ce checkpoint serveur seul constitue désormais son historique.
...
Le [contrat Chat global V1](../specifications/global-chat-v1.md) porte les décisions validées R872–R883. Le panneau desktop peut être replié et portera Chat/MP et leurs non-lus. Le Lot 1 promu a matérialisé les fondations serveur/DB du flux global ; le candidat sur `review` ajoute l'orchestration des commandes et leurs réponses publiques. La coque `ChatPanel` reste mockée, les MP ne sont pas encore physiques et Twitch ainsi que son pont optionnel restent reportés.
