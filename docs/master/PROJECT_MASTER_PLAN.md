...
Version : trois findings de review Chat corrigés sur le candidat player-facing ; migration 039 contrôlée sur DEV
Date : 2026-09-23
...
- **Candidat review MP player-facing corrigé jusqu'au commit `42e1122c1ffe397e526b33ce0302e90d830ef5f1`.** Le panneau droit reste le propriétaire unique `Chat | MP` et tout le vertical MP de `37da8c4` est conservé. Les trois findings de review après `e29fb6e` sont corrigés : un `CHAT_PACING_LIMIT` retire uniquement sa propre tentative du pacing local et recalcule les bornes sans effacer un envoi plus récent ; le pacing serveur ordonne les timestamps `createdAt` issus du `Clock`, indépendamment de `submissionOrder` ; `updates` livre en une réponse toutes les lignes inconnues de la fenêtre canonique de 200. R895 conserve l'ordre serveur persistant réservé avant les traitements, sans horloge client ni délai d'attente, et une arrivée A tardive se replace immédiatement avant B. R893 place toujours le snapshot Chat au bas avant le premier paint de réouverture. Le candidat n'est pas validé publiquement et n'est pas promu sur main.
...
- **Chat global — gate public inchangé par ce lot.** L'intégration préserve R890–R893 et l'ordre R891 `Signaler, Masquer, Copier, Mentionner, Répondre`, puis ajoute R894–R895 sans les déclarer validées publiquement. L'accès MP reste attaché à l'identité Player hors de cette rangée.
...
- Validation technique du candidat corrigé : **727 tests frontend / 85 fichiers**, **389 tests backend / 54 fichiers** et **34 scénarios PostgreSQL ciblés** réussis (**26 Chat + 8 MP**). Les ciblés UI Chat/MP passent à **63/63**. Les preuves nouvelles couvrent le rejet pacing serveur sans prolongation des 750 ms ni de la rafale, son retour après un envoi plus récent, la course R894+R895 où A réserve avant B mais termine après, la rafale selon les trois temps serveur et le rattrapage exact de 150 inconnus sans perte ni doublon ; l'arrivée tardive antérieure, les changements connus et la limite 200 restent couverts. Génération Prisma, `migrate status` DEV à 39 migrations, typecheck et builds frontend/backend réussis. Le lint ne conserve que les deux avertissements `ContestScreen` préexistants ; Vite conserve son avertissement de chunk. Le vrai `GameShell`, le vrai `ChatPanel` et le CSS de production, alimentés par des messages synthétiques sans persistance, ont été inspectés à **2560×1440, 1920×1080, 1366×768 et 390×844** lors du lot précédent : ordre et densité des messages, réponse citée, mention, onglets, composeur et pile mobile restent lisibles sans recouvrement ni overflow horizontal. Captures et harnais ont été retirés du lot. Ces preuves ne constituent ni déploiement ni validation publique.
...
- Domaine actif : **Messages privés (MP)**. Le socle backend/DB, le candidat UI, son correctif R893–R895 et les trois corrections de review Chat sont physiques sur `review` ; la validation publique MP reste absente. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R895, sans transfert depuis Event Game C.
