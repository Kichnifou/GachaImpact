...
Version : Batch B — Social Foundations, candidat de review
Date : 2026-09-20
Statut : BATCH A PROMU ET VALIDÉ PUBLIQUEMENT ; BATCH B SOCIAL FOUNDATIONS CANDIDAT — REVIEW INDÉPENDANTE REQUISE
...
- Référence publique de départ : `origin/main = 0da73d1d5391bdb67e1f23267994553e12cfb1af`. Batch A est promu. Le propriétaire confirme Configuration, suppression de la phrase de pagination, Event courant, signal Code Festival et vote réel avec état après vote. Le polish Catalogue observé publiquement est traité dans Batch B. Noël naturel reste à tester en décembre.
...
- Domaine actif : **Social Foundations**, étapes 6 et 7 après Boss : Profils/annuaire/présence + Confidentialité/consultations publiques. Menu Activités et polish Catalogue font partie du même candidat. R633/R634 restent reportées vers Historique global ; commandes vers Chat/Twitch. Aucun nouvel audit Social ni Rxxx.
- Base DEV : **28 migrations Prisma**, dernière additive `20260920130000_028_add_social_presence`, deux tables RLS sans grants navigateur. Aucun reset Privacy ni backfill des comptes. Migrations 001–027 intactes.
...
- Prochaine étape exacte : **review indépendante ChatGPT du vrai diff GitHub Batch B sur review**, puis seulement promotion explicitement approuvée, déploiements et validation publique. Après ces gates : Amitié, puis échanges de particules, puis Chat global + MP. Aucun de ces domaines suivants n'est développé ici.
- Lire [le workflow](../process/implementation-workflow.md), [l'audit Social clôturé](../legacy/14-ami-social-audit.md), [l'architecture](../architecture/backend-architecture-v1.md#social-foundations), [la navigation](../specifications/navigation-shell-v1.md) et [l'ordre V1](../roadmap/implementation-order-v1.md). Les checkpoints ci-dessous restent des preuves historiques, pas des pointeurs vivants.

---

# 0. RÈGLE D'OR DU PROJET

## Checkpoint candidat — Batch B Social Foundations, 2026-09-20
...
- Sept scénarios DB Social passent dans des schémas privés UUID, fixtures effectivement écrites puis schémas supprimés exactement. Aucun Player réel ou compte protégé utilisé. Contrôle final : aucun schéma `batch_test_*` résiduel, zéro ligne dans les deux nouvelles tables publiques. Les premières erreurs de fixtures/tests ont été corrigées puis relancées ; elles ne sont pas qualifiées de flaky.
...
- Batch B n'est ni promu ni validé publiquement. Aucun Ami/cœur/demande, MP, Chat réel, Twitch, avatar/titre déblocable, historique global, échange de particules, migration legacy, Story, image générée ou service payant.
...
Pour la prochaine étape exacte, consulter le bloc **Reprise rapide — état vivant** au début du Master. Batch A est promu et les contrôles publics communiqués par le propriétaire sont acquis ; le Calendrier de Noël naturel reste à observer en décembre. Batch B Social Foundations reste candidat. Après approbation de sa review seulement : promotion contrôlée, déploiements automatiques, vérifications Railway et Cloudflare, puis validation publique du propriétaire. L'historique Event transversal, Amitié, Twitch/chat et la migration legacy restent hors de ce candidat.
