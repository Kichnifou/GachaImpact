...
Version : premier vertical player-facing MP candidat sur review ; fondations serveur/DB et migration 038 contrôlées sur DEV
Date : 2026-09-23
...
- **Candidat review MP player-facing.** Le panneau droit est désormais le propriétaire unique `Chat | MP` : les deux vues restent montées pour conserver draft/scroll/intents du Chat, mais seule la vue réellement active peut avancer son curseur de lecture. L'onglet MP fournit badges croisés, liste normale triée serveur, archives, recherche Player, premier message différé jusqu'à l'envoi, demandes reçues/sortantes, conversation chronologique paginée, composer multiligne 1 000 caractères, rendu texte/URL sûr, lecture seule autoritative, accusés temporels, archivage, blocage/déblocage et deep-links depuis auteur Chat/Profil. Aucun écran principal, `!mp`, notification cloche, média, Realtime/WebSocket/SSE ni fonction MP avancée n'est ajouté. Ce candidat n'est pas validé publiquement et n'est pas promu sur main ; prochaine étape : review indépendante ChatGPT du candidat.
...
- **Chat global — gate public inchangé par ce lot.** L'intégration préserve R890–R893 et l'ordre R891 `Signaler, Masquer, Copier, Mentionner, Répondre`; l'accès MP est attaché à l'identité Player hors de cette rangée. Aucune confirmation publique propriétaire nouvelle n'a été fournie dans cette mission : le micro-polish Chat R890–R893 ne doit donc pas être déclaré nouvellement acquis par ce candidat et ce gate reste à signaler.
...
- Base DEV : migrations 036–038 appliquées via Prisma ; **38 migrations Prisma** suivies et `migrate status` à jour. Checksum 038 : `250cca88f898a404f9d656832274605ca6bf930efa6bbdadbecf828b9c68548f`. La colonne `direct_conversation_participants.last_shared_read_at` est un `timestamptz` nullable. Les quatre tables MP restent vides, RLS active, sans policy navigateur et sans grant `PUBLIC`/`anon`/`authenticated`. Aucun Player, seed économique, import legacy ou réglage Privacy n'a été modifié.
- Validation technique du candidat : **718 tests frontend / 85 fichiers**, **389 tests backend / 54 fichiers** et **7 scénarios PostgreSQL MP multi-compte** réussis ; génération Prisma, typecheck et builds frontend/backend réussis. Le lint ne conserve que les deux avertissements `ContestScreen` préexistants ; Vite conserve son avertissement de chunk. Le vrai `GameShell` avec CSS de production et données synthétiques sans persistance a été inspecté à **2560×1440, 1920×1080, 1366×768 et 390×844** : Chat, liste, conversation, messages courts/multilignes et URL, demandes reçue/sortante, lecture seule, archives, menu, composer long, badges et panneau replié. Le contrôle a détecté puis corrigé l'affichage simultané des deux panes via la règle explicite `[hidden]`; le viewport mobile réel mesuré reste à 390 px sans overflow horizontal. Ces preuves ne constituent ni déploiement ni validation publique.
...
- Domaine actif : **Messages privés (MP)**. Le socle backend/DB et le candidat UI sont physiques sur `review`; la validation publique MP reste absente. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R893, sans modification de l'audit Social ni transfert depuis Event Game C.
- Prochaine étape exacte : **review indépendante ChatGPT du candidat review MP**. Après approbation seulement viendront promotion dédiée sur `main`, vérifications GitHub/déploiements/healthcheck puis checklist et test public Axel. Les fonctions avancées édition, suppression/restauration, historique complet, recherche/date, signalement/admin MP, `!mp`, Twitch et Realtime restent reportées. `PAID_INFRA_APPROVED = false`.
