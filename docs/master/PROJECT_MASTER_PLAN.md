...
Version : candidat review de polish public Chat/MP après le vertical promu
Date : 2026-09-23
...
- **Candidat polish public Chat/MP sur `review`, base `a359e88`.** Le panneau droit reste le propriétaire unique `Chat | MP`. R893 révisée et R894 sont désormais **VALIDÉES PUBLIQUEMENT**. R895 est révisée : `submissionOrder` reste l'autorité backend des snapshots, fenêtres, pages, non-lus et curseurs, mais le frontend live ne déplace plus une ligne déjà affichée ; inconnues ajoutées à la livraison, lignes connues et optimistic remplacés en place. Chat poll à 350 ms start-to-start lorsqu'il est ouvert et visible, sans overlap, et garde le bas dans le même layout si le lecteur y était déjà.
...
- Validation technique de ce candidat : **747 tests frontend / 85 fichiers** et **389 tests backend / 54 fichiers** réussis. Les ciblages finaux passent à **83/83** pour `ChatPanel` + `DirectMessagePanel` et **12/12** pour le `GameShell`. Puisque `DirectMessageService` change, les **8/8 scénarios PostgreSQL MP** ont été relancés ; la suite PostgreSQL Chat n'a pas été relancée car `GlobalChatService` ne change pas. Builds frontend/backend et typecheck serveur réussissent. Le lint frontend ne conserve que les deux avertissements `ContestScreen` préexistants ; le serveur n'expose pas de script lint et Vite conserve son avertissement de chunk. Le vrai `GameShell`, les vrais panneaux Chat/MP et le CSS de production, alimentés par des réponses synthétiques sans persistance, ont été inspectés dans ce lot à **2560×1440, 1920×1080, 1366×768 et 390×844** : pseudo Chat sans MP inline et action MP dans l'overlay, liste et onglets MP, recherche avec avatar `A`, fil avec chevron/menu/composer, message long sans débordement, bulles sans heure, scrollbar masquée au bas et statuts exclusifs `Envoi...`, `Envoyé`, `Lu ✓` et relatif. Captures et harnais temporaires ont été retirés du lot. Ces preuves ne constituent ni déploiement ni validation publique.
