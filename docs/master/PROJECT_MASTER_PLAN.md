...
Version : micro-corrections finales et R515 Historique complet MP promus sur main
Date : 2026-09-24
Statut : LOT A ET R515 PROMUS SUR MAIN, VALIDATIONS PUBLIQUES EN ATTENTE
...
- **Lot A promu sur `main` dans `d8e36ff`, suivi de R515 dans `2a9aa12`.** R894 fixe désormais le verrou long à exactement trois secondes après M3, frontend et backend ; R897 sépare ce verrou visuel de la sécurité transport, réactive automatiquement le composer et restaure focus/caret sans voler un focus volontaire. R891 et R895 restent **VALIDÉES PUBLIQUEMENT** et ne changent pas. Les versions révisées R894/R897 restent **À REVALIDER PUBLIQUEMENT** après déploiement vérifié.
...
- **Chat global — état public réconcilié.** R890, R891 révisée, R892, R893 révisée et R895 révisée sont validées publiquement. R894 est rouverte uniquement pour sa durée trois secondes et R897 pour trois secondes + auto-focus ; les protections 750 ms, fenêtre de détection `< 4 000 ms`, garde 10/10 secondes, timestamp après verrou Player et R895 restent inchangés.
...
- **MP avancés — R504/R513/R514 validés hors nouveau micro-polish.** Actions hover/tactile, suppression, restauration, statuts, accusés Lu et scroll éditeur sont validés publiquement. Le candidat ajoute Entrée = sauvegarder, Ctrl/Meta+Entrée = nouvelle ligne, protection IME, annulation outside/Escape, et ferme la confirmation de suppression immédiatement avant le réseau. Ces seules interactions restent à revalider. La fenêtre/purge R504 reste strictement inchangée et R515 n'appartient pas à ce lot A.
- **R515 — implémentation présente sur `main`, non validée publiquement.** `Historique complet` est une vue interne séparée du fil live borné à 500 : cache éphémère isolé, scroll keyset `submissionOrder` ancien/récent/autour, recherche PostgreSQL case-insensitive excluant tous les tombstones, date locale vers ancre serveur, highlight et actions auteur partagées avec `canRestore`. Les GET participant-only ne marquent rien lu et restent accessibles en lecture seule/blocage.
- **R515 — index mesurés, aucune migration 040.** Des `EXPLAIN (ANALYZE, BUFFERS)` transactionnels sur 250 000 lignes synthétiques confirment l'index existant `(conversation_id, submission_order DESC)` pour le keyset. La recherche mesurée est plus lente avec le GIN trigram expérimental (~20,4 ms contre ~5,5 ms) et l'index date n'apporte qu'un gain limité (~2,3 vers ~1,8 ms). `pg_trgm` est disponible mais non installé : 39 migrations, schéma, RLS et grants restent inchangés.
...
- Validation technique des candidats A + R515 : frontend **772/772 sur 85 fichiers**, dont `ChatPanel` **63/63** et `DirectMessagePanel` **45/45** ; backend **390/390 sur 54 fichiers** ; routes MP **3/3** ; PostgreSQL MP **13/13** et PostgreSQL Chat **26/26**. Builds frontend/backend et typecheck serveur réussissent ; le lint conserve seulement deux avertissements préexistants dans `ContestScreen`, et le diff-check est propre. Le layout `GameShell` avec les CSS de production a été contrôlé à **2560×1440, 1920×1080, 1366×768 et 390×844**, sans débordement horizontal constaté ; cela ne constitue ni un déploiement ni une validation publique.
...
- Le lot Chat player-facing est validé publiquement pour R890, R891 révisée, R892, R893 révisée et R895 révisée. Seules la durée trois secondes de R894 et la version trois secondes + auto-focus de R897 attendent revalidation publique. R896 est validée publiquement ; R504/R513/R514 le sont fonctionnellement et ergonomiquement hors nouvelles interactions clavier/outside et fermeture immédiate de la confirmation.
...
- Domaine actif : **vérifier le déploiement déclenché par `main`, puis la validation publique du Lot A et de R515**.
- Prochaine étape exacte : **ChatGPT vérifie `main == review`, Railway, `Application ready`, `/health` et le frontend si possible ; Axel teste ensuite publiquement le verrou Chat trois secondes/focus/M4, les micro-interactions MP et R515** (Historique, scroll, recherche, date, tombstones, actions et absence de marquage lu). R894/R897 et les nouvelles interactions MP restent à revalider ; R515 est sur `main` mais reste à validation publique. R891, R895 et R896 ne sont pas rouvertes. Après cette validation, un checkpoint documentaire sera fait ; le prochain lot MP reporté est signalement + modération privée selon R509/R510/R517. Médias, `!mp`, Twitch et Realtime/WebSocket/SSE restent reportés. `PAID_INFRA_APPROVED = false`.
