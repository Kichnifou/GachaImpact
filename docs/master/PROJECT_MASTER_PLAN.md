...
Version : Correctifs Chat global du premier test public promus sur main ; déploiements et retest public à vérifier ; MP encore non physique
Date : 2026-09-22
Statut : PREMIER TEST PUBLIC CHAT GLOBAL EFFECTUÉ ; CORRECTIFS APPROUVÉS ET PROMUS SUR MAIN ; VALIDATION PUBLIQUE EN ATTENTE
...
- Le premier lot Chat global player-facing est **implémenté, reviewé, promu et déployé** sur `main` ; le premier test public d'Axel a confirmé le fonctionnement de base des parcours déjà testés et relevé les défauts désormais corrigés. La correction couvre l'envoi optimiste, la stabilité du composer, les overlays, la densité, la couleur Élément, les réponses, le masquage inverse, les mentions manuelles R885 et `!clear` modération R886. Les tests automatisés du candidat et sa review indépendante ChatGPT sont réussis et approuvés. La migration additive 035 est appliquée sur DEV : **35 migrations Prisma** sont suivies ; aucun MP général n'est physique. Le dernier déploiement indépendamment vérifié avant cette promotion est `ff61d47b702e84cfa5b62a5717bc1dc31e8350a6` sur `main` : Cloudflare Pages check-run `106729053789` completed/success ; Railway deployment `02663e3b-e7be-4696-9d60-6b8624ed0543` SUCCESS sur `main`, `Application ready`. Les déploiements Cloudflare et Railway du nouveau HEAD promu, ainsi que le healthcheck post-promotion, restent à vérifier par ChatGPT. Le Chat global n'est **pas encore validé publiquement** : Axel doit retester les corrections, puis, si elles sont concluantes, reprendre la recette à partir de « Commandes publiques ». MP, Twitch et Realtime demeurent non physiques ; `PAID_INFRA_APPROVED = false`. La conversation Codex actuelle continue en CONTINUITÉ.
...
Le [contrat Chat global V1](../specifications/global-chat-v1.md) porte les décisions validées R872–R886. Le panneau desktop repliable accueille le Chat global réel ; l’onglet MP prévu reste non physique. Les fondations serveur/DB, les commandes et leurs réponses publiques, puis le `ChatPanel` connecté ont été implémentés et reviewés indépendamment. La chaîne player-facing est promue sur `main` et déployée. Axel a effectué le premier test public : les parcours de base essayés fonctionnent, et ses retours UX et fonctionnels sont consolidés dans ce candidat correctif sur `review`. Le Chat global n’est pas encore validé publiquement ; les commandes avancées restent à retester après correction, puis la recette reprendra à « Commandes publiques ». Twitch et son pont optionnel restent reportés ; Realtime est absent.
