...
Version : Statistiques générales validées publiquement ; correctif Chat R907 candidat en review
Date : 2026-09-25
Statut : STATISTIQUES GÉNÉRALES VALIDÉES PUBLIQUEMENT PAR LE PROPRIÉTAIRE ; CORRECTIF CHAT R907 EN CANDIDAT REVIEW ; TOP / CLASSEMENTS GLOBAUX DOMAINE SUIVANT ; LOTS MISSIONS 1 À 5 ET CYCLE MESSAGES PRIVÉS PROMUS ET VALIDÉS PUBLIQUEMENT ; WORKFLOW CODEX PROMU ET ACTIF
...
- **Statistiques générales promues sur `main` et validées publiquement par le propriétaire.** `Profil > Statistiques` présente Progression, Gacha, Économie et Activités depuis une projection serveur en lecture seule des sept tables propriétaires. Les 23 métriques, dont le taux de 5★ dérivé sans perte de précision, suivent `GENERAL_STATISTICS` ; `!infos` reste compact. Le propriétaire confirme les quatre groupes, le taux Gacha, les valeurs Économie/Activités, la confidentialité `GENERAL_STATISTICS` et le rendu mobile. Pendant cette validation, `!infos @Pseudo` a révélé un défaut de résolution des références Player ; R907 le corrige dans un lot Chat dédié. Aucune migration Prisma supplémentaire. Les Lots Missions 1 à 5 et le cycle Messages privés restent promus et validés publiquement. Top / classements globaux est le domaine suivant. Restent non implémentés les notifications de complétion Missions player-facing, Twitch et la migration legacy R328.
...
- Domaine actif : **TOP / CLASSEMENTS GLOBAUX — À IMPLÉMENTER APRÈS LE CORRECTIF CHAT R907**. Statistiques générales sont validées publiquement par le propriétaire. R907 normalise les seuls arguments qui désignent un Player dans les commandes Chat : `Pseudo` et `@Pseudo` ciblent le même Player, sans changer les mots-clés ou autres identifiants. La dernière migration reste `20260925100000_043_add_direct_message_replies` ; DEV et dépôt comptent **43 migrations Prisma**.
- Prochaine étape exacte : **publier le correctif R907 sur `review`, puis implémenter et contrôler Top / classements globaux dans un lot séparé avant review indépendante**. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
