...
Version : Chat global promu ; trois derniers ajustements visuels promus, validation publique à faire ; MP encore non physique
Date : 2026-09-22
Statut : CHAT GLOBAL PRESQUE VALIDÉ ; TROIS DERNIERS AJUSTEMENTS VISUELS PROMUS, VÉRIFICATIONS PUBLIQUES ET VALIDATION FINALE EN ATTENTE
...
- Le premier lot Chat global player-facing, le lot réactivité/UX, le mini-lot final de polish, le dernier lot UX Composer et ses micro-correctifs sont **implémentés, testés et promus sur `main`**. Le HEAD public `b0287c5dda986175d01a349369846f929f9bd8e6` a été testé par Axel : le micro-polish précédent est globalement validé. Trois derniers ajustements mineurs ont été demandés et sont implémentés puis promus dans la mission courante : chevron d'envoi légèrement remonté sans déplacer sa zone cliquable, texte et placeholder du composer légèrement redescendus à hauteur globale constante, et séparateur ajouté sous Joueurs connectés. Cloudflare, Railway et le healthcheck du nouveau HEAD restent à vérifier, puis sa validation publique reste à faire par Axel. Le Chat global est **presque validé**, sans clôture publique définitive ; les MP restent non physiques. Twitch, Realtime/WebSocket/SSE demeurent absents ; `PAID_INFRA_APPROVED = false`. La conversation Codex actuelle continue en CONTINUITÉ.
