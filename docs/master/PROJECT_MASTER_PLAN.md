...
Version : Missions permanentes — Lot 5 candidat sur review ; R898 et sécurisation des fins d'envoi MP candidats au-dessus
Date : 2026-09-25
Statut : LOTS MISSIONS 1 À 4 PROMUS ET VALIDÉS PUBLIQUEMENT ; LOT 5 `!mission` / PROFIL / PRIVACY / POLISH ÉCRAN, CORRECTIF MP R898 ET SÉCURISATION DES FINS D'ENVOI CANDIDATS SUR `review` EN TROIS COMMITS DISTINCTS ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : Missions permanentes B/A/S/Z — Lot 5 candidat sur `review`.** Les Lots 1 à 4 sont promus et validés publiquement, et tous les producteurs autoritatifs du catalogue permanent sont raccordés. Le candidat Lot 5 active `!mission` dans le Chat interne, ajoute Missions au Profil avec consommation réelle de Privacy `MISSIONS`, et partage le polish premier rang incomplet / non-terminées d'abord entre les deux écrans. Restent non implémentés les notifications de complétion player-facing, Twitch et la migration legacy R328.
- **Correctif MP R898 candidat au-dessus du Lot 5, sans le modifier.** La migration additive 043 porte les réponses persistantes dans une même conversation ; liste, fil récent, historique, recherche et preuves de signalement projettent leur contexte selon le contrat dynamique/figé. Le composer restaure focus et caret après succès ou échec sans voler un focus volontaire, et Répondre depuis l'Historique revient au fil récent. Ce correctif reste à reviewer puis à tester publiquement ; il ne promeut ni Lot 5 ni `main`.
- **Sécurisation frontend des fins d'envoi MP, troisième commit candidat.** Une génération de session invalide les effets tardifs sur brouillon, réponse, navigation et focus après changement de conversation, de vue, d'onglet ou de Player, y compris lors d'un aller-retour. Un échec tardif reste récupérable explicitement dans sa conversation d'origine avec sa clé de retry ; une initiation tardive ne sélectionne plus son résultat après sortie du parcours. Aucun changement backend ni migration supplémentaire. Review indépendante et validation publique restent à faire.
...
- Le cycle Chat/MP antérieur est **CLÔTURÉ PUBLIQUEMENT** : R890–R897 et R504–R517 sont acquis. Le nouveau correctif R898 est un candidat séparé non encore promu ni validé publiquement ; il ne remet pas en cause ces acquis.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 5 CANDIDAT SUR `review`, SUIVI DES DEUX CORRECTIFS MP**. Les Lots 1 à 4, pagination/loaders et le correctif responsive Boutique `7106f39` sont validés publiquement ; le bootstrap demeure stable et publiquement acceptable avec sa latence résiduelle non bloquante. `!mission`, Profil Missions et Privacy `MISSIONS` restent dans leur commit Lot 5 intact. La migration 043 appartient seulement au premier correctif MP.
- Prochaine étape exacte : **review indépendante ChatGPT du nouveau SHA final cumulé sur `review` : Lot 5, R898, puis sécurisation des fins d'envoi MP**. Aucun passage sur `main` n'est autorisé dans ce lot. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
