...
Version : Échanges promus — corrections de validation publique, candidat review
Date : 2026-09-21
Statut : ÉCHANGES PROMUS ET PREMIER TEST PUBLIC EFFECTUÉ ; VERTICAL NON CLÔTURÉ, CORRECTIONS EN REVIEW
...
- Référence publique courante : `41522d5faf63b176647df3f101b8e14593805cf0`, promotion main vérifiée par fetch ; déploiements Railway/Cloudflare et première validation publique propriétaire rapportés dans la mission. Échanges reste ouvert : lenteur et faux échec après succès, accès contextuels et polish sont consolidés dans le candidat ci-dessous. Le microfix « Nouvel ami » est promu ; sa validation publique spécifique n'est pas explicitement confirmée. Batch C / Social demeure validé ; Noël naturel reste à observer en décembre.
...
- Domaine actif : **stabilité et polish de validation publique Échanges**, avant clôture. Les règles économiques R5–R53 sont conservées ; le candidat traite synchronisation, lectures redondantes, accès UI, présentation Event et procédures opératoires.
...
- Aucune action Railway/Cloudflare ni promotion dans ce lot ; `PAID_INFRA_APPROVED = false`. La validation technique locale des échanges ne vaut pas test public du propriétaire.
...
- **Prochaine étape immédiate : review indépendante du candidat de correction**, puis promotion dédiée et nouveau test public. **Chat global + messages privés** ne démarre qu'après clôture publique des Échanges et du polish courant, conformément à [implementation-order-v1.md](../roadmap/implementation-order-v1.md). R633/R634 restent différées vers Historique global.
...
## Candidat — stabilité et polish après premier test public, 2026-09-21

- Baseline vérifiée : main = review = `41522d5faf63b176647df3f101b8e14593805cf0`, 0/0, worktree propre. Ce lot n'est pas encore validé publiquement.
- Commits du candidat : serveur `7757d4b62af76702ad9e097c5ab6fb1f22d2e18d` (parent baseline), frontend `a4e3dbdab9c42d90771a0371818883a6f9d95e3c` (parent `7757d4b`). Le checkpoint documentaire est porté par le commit suivant, sans promotion main.
...
- Lectures : snapshot Trade indépendant des partenaires, recherche temporisée 250 ms et ignorée hors Partenaires ; treize requêtes SQL retirées du snapshot à sept éléments grâce à l'agrégation des réservations. Notifications : polling fermé 15 s, ouvert 3 s, focus/visibilité conservés et GET simultanés mutualisés. Pas de changement des transactions cœur ni des délais serveur.
...
- Documents évalués : navigation et architecture actualisées pour les accès et mécanismes de lecture ; contrat UI inchangé (réutilisation des primitives), decisions-log sans nouvelle décision produit. Aucun déploiement, migration, push main, force-push, rebase, squash ni modification Story. Review indépendante puis promotion/test public restent nécessaires ; P2028 et latence distante restent à observer après déploiement.
