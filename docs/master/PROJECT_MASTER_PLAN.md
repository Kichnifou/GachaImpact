...
Version : correctif pacing/ordre Chat + MP du candidat player-facing ; migration 039 contrôlée sur DEV
Date : 2026-09-23
...
- **Candidat review MP player-facing corrigé.** Le panneau droit reste le propriétaire unique `Chat | MP` et tout le vertical MP de `37da8c4` est conservé. R894 ajoute uniquement au Chat le pacing silencieux 750 ms et le verrou quatre secondes après trois envois en moins de quatre secondes, avec retry idempotent exclu. R895 donne au Chat et aux MP un `submissionOrder` serveur persistant réservé avant les traitements : aucune horloge client, aucun délai d'attente, et une arrivée A tardive se replace immédiatement avant B. R893 place désormais le snapshot Chat au bas avant le premier paint de réouverture. Le candidat n'est pas validé publiquement et n'est pas promu sur main.
...
- Validation technique du candidat corrigé : **724 tests frontend / 85 fichiers**, **389 tests backend / 54 fichiers** et **33 scénarios PostgreSQL ciblés** réussis (**25 Chat + 8 MP**, dont les deux courses A réservée avant B mais terminée après). Génération Prisma, typecheck et builds frontend/backend réussis. Le lint ne conserve que les deux avertissements `ContestScreen` préexistants ; Vite conserve son avertissement de chunk. Le vrai `GameShell`, le vrai `ChatPanel` et le CSS de production, alimentés par des messages synthétiques sans persistance, ont été inspectés à **2560×1440, 1920×1080, 1366×768 et 390×844** : ordre et densité des messages, réponse citée, mention, onglets, composeur et pile mobile restent lisibles sans recouvrement ni overflow horizontal. Captures et harnais ont été retirés du lot. Ces preuves ne constituent ni déploiement ni validation publique.
...
- Domaine actif : **Messages privés (MP)**. Le socle backend/DB, le candidat UI et son correctif R893–R895 sont physiques sur `review`; la validation publique MP reste absente. Le cadrage appartient à [l'audit Social R501–R522](../legacy/14-ami-social-audit.md), complété par R883 et R889–R895, sans transfert depuis Event Game C.
