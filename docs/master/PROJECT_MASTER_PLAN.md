...
Version : Chat global + MP ; fondations et orchestrateur de commandes Chat global promus
Date : 2026-09-22
Statut : LOT 1 CHAT GLOBAL ET ORCHESTRATEUR COMMANDES PROMUS ; DÉPLOIEMENTS À VÉRIFIER
...
- Après cette promotion, ChatGPT vérifie les déploiements normaux sur le SHA `main`, Railway, Cloudflare Pages et le healthcheck. Aucune validation publique fonctionnelle n'est requise pour ce lot serveur-only : le `ChatPanel` reste mocké, aucune route Chat navigateur n'est active, les MP ne sont pas physiques et Twitch reste reporté. La prochaine étape est le vrai Chat global player-facing : API authentifiées, lecture/pagination, polling, envoi `PLAYER`/`COMMAND`, `ChatPanel`, non-lus, autoscroll et indicateur de nouveaux messages, réponses, mentions, suppression auteur, masquage local, signalement, Profil/menu contextuel, ainsi que revalidation immédiate des projections UI touchées après chaque commande mutationnelle. Un `!pull 10` doit par exemple mettre à jour sans F5 le résultat Chat, les Primogemmes, Gacha/pity/garantie/Capture, Box/constellations/copies, Défi Pulls et la progression éventuelle. Aucun parcours Chat player-facing n'est encore déclaré commencé ou validé publiquement.
