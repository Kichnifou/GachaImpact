...
Version : promotion Chat/MP — dernier polish sur main, à revalidation publique
Date : 2026-09-24
Statut : CHAT/MP PUBLIC VALIDÉ ; a30ce315 sur main, dernier polish R509/MP À REVALIDER PUBLIQUEMENT
...
- **R509/R510/R517 — socle validé publiquement ; dernier polish sur main à revalider.** Signalement depuis fil live/R515, création du dossier, absence de notification immédiate, accès privé Communauté, absence de free-browse et snapshot gelé sont acquis. Sur main, le reporter voit seulement précédent/cible/suivant, avec cible jaune/or et information sur le contexte plus large ; fingerprint, stale et preuve stockée restent sur 10/1/10. Cette révision du preview reste **À REVALIDER PUBLIQUEMENT**.
- **Dernier polish Chat/MP sur main, à revalider publiquement.** La liste Conversations/Archives a le menu `⋯` partagé avec le header, sans lecture implicite ; le feedback de succès ou doublon du report dure exactement trois secondes. MODERATOR/ADMIN peuvent supprimer après confirmation le seul dossier `direct_message_reports`, avec audit sans contenu privé ; MP, conversation, notifications, blocages et amitiés sont inchangés. Les curseurs Communauté enabled/disabled sont explicites. Le menu, le feedback et la suppression restent **À REVALIDER PUBLIQUEMENT**.
- **Promotion contrôlée.** `a30ce31595dcb0bc9527943d89c028c465d27b8c` (`fix: finalize private message reporting polish`) est présent sur main ; review et main doivent converger sur le checkpoint documentaire de promotion. Cette promotion Git ne vaut ni déploiement ni validation publique.
...
- Validation technique du socle sur main et du dernier polish promu : frontend **784/784 sur 85 fichiers**, dont `ChatPanel` **63/63**, `DirectMessagePanel` **54/54** et `ModerationScreen` **13/13** ; backend **392/392 sur 55 fichiers** ; routes report **2/2** ; PostgreSQL report **7/7** et PostgreSQL Chat **26/26** isolés. Builds frontend/backend et typecheck serveur réussissent ; le lint conserve seulement deux avertissements préexistants dans `ContestScreen`, et le diff-check est propre. La campagne PostgreSQL générale parallèle n'est pas relancée dans ce polish ; son dernier résultat reste **262/289** avec 27 échecs dus au client `pg` partagé et à la dette historique du smoke exhaustif, sans les masquer comme réussis. Le vrai `GameShell`, les panneaux réels et les CSS de production ont été contrôlés à **2560×1440, 1920×1080, 1366×768 et 390×844** : preview de trois cartes, cible jaune, footer visible, feedback, menu de liste, liste/confirmation/détail Communauté. Les mesures Chromium donnent `scrollWidth = innerWidth` aux quatre tailles ; le footer mobile finit à 618,5 px dans un viewport de 844 px. Cela ne constitue ni un déploiement ni une validation publique.
...
- Le lot Chat player-facing est validé publiquement pour R890 à R897 dans leur état révisé. R504/R513/R514/R515 et la fermeture instantanée de l'éditeur sont validées publiquement. Le preview condensé, le menu de liste et la suppression de dossier constituent le dernier polish candidat à revalider.
...
- Domaine actif : **cycle Chat/MP presque clôturé : dernier polish R509, menu `⋯`, feedback trois secondes et suppression des dossiers promus sur main, à revalidation publique ciblée**.
- Prochaine étape exacte : **vérifier les déploiements déclenchés par main, puis réaliser la validation publique ciblée du dernier polish Chat/MP**. Après cette validation, le cycle actuel peut être clôturé et un nouveau domaine choisi par le propriétaire depuis le Master/roadmap. Restent explicitement reportés : médias/pièces jointes/GIF, `!mp`, Twitch, Realtime/WebSocket/SSE, sanctions/avertissements après signalement, raisons de signalement, notification au signalé après décision future, réponses privées de commandes, annonces automatiques/UI→Chat et Historique global R633/R634. Aucun domaine suivant n'est choisi ici. `PAID_INFRA_APPROVED = false`.
