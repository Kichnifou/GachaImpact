...
Version : clôture publique Chat/MP — passation vers le choix du prochain domaine
Date : 2026-09-24
Statut : CHAT/MP CLÔTURÉ PUBLIQUEMENT ; PASSATION / CHOIX DU PROCHAIN DOMAINE
...
- Domaine actif : **PASSATION / CHOIX DU PROCHAIN DOMAINE**. Le cycle Chat/MP ne porte aucun défaut ouvert. Selon la roadmap, **Missions permanentes B / A / S / Z** vient séquentiellement après Chat global et messages privés, mais ce domaine n'est ni actif ni confirmé par le propriétaire.
- Prochaine étape exacte : **ouvrir une NOUVELLE conversation ChatGPT et lui fournir, après vérification indépendante de cette promotion, le prompt court officiel de Bootstrap préparé par ChatGPT. Cette conversation commence strictement read-only : elle lit `AGENTS.md`, `.chatgpt/CHATGPT_GUIDE.md`, ce Master, `implementation-workflow.md`, `conversation-handoff.md`, `implementation-order-v1.md`, puis seulement les propriétaires du domaine pertinent ; elle vérifie le vrai GitHub (`main`, `review`, divergence et derniers commits), reformule SHA, dernière migration, dernier cycle terminé, validations publiques, défauts ouverts, prochaine étape et hors-scope, sans modifier quoi que ce soit ; elle attend ensuite le choix explicite du propriétaire. Après confirmation du prochain domaine, ouvrir une NOUVELLE conversation Codex en MODE BOOTSTRAP, jamais CONTINUITÉ.** Selon la roadmap, Missions permanentes B / A / S / Z reste seulement le prochain domaine séquentiel, non actif et non confirmé ; aucune implémentation ne commence avant cette confirmation. Restent explicitement reportés et non bloquants : médias/pièces jointes/GIF, `!mp`, Twitch/transport, Realtime/WebSocket/SSE, raisons de signalement, sanctions/avertissements/mute/ban, future notification au joueur signalé, réponses privées de commandes, annonces globales automatiques/UI→Chat, Historique global R633/R634 et autres sujets `FUTUR`. `PAID_INFRA_APPROVED = false`.
