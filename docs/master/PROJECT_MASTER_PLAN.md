...
Version : candidat Missions permanentes — Lot 1 socle backend/DB
Date : 2026-09-24
Statut : MISSIONS PERMANENTES LOT 1 CANDIDAT SUR `review` ; NON PROMU ; NON VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : Missions permanentes B/A/S/Z.** Le Lot 1 matérialise sur `review` le catalogue de 31 définitions, l’état/progression Player, le provisionnement et le moteur transactionnel central. Il s’agit d’un candidat technique : aucune UI, route player-facing, commande `!mission`, projection Profil, notification finale, intégration Twitch, migration legacy ni connexion aux producteurs métier n’est incluse ou autorisée par ce checkpoint.
- **Migration candidate 041 appliquée sur DEV.** `20260924170000_041_add_permanent_missions` ajoute les trois tables permanentes, leurs enums, contraintes, index, RLS et révocations, puis initialise les Players existants avec baseline standalone 0 et rang B actif. Elle ne crédite aucun wallet, ne lance aucun rattrapage économique et ne touche pas au Défi quotidien. Le registre Prisma et les 41 dossiers de migration sont alignés ; `prisma migrate status` est à jour.
- **Moteur candidat, non encore activé par les producteurs.** `PermanentMissionService` réutilise une `Prisma.TransactionClient` existante, lit les compteurs autoritatifs, cascade B→A→S, débloque/évalue Z et crédite chaque réussite une fois via Economy + `BusinessOperation` dans la transaction appelante. La projection interne masque tout le contenu Z avant déblocage. Le futur rattrapage des standalone existants devra appeler ce même moteur ; aucun paiement massif SQL n’a été exécuté.
- **Validation du candidat Lot 1.** Backend complet 394/394, catalogue Missions 2/2, PostgreSQL Missions 6/6 et régression PostgreSQL du provisionnement 1/1 ; typecheck, build serveur, validation Prisma et diff-check réussissent. Le lint reste vert avec seulement les deux avertissements préexistants de `ContestScreen`. La campagne PostgreSQL générale n’est pas relancée sur la DEV partagée.
...
- Base DEV : migrations 036–041 appliquées via `prisma migrate deploy` ; **41 migrations Prisma** suivies et statut à jour. La 041 est additive, initialise seulement les lignes Missions des Players existants et ne modifie directement aucun wallet, mouvement économique, import legacy, Défi quotidien, message, Notification ou réglage Privacy. `prisma migrate dev` reste historiquement inadapté à cette base partagée en raison du P3006 de la migration 010 dans le shadow database ; le workflow retenu reste `migrate deploy` puis `migrate status`.
...
- Domaine actif : **MISSIONS PERMANENTES B / A / S / Z — LOT 1 SOCLE BACKEND/DB CANDIDAT**. La cible produit R299–R339 reste documentée ; l’état physique 041 et le service central sont candidats sur `review`, non promus sur `main` et non validés publiquement.
- Prochaine étape exacte : **review indépendante ChatGPT du vrai commit GitHub `review` de ce Lot 1**. Elle doit contrôler le diff, le registre/migration 041, l’absence de paiement SQL et de raccord producteur, les invariants atomiques/idempotents, le secret Z et les preuves de test. Le Lot 2 n’est pas présenté comme autorisé avant cette review et l’arbitrage explicite du propriétaire. Restent reportés à ce futur lot : raccords producteurs, rattrapage économique standalone via le moteur, route/projection player-facing, UI Missions, `!mission`, Profil, restitution/notifications et Twitch. `PAID_INFRA_APPROVED = false`.
