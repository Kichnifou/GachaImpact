...
Version : Missions permanentes — Lot 5 validé publiquement ; polish MP et workflow candidats sur review
Date : 2026-09-25
Statut : LOTS MISSIONS 1 À 5 PROMUS ET VALIDÉS PUBLIQUEMENT ; R898 MP ET SÉCURISATION DES FINS D'ENVOI PROMUS, CŒUR DES RÉPONSES OBSERVÉ PUBLIQUEMENT MAIS CYCLE MP NON ENCORE VALIDÉ ; POLISH MP CANDIDAT SUR REVIEW, NON VALIDÉ PUBLIQUEMENT ; BOOTSTRAP STABLE ET PUBLIQUEMENT ACCEPTABLE AVEC LATENCE RÉSIDUELLE NON BLOQUANTE ; CORRECTIF RESPONSIVE BOUTIQUE `7106f39` VALIDÉ PUBLIQUEMENT
...
- **Domaine actif : polish frontend MP candidat après validation publique du Lot 5 Missions.** Les Lots 1 à 5 sont promus et validés publiquement ; le propriétaire confirme premier rang incomplet, non-terminées d'abord, navigation Missions, `!mission` et Missions Profil. Le Lot 5 active aussi Privacy `MISSIONS`. Restent non implémentés les notifications de complétion player-facing, Twitch et la migration legacy R328.
...
- **Polish MP candidat sur `review`, en attente de review indépendante puis nouveau test propriétaire.** Dans une conversation existante, l'envoi actif libère immédiatement le composer et son focus. Une seule intention suivante peut attendre, texte et réponse visibles dans un composer grisé ; elle garde sa clé UUID et ne démarre qu'après succès du précédent. Un échec libère cette intention comme brouillon éditable, garde l'envoi échoué récupérable avec sa clé et ne lance aucun envoi automatique. La navigation invalide la queue et la rend récupérable uniquement à l'origine. La confirmation de suppression se révèle par scroll minimal dans son fil propriétaire ; les actions sont dans les bulles, sans débordement horizontal. Ce polish n'est pas encore validé publiquement.
- **Contrôles locaux du polish MP candidat.** `DirectMessagePanel` 70/70, frontend complet 837/837 sur 91 fichiers, build (incluant typecheck), lint et diff-check réussissent ; seuls deux avertissements lint préexistants dans `ContestScreen` et l'avertissement de taille du bundle subsistent. Le premier lancement de la suite complète en parallèle du build a eu un timeout Chat inchangé ; `ChatPanel` isolé 63/63 puis suite complète seule 837/837 sont verts. Un harness temporaire avec vrai composant React et CSS de production a vérifié sous Edge/Chromium à 2560×1440, 1920×1080, 1366×768 et 390×844 les actions internes, l'absence de débordement horizontal, la confirmation visible, le composer queued avec réponse et le focus après promotion. Harness et captures exclus du commit. Aucun test DB n'a été exécuté pour ce diff frontend seul.
- **Outillage Codex candidat sur `review`.** Guide actualisé pour GPT-6 Sol et niveaux de réflexion ; lectures et sorties terminal ciblées ; validations proportionnées ; trois Skills repo ; scripts `verify:quick`/`verify:full` non-DB ; note locale `TASK_STATE.md` ignorée et template suivi ; sessions Codex bornées par tâche. `verify:quick` passe 5/5 étapes et `verify:full` 8/8, avec frontend 837/837 et backend non-DB 408/408. RTK 0.48.0 a été évalué sur Windows : sorties brutes/filtrées en octets `git status` 30/461 (avertissement sans hook), `git diff` 381/380, `rg` 1 318/1 318, test verbose via `rtk npx` 11 280/11 273 ; `rtk test` réduit ce test vert à 190 octets mais omet le message essentiel d'un échec « aucun fichier de test ». Le binaire d'essai a donc été désinstallé et aucune intégration Codex globale/projet n'a été activée ; QMD et Caveman ne sont pas installés par ce lot. Le validateur Python des Skills n'est pas disponible localement ; leurs trois fichiers et frontmatters sont contrôlés directement.
...
- Le cycle Chat/MP antérieur est **CLÔTURÉ PUBLIQUEMENT** : R890–R897 et R504–R517 sont acquis. R898 et la sécurisation des fins d'envoi MP sont approuvées et promues ; seules les réponses à soi/autrui et leur aperçu sont observés publiquement pour R898, sans validation publique du cycle complet. Le polish MP sur `review` attend review indépendante et test propriétaire.
...
- Domaine actif : **POLISH MP ET OUTILLAGE CODEX CANDIDATS SUR REVIEW ; LOT 5 MISSIONS PROMU ET VALIDÉ PUBLIQUEMENT**. Les Lots 1 à 5, pagination/loaders et le correctif responsive Boutique `7106f39` sont validés publiquement ; le bootstrap demeure stable et publiquement acceptable avec sa latence résiduelle non bloquante. R898 et la sécurisation MP restent promus, mais le cycle MP complet et le nouveau polish ne sont pas validés publiquement. La migration 043 appartient seulement au premier correctif MP.
- Prochaine étape exacte : **review indépendante ChatGPT des deux nouveaux commits sur `review`**. Ne pas promouvoir `main` dans cette mission. Restent non implémentés : notifications Missions player-facing, Twitch et migration legacy R328. `PAID_INFRA_APPROVED = false`.
