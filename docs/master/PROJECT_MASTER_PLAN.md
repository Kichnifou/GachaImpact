...
Version : Chat global player-facing implémenté, reviewé, promu et déployé ; validation publique à faire ; MP encore non physique
Date : 2026-09-22
Statut : PREMIER LOT PLAYER-FACING CHAT GLOBAL APPROUVÉ ET DÉPLOYÉ ; HEALTHCHECK VERT ; TEST PUBLIC PAR AXEL À FAIRE
...
- Le premier lot Chat global player-facing est **implémenté, reviewé indépendamment, approuvé et promu sur `main` par fast-forward** (chaîne `0d4211bb41b36d99ddf2e25c60a88249f908c3cf` → `2a0f38cdaf9d5fe31af05a38ecad45537c0a372c` → `964fb57db5831b5f296a2c902679d049f073c081`). Sept routes authentifiées/no-store, `ChatPanel` réel, polling HTTP, historique progressif, non-lus, envoi PLAYER, commandes, réponses publiques, replies, mentions, Profil, suppression auteur, masquage local, signalement privé, URLs sûres, XP Chat, Défi Messages et level-up sont physiques. Les corrections de review préservent l’écran central lors des refresh Chat, ordonnent PLAYER puis GAME_RESULT de level-up (+1 ms, sans doublon au replay) et synchronisent Pull/Stella avec Team, Combat quotidien, Boss et Concours. `!concours` reste en lecture seule. La migration 034 a été validée sans modification et **34 migrations Prisma sont appliquées sur DEV**. Contrôles du correctif final : 385 tests backend, 674 tests frontend, builds et typecheck réussis. MP reste non physique : aucune table, API, UI, demande, accusé de lecture, archive ou signalement MP général. Twitch est reporté, sans pont Chat réel ; Realtime est absent et le Chat global utilise le polling HTTP. Déploiements du SHA `136be89883f39c9ed323b5f0b3edaf1873f4dcf4` vérifiés par ChatGPT : Cloudflare Pages check-run `106723089962` completed/success ; Railway deployment `3a0e71b6-05a9-463d-8899-9ef0afc5cb88`, branche `main`, SUCCESS, `Application ready` et `/health` réussi. Le Chat global n’est pas encore testé ni validé publiquement par Axel. Prochaine étape : bootstrap documentaire de la nouvelle conversation ChatGPT, puis, après confirmation d’Axel, test public du Chat ; corriger tout retour avant d’ouvrir les MP R501–R522. La conversation Codex actuelle continue en CONTINUITÉ.
...
- Base DEV du checkpoint précédent : **33 migrations Prisma** à jour ; `032` demeure intacte et `20260922080000_033_fix_global_chat_content_check` corrige additivement sa contrainte de contenu interne. Les migrations sont appliquées via Prisma et vérifiées par checksum, `prisma migrate status`, contrainte, RLS et grants. Migrations 001–031 intactes. Aucun seed économique, import legacy ou reset Privacy.
...
Le [contrat Chat global V1](../specifications/global-chat-v1.md) porte les décisions validées R872–R884. Le panneau desktop repliable accueille le Chat global réel ; l’onglet MP prévu reste non physique. Les fondations serveur/DB, les commandes et leurs réponses publiques, puis le `ChatPanel` connecté ont été implémentés et reviewés indépendamment. La chaîne player-facing est promue sur `main` et déployée ; le test et la validation publique par Axel restent à faire. Twitch et son pont optionnel restent reportés ; Realtime est absent.
