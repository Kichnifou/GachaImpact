...
## Missions permanentes — Lot 3 candidat sur `review`
...
Le candidat Lot 3 ajoute les producteurs restants sans nouveau schéma. `expedition.claim` exécute le catch-up avant récompense et compteur, crédite avec `skipPermanentMissions`, puis effectue une seule passe finale pour `EXPEDITIONS_COMPLETED` et l’éventuelle métrique Moras/particules ; start et claim portent le vrai canal UI ou INTERNAL_CHAT. Le Combat quotidien suit le même ordre et réconcilie sur victoire `COMBAT_WINS`, `MORAS_EARNED` et seulement en mode MANUAL `MANUAL_COMBAT_WINS`; une défaite ne progresse aucune de ces métriques.
