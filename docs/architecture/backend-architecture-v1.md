...
Chaque complétion crée une `BusinessOperation` `permanent-mission.reward` avec clé stable Player/external key, appelle `PrismaEconomyService.credit` pour les Primogemmes, finalise l’opération et lie `rewardOperationId` à la progression avant le commit de la transaction appelante. Le verrou Player et les contraintes uniques rendent retry et concurrence sans double crédit ; un rollback externe annule ensemble complétion, opération, mouvement et solde. Les producteurs futurs devront appeler ce service seulement pour leur mutation initiale : le crédit de récompense Mission ne doit pas reboucler comme un nouveau gain producteur.

La projection interne B/A/S contient uniquement les données nécessaires aux futurs consommateurs. Tant que `zUnlockedAt` est nul, Z est réduit à son état global `LOCKED` : aucune définition, cible, progression ou récompense secrète n’est projetée. Ce lot n’expose pas de route HTTP et ne raccorde aucun producteur, UI, Chat, Profil, Notification, Twitch ou migration legacy.
