...
Le [contrat Chat global R872–R886](../specifications/global-chat-v1.md) fixe le comportement produit ; les MP gardent leur contrat Social R501–R522. Les migrations 032–035, `GlobalChatService` et `ChatCommandDispatcher` alimentent les sept routes player-facing authentifiées et le `ChatPanel` réel. La 035 appartient au candidat correctif sur `review`, sans déploiement public encore vérifié.
...
Les routes `GET/POST /api/v1/chat/messages`, `GET /api/v1/chat/unread`, `POST /api/v1/chat/read`, `DELETE /api/v1/chat/messages/:messageId`, `GET /api/v1/chat/mentions` et `POST /api/v1/chat/messages/:messageId/report` sont authentifiées, no-store et résolvent le Player serveur. `GlobalChatMention` résout les pseudos entiers côté serveur avec la normalisation Social, statut ACTIVE et blocages bidirectionnels ; les IDs client ne créent pas de mention. Le DTO ne révèle que `mentionedMe` et `repliedToMe` au lecteur concerné. `GlobalChatReport` fige la cible et dix messages avant/après pour la modération ultérieure, sans route de lecture navigateur. Un message PLAYER qui obtient réellement de l'XP fait progresser le Défi Messages dans la même transaction ; un level-up Chat publie une ligne GachaImpact sans XP ni compteur supplémentaire. Le panneau poll toutes les 2,5 secondes ouvert, toutes les 5 secondes replié pour les non-lus, et suspend le polling quand le document est caché.
