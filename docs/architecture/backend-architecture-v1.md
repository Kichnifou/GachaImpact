...
`AppearanceService` est le propriétaire unique du catalogue, des possessions et de l'équipement. `unlockCosmetic` accepte une transaction métier existante, utilise la PK `(player_id, cosmetic_id)` avec insertion idempotente, conserve source/date/provenance et n'émet une notification persistante que sur une nouvelle insertion réelle explicitement notifiable. Aucun producteur de gameplay n'est branché faute de règle de déblocage canonique. `GET /api/v1/me/appearance` est sans écriture ; `PATCH` accepte seulement le type et l'ID cosmétique ou `null` de l'acteur authentifié. Le serveur refuse type erroné, définition inactive et cosmétique non possédé. Les entrées SECRET non possédées ne sortent pas, MYSTERY masque nom, asset et condition ; VISIBLE expose la condition déclarée.

Les DTO d'identité Player de Social, Classements, Chat et MP joignent la définition d'avatar équipée dans leur lecture de liste ; aucun aller-retour par ligne. Le frontend choisit l'asset officiel équipé, l'asset de l'élément permanent, puis l'initiale si aucun élément ou si les images échouent. Le titre est projeté uniquement dans le DTO Profil. Le catalogue officiel n'est pas modifiable depuis le navigateur ; la migration 045 active RLS et retire les droits directs `anon`/`authenticated`. Aucun upload, URL externe, titre de départ, récompense économique ni intégration Twitch n'est ajouté.
