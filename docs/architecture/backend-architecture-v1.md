...
Le [contrat Chat global R872–R887](../specifications/global-chat-v1.md) fixe le comportement produit ; les MP gardent leur contrat Social R501–R522. Les migrations 032–035, `GlobalChatService` et `ChatCommandDispatcher` alimentent les huit routes player-facing authentifiées et le `ChatPanel` réel. La 035 est incluse dans le checkpoint public `7ed10b475e8f50ad71a4bac20be7c1a87f8d7516` et appliquée sur Supabase DEV. L'état courant va jusqu'aux migrations 036–038 du socle MP, soit 38 migrations Prisma suivies ; elles ne changent pas le transport HTTP incrémental du Chat global.
...
Les routes `GET/POST /api/v1/chat/messages`, `POST /api/v1/chat/updates`, `GET /api/v1/chat/unread`, `POST /api/v1/chat/read`, `DELETE /api/v1/chat/messages/:messageId`, `GET /api/v1/chat/mentions` et `POST /api/v1/chat/messages/:messageId/report` sont authentifiées, no-store et résolvent le Player serveur. `updates` reçoit en JSON la génération, une ancre `(createdAt, id)` facultative et jusqu'à 200 IDs de lignes actives déjà chargées. Liste, curseurs, updates, suppressions connues, non-lus et mémoire client restent dans les 200 lignes les plus récentes de la génération ; les lignes antérieures restent en base. Il retourne un reset au changement de génération et au dépassement de fenêtre, puis au plus 100 nouvelles lignes par réponse. `GlobalChatMention` résout les pseudos entiers côté serveur avec la normalisation Social, statut ACTIVE et blocages bidirectionnels ; les IDs client ne créent pas de mention. Le DTO projette les mentions résolues nécessaires au rendu sans exposer de relation sociale supplémentaire, ainsi que `mentionedMe` et `repliedToMe` au lecteur concerné. `GlobalChatReport` fige la cible et dix messages avant/après pour la modération ultérieure, sans route de lecture navigateur. Un message PLAYER qui obtient réellement de l'XP fait progresser le Défi Messages dans la même transaction ; un level-up Chat publie une ligne GachaImpact sans XP ni compteur supplémentaire. Le panneau utilise `updates` environ toutes les 500 ms ouvert et visible, sans overlap ; replié, il interroge les non-lus toutes les 5 secondes, suspend tout polling quand le document est caché et recharge la page récente à chaque réouverture.
...
Les routes no-store `/api/v1/me/direct-conversations` couvrent liste normale/archivée, initiation, pages de messages, envoi, acceptation, refus, blocage/déblocage, non-lus, lecture monotone, accusés et archivage personnel. Elles n'acceptent jamais l'auteur depuis le navigateur. La projection `canSend` réutilise le prédicat serveur de permission et reste neutre sur sa cause ; `blockedByMe` permet seulement à l'acteur de retirer son propre blocage. La projection normale reste limitée aux 500 derniers messages de chaque conversation ; les curseurs plus anciens sont refusés sans supprimer les lignes. Le timestamp partagé avance uniquement lorsqu'un nouveau curseur interne est accepté et que les accusés sont actifs.

Le frontend porte un propriétaire `useDirectMessages` dans le panneau `Chat | MP`, sans nouvelle route de navigation. Il maintient listes normale/archivée, fil sélectionné, pages, non-lus, invalidations de mutation et révisions anti-réponse obsolète. Les lectures liste/fil/non-lus et mutations sont sérialisées par catégorie ; le retry d'envoi conserve sa clé UUID et son brouillon. Le polling HTTP est de 1,2 s pour un fil actif, 3 s pour la liste MP active et 5 s pour les seuls badges lorsque Chat est actif ou le panneau replié, avec reprise immédiate au focus/retour visible. Ouvrir seulement l'onglet MP ne marque rien lu ; seule une conversation ouverte et positionnée au récent avance son curseur. Aucun `!mp`, pont Twitch, Realtime, WebSocket ou SSE.
