...
Le candidat avancé ajoute `PATCH .../messages/:messageId`, `POST .../:messageId/delete` et `POST .../:messageId/restore`. Le service vérifie participant, conversation, auteur et état sous verrou de ligne, puis conserve ID, `submissionOrder` et `createdAt`. L'édition normalise le même texte Unicode 1..1 000 que l'envoi et renseigne `editedAt`; suppression et restauration basculent `deletedAt`/`restoredAt`. Les trois opérations sont rejouables par clé via `BusinessOperation`, sans nouveau message ni non-lu. La projection force `content = null` pour tout tombstone, même à l'auteur. Restore exige encore le contenu et moins de 500 messages plus récents. L'insertion suivante calcule la borne des 500 par l'index `(conversation_id, submission_order DESC)` et purge en SQL uniquement les tombstones antérieurs.

Dans le client, le menu `⋯` n'existe que sur ses propres messages. L'édition reste inline et multiligne, la suppression demande confirmation, le tombstone garde sa position et Restore réutilise la même ligne. Le cache protège une mutation optimistic d'une projection de polling obsolète, rollbacke une erreur déterministe et conserve la clé d'une erreur ambiguë pour `Réessayer`. Un dernier message supprimé porte l'aperçu `Message supprimé` et ne montre aucun statut de livraison. Le signalement MP et tout accès administrateur demeurent absents ; un futur signalement devra figer son propre snapshot avant toute purge.
