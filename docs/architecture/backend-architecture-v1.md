...
`EventService` est l’unique orchestration des fondations mensuelles. Il reçoit une horloge injectable, dérive la business date et les deux débuts de mois en `Europe/Paris`, sélectionne la définition active par mois puis upsert l’édition unique définition/année. La définition est data-driven ; aucune branche métier propre à septembre ou à un autre Festival n’existe. Le snapshot d’édition fige l’identité utile aux futures évolutions de configuration.
