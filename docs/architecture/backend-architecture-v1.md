...
## Vertical backend physique candidat Event Lot 4 — Jeu C

`GET /api/v1/me/event/game-c/recipients` recherche des Players éligibles par pseudo avec pagination serveur de vingt, sans exposer de cible arbitraire non résolue. Le prédicat Social partagé vérifie statut `ACTIVE`, blocages dans les deux sens et permission MP `PUBLIC | FRIENDS | PRIVATE` contre une amitié active. L'envoi reçoit exclusivement le Player ID sélectionné, un texte nettoyé borné techniquement à 500 caractères et une clé UUID ; cette limite reprend la taille des descriptions joueur déjà acceptées par l'API, sans créer une règle produit.
