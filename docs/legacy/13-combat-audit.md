...
# Appendice présentation physique 0.88 — hiérarchie et feedback finalisés

Le candidat 0.88 conserve intégralement le moteur 0.86/0.87. L'ordre de présentation devient `Ennemis`, les quatre cartes ennemies, une command bar intermédiaire, puis `Votre formation` avec ses actions et les quatre cartes joueur. La command bar affiche uniquement `Rencontre du jour` à gauche, Combattre au centre, puis chance, `Détails du calcul →` et `Mode : Manuel/Auto` à droite ; sa seconde ligne reste réservée aux résultats et erreurs. La date métier reste dans le DTO mais disparaît de cette présentation.

Combattre demeure à position stable dans tous les états. Lorsqu'il est indisponible, il est gris sombre, atténué, neutre, sans glow et utilise `not-allowed`. Les restitutions normales deviennent `Formation prête.`, `Sélectionnez 4 personnages disponibles.`, `Bloqué · Moins de 4 personnages disponibles.` et `Défaite · 4 personnages KO jusqu’à demain.` ; la victoire conserve son gain réel. Dans Quotidiennes, TODO devient `Prêt à combattre` sans détail redondant.

La modale de chance distingue visuellement Base, Bonus, Malus et Résultat après clamp, sans exposer `rawHalfPoints` ni le terme player-facing `Brut`. La fiche partagée remplace son lien d'état par `Statistiques →` ; la modale secondaire conserve uniquement statut OK/💀 KO, combats, victoires, défaites et taux, sans disponibilité du lendemain. Son artwork plein-hauteur couvre structurellement toute sa région utile sans scale arbitraire ni bande morte. Aucun gameplay, calcul, RNG, mode Auto, KO, économie, idempotence, backend, schéma ou migration ne change.

# Appendice physique 0.91 — Boss mensuel

Le candidat 0.91 matérialise le Boss sans modifier les décisions R390 à R450. La migration additive `20260913160000_016_add_monthly_boss` crée `monthly_bosses`, les quatre slots persistants, les attaques et leurs membres snapshotés, les participations, statistiques lifetime et récompenses uniques. Toutes ces tables activent la RLS et révoquent les rôles navigateur ; aucune instance courante n’est seedée en SQL.

`MonthlyBossService` crée l’instance du mois sous verrou transactionnel au démarrage mensuel ou au premier accès, selon le calendrier Europe/Paris. Le nom calendaire, la base adaptative, la variation entière uniforme ±15 % arrondie aux 10 000 PV et la résistance uniforme injectable sont persistés. Une attaque reçoit uniquement `bossId` et clé d’idempotence, reverrouille Player et Boss, revalide quatre possessions distinctes et actives, snapshotte chaque contribution puis applique les dégâts. Une course létale ne produit qu’un coup final ; le perdant reçoit `BOSS_DEFEATED` sans consommation quotidienne.
