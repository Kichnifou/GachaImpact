...
**R911 — révision physique du snapshot.** La migration 044 ajoute `banner_rotations.generation_vote_snapshot` en JSONB nullable. Une transaction de fermeture durable inscrit sur la rotation source son ID, l'instant et le pool éligible des candidats 5★ avec les compteurs exacts y compris zéro. Une seconde transaction utilise cette capture pour créer la suivante et son snapshot final, incluant le personnage/source du slot communautaire ; après échec ou redémarrage, elle reprend la fermeture existante. L'éligibilité pendant la semaine reste dynamique : le pool est figé seulement à la génération suivante. La première rotation sans source garde `NULL`, comme les rotations antérieures sans capture ; leurs candidats à zéro vote ne sont pas reconstitués. Historique n'expose que les snapshots finaux et affiche sinon « Snapshot détaillé des votes indisponible pour cette rotation antérieure. » Le lot est candidat `review`, sans validation publique.
