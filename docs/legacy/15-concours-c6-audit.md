...
Le candidat 0.92 matérialise R526 à R593 sans recréer les cinq statistiques existantes. `ContestService` possède le thème journalier Europe/Paris, le lobby global unique, l’éligibilité par possession 5★ C6 active, Ready, lancement explicite, snapshots, bots, ordre, tours, timeouts, remplacements, spectateurs, soutien, victoire, annulations et reprise. Le délai lobby correspond à dix minutes d’inactivité et est rafraîchi centralement par les mutations réussies des participants ; GET, polling, replay idempotent et membership spectateur ne le prolongent pas. Ready reste individuel : join, leave et remove conservent les autres états, tandis qu’un changement de Légende réinitialise seulement son joueur. Les mutations sensibles passent par des transactions sérialisables, un verrou global Concours, `BusinessOperation` et une clé d’idempotence stable ; le scheduler et chaque accès réutilisent la même réconciliation persistante.
...
Le frontend réel expose états vide/lobby/running/result, contrôles selon rôle, countdowns informatifs, soutien sélectionné, dernier résultat, `Mes Légendes` personnel et historique public FINISHED à dix entrées par page avec détail interprété. Le picker personnel affiche la statistique du thème `/20`, se recale sur la Légende persistée et reste obligatoire avant un nouveau lobby. Résultat et détail affichent toutes les promotions de titre projetées depuis les événements durables ; liste et détail conservent vainqueur, classement, scores, récompenses, ordre, horaires, départs, remplacements et soutiens sans dump JSON. Le polling actif de deux secondes est complété par focus et retour visible, avec cleanup des listeners. Les `/20` exacts restent dans les contrôles et consultations personnels et ne sont pas exposés dans les cartes ou historiques publics. La présence passive, Social, Realtime, chat/Twitch et l’ancien historique legacy restent reportés.

Le correctif final de conformité dérive les titres dans leur forme française complète (`de Bronze`, `d’Argent`, `d’Or`, `de Platine`) et conserve `Idôle`. La projection autoritaire expose un rang live départagé par score, ordre de tour puis slot sans déplacer les cartes, tandis que le rang final reste distinct. Les rangs de titre disposent de styles stables Bronze à Platine. Le retrait dédié d’un spectateur autorise l’organisateur ou un ADMIN, ferme immédiatement sa fenêtre de soutien éventuelle et ne touche ni daily ni place participante ; les traces existantes empêchent également tout ancien participant de devenir spectateur actif. Le retrait administratif exceptionnel du lobby ne compte pas dans l’anti-abus organisateur. Après lancement, le transfert d’organisateur suit l’ordre de tour. Une Basic automatique gagnante au troisième timeout termine avant tout remplacement. Les remplacements conservent les snapshots humains en stockage, projettent une identité Bot générique en direct et restituent l’identité d’origine dans l’historique. Enfin, la réconciliation annule un lobby resté ouvert après le changement de journée sans affecter un Concours déjà lancé, et la toolbar affiche toujours le thème figé du Concours actif.

# Appendice physique 0.93 — stabilité runtime et consultation personnelle

La stabilisation 0.93 conserve intégralement R526 à R593. Le scheduler Concours devient séquentiel, ses rejets sont gérés localement et la réconciliation est coalescée dans chaque processus. Une prélecture légère évite une transaction lorsqu’aucune phase n’est due ; lorsqu’une transition l’est, le service tente le verrou inter-processus, recharge l’état live et revalide sous verrou. Les mutations n’attendent plus un rattrapage complet de tours automatiques et les projections live n’embarquent plus l’historique détaillé. Aucun délai, score, RNG, récompense, daily, titre, règle de remplacement ou garantie d’idempotence n’est modifié.

Le polling web d’une partie active n’autorise qu’une lecture en vol, fusionne polling/focus/retour visible, s’arrête lorsque l’onglet est caché et reprend avec un backoff borné après indisponibilité. Une réponse de lecture antérieure à une mutation confirmée ne peut plus republier un score ou une phase obsolète. Les actions affichent immédiatement leur état pending, bloquent synchroniquement un double clic et réutilisent la même clé lorsqu’une réponse réseau reste ambiguë.

`Mes Légendes` reste strictement personnel et affiche au plus trois Légendes par page. Les opérations suivent `filtrer → trier → paginer` avec recherche de nom insensible à la casse et aux accents, filtre Élément, ordre Nom croissant par défaut, direction réversible et tris Nom, Puissance totale, statistique du thème, participations ou victoires. La Puissance totale est uniquement la somme d’affichage des cinq statistiques C6, au plus 100 ; elle ne change jamais les points de base. Les égalités sont départagées par nom puis ordre stable. Les statistiques `/20` ne quittent pas cette consultation personnelle.

L’Historique conserve exactement dix Concours FINISHED par page et un détail complet distinct. Sur desktop, trois emplacements Légendes ou dix lignes d’historique maintiennent la position du footer même pour une page partielle ou vide ; les emplacements sans donnée restent purement structurels et non accessibles.

# Appendice physique 0.94 — réactivité et feedback de score

L’écran Concours revalide désormais sa projection dès son entrée. Son polling mono-vol reste à environ deux secondes avec un Concours actif et à environ trois secondes sans actif, uniquement tant que cet écran et l’onglet sont visibles ; focus et retour visible fusionnent avec la lecture en cours. Les succès Stella et Pull invalident également la projection via le même coordinateur afin qu’une nouvelle Légende C6 ou un lobby distant apparaisse sans rechargement manuel.

La projection active charge au plus un `ContestEvent` pertinent par un `select` minimal newest-first. `latestScoreChange` expose seulement `eventId`, slot bénéficiaire, points, type et date pour `TURN_PLAYED`, `BOT_TURN_PLAYED`, `TURN_AUTO_BASIC` ou `SUPPORT_PLAYED`. L’événement déjà présent au premier rendu est considéré vu ; chaque nouvel identifiant affiche une seule fois `+0` ou `+N` sur le participant ciblé, y compris un Bot ou la cible d’un soutien. L’historique événementiel complet reste absent du polling live.

Les quatre cartes participant possèdent une hauteur extérieure fixe par breakpoint, gardent le portrait compact sur desktop court et laissent le body Concours défiler si nécessaire. Le fallback d’asset retrouve le portrait depuis le nom de Légende lorsqu’un snapshot est vide ou invalide. Noms, scores, rangs, actions et footer ne sont plus coupés ; les dix emplacements spectateurs conservent leur hauteur et séparent le pseudo du bouton `Retirer`.

# Appendice physique 0.95 — actions en cartes et feedback partagé

Pendant un Concours RUNNING, la région d’action inférieure est supprimée. Basic/Risque apparaissent en overlay absolu uniquement sur la carte du viewer autorisé à jouer ; le spectateur sélectionné obtient de la même manière une cible Soutenir sur chacune des quatre cartes, bots compris. Les boutons restent focusables et directement visibles sans hover sur tactile, sans modifier les dimensions des cartes. Le bandeau garde une zone de statut compacte et stable. Le libellé player-facing de la liste devient `Spectateurs` et la suppression organisateur utilise une croix accessible dans une colonne réservée, sans recouvrir le pseudo.

La projection active devient `recentScoreChanges` : elle sélectionne uniquement les champs nécessaires des huit événements les plus récents parmi `TURN_PLAYED`, `BOT_TURN_PLAYED`, `TURN_AUTO_BASIC` et `SUPPORT_PLAYED`, puis les restitue du plus ancien au plus récent. Chaque client prend la première fenêtre comme baseline, déduplique par `eventId` et joue tous les nouveaux gains dans une file séquentielle, y compris `+0`, les bots et la cible d’un soutien. La première projection fraîche après reprise d’un onglet caché réamorce le baseline afin de ne pas rejouer une série ancienne. Aucun historique complet ni relation lourde ne revient dans le polling live.
