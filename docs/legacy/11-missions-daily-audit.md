...
## Missions permanentes — Lot 1 promu et Lot 2 candidat
...
Le provisionnement crée l’état Player et les 31 lignes dans la même transaction : B actif, A/S verrouillés et Z globalement verrouillé. Depuis le candidat Lot 2, un nouveau Player reçoit aussi immédiatement son marqueur R301 : aucune carrière antérieure ne lui est inventée. La migration additive 042 ajoute aux états existants `standalone_catchup_completed_at`, nullable et chronologiquement postérieur à l’initialisation ; elle ne crédite aucun wallet, ne complète aucune Mission et ne lance aucun import.

`PermanentMissionService` reste transaction-local. Le candidat Lot 2 ajoute une réconciliation bornée aux métriques du producteur et un catch-up standalone R301 exactly-once. Ce catch-up verrouille le Player, crée une opération SYSTEM dédiée, réconcilie tous les compteurs historiques par le vrai moteur, crédite via Economy puis écrit son marqueur dans la même transaction ; une erreur annule ensemble marqueur, progressions, opérations et crédits. Les récompenses conservent durablement le trigger, le canal et le contexte `STANDALONE_CATCHUP` ou `CURRENT_ACTION`.

Recroisements physiques Lot 2 : Chat avance seulement `COUNTED_MESSAGES` après le vrai message XP-counted ; XP relit `PLAYER_LEVEL` ; Gacha regroupe `PULLS`, possessions distinctes 4★/5★, C6 et ses gains économiques après l’état final ; Stella propage UI ou INTERNAL_CHAT et relit C6 ; Economy relit uniquement Moras et particules de l’élément personnel après un vrai crédit ; la Banque relit Moras après intérêt positif avec canal SYSTEM. Dépôt/retrait, échange/transfert de particules, commandes Chat, cooldown, replay et action refusée ne deviennent pas des gains. Une garde interne exclut les crédits Primogemmes des récompenses Missions du hook Economy et empêche toute récursion.
