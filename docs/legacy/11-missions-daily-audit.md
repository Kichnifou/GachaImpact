...
## Missions permanentes — Lots 1 à 4 validés publiquement ; Lot 5 promu, validation publique en attente
...
Le provisionnement crée l’état Player et les 31 lignes dans la même transaction : B actif, A/S verrouillés et Z globalement verrouillé. Depuis le Lot 2 promu, un nouveau Player reçoit aussi immédiatement son marqueur R301 : aucune carrière antérieure ne lui est inventée. La migration additive 042 ajoute aux états existants `standalone_catchup_completed_at`, nullable et chronologiquement postérieur à l’initialisation ; elle ne crédite aucun wallet, ne complète aucune Mission et ne lance aucun import.

`PermanentMissionService` reste transaction-local. Le Lot 2 promu ajoute une réconciliation bornée aux métriques du producteur et un catch-up standalone R301 exactly-once. Ce catch-up verrouille le Player, crée une opération SYSTEM dédiée, réconcilie tous les compteurs historiques par le vrai moteur, crédite via Economy puis écrit son marqueur dans la même transaction ; une erreur annule ensemble marqueur, progressions, opérations et crédits. Les récompenses conservent durablement le trigger, le canal et le contexte `STANDALONE_CATCHUP` ou `CURRENT_ACTION`.
...
Le Lot 4, promu et validé publiquement, expose la projection personnelle par `GET /api/v1/me/missions` et par le vrai écran `Activités > Missions`. Le premier GET d’un standalone dont le marqueur est nul exécute R301 exactly-once puis projette l’état confirmé ; les consultations suivantes sont essentiellement projectives. B/A/S exposent neuf missions dans l’ordre canonique et Z verrouillé ne sérialise que son statut global, sans définition secrète. Le frontend charge à la demande, rafraîchit Ressources seulement lorsqu’un catch-up vient d’être appliqué et ne propose aucune action sur les Missions automatiques. Le test public confirme B/A/S, secret Z, changement de rang et responsive ; la vérification DB après ouverture conserve une opération de catch-up et 19 récompenses historiques, sans replay R301.

Le Lot 5 promu matérialise les recroisements R335/R336 sans nouvelle décision : `!mission`, ses rangs et l’alias compatible `resume` réutilisent la projection personnelle et le Défi quotidien propriétaires ; les réponses longues restent confiées au split atomique du Chat. Le Profil ajoute une rubrique Missions lazy. Le propriétaire passe par la route personnelle avec R301 exactly-once ; un tiers passe par `GET /api/v1/players/:playerId/missions`, après Privacy `MISSIONS`, sur une projection persistée strictement read-only. Aucune lecture tierce ne déclenche catch-up, réconciliation, récompense, opération ou mouvement. Activités et Profil partagent l’auto-sélection B→A→S→Z par statuts et la partition stable non terminées puis terminées. Notifications player-facing, Twitch et import legacy restent hors du lot.
