...
import { closeBannerVoteSnapshot, generationVoteSnapshot, readClosedVoteSnapshot, selectBannerFeatured, type GachaCharacter } from '../src/domain/gacha/gacha.js';
...
describe('generationVoteSnapshot', () => {
  it('records all eligible candidates, including zero votes, from the generation pool', () => {
    ...
    const closed = closeBannerVoteSnapshot('prior', new Date('2026-09-20T22:00:00Z'), pool, previous, [{ characterId: 'five-4', votes: 3 }]);
    const snapshot = generationVoteSnapshot(closed, selected);
  ...
  it('keeps the random fallback when no eligible vote exists', () => {
    const closed = closeBannerVoteSnapshot('prior', new Date(0), pool, new Set(), []);
    const selected = selectBannerFeatured(pool, new Set(), [], roll);
    expect(generationVoteSnapshot(closed, selected)).toMatchObject({ selectionSource: 'RANDOM_FALLBACK', selectedCharacterId: selected[3]!.character.id });
  });

  it('rejects a malformed persisted closure instead of silently recapturing votes', () => {
    expect(() => readClosedVoteSnapshot({ closedVoteSnapshot: { state: 'CLOSED', sourceRotationId: 'wrong' } }, 'prior')).toThrow('Invalid closed banner vote snapshot');
  });
});
