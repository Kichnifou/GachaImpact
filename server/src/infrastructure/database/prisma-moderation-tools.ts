...
export class PrismaModerationTools implements ModerationTools {
  ...
  public async setXp(identity: AuthenticatedIdentity, targetPlayerId: string, input: Parameters<ModerationTools['setXp']>[2]) {
    const actor = await this.getCurrentPlayer.execute(identity); const target = await this.authorizeTarget(actor.id, targetPlayerId, 'super')
  ...
  private async authorizeTarget(actorPlayerId: string, targetPlayerId: string, purpose: 'state' | 'resource' | 'super' | 'roles') {
    ...
    if ((purpose === 'super' && !permissions.capabilities.superTools) || (purpose === 'roles' && !permissions.capabilities.canManageTesters)) throw forbidden()
  ...
  private async permissionsFor(playerId: string): Promise<ModerationPermissionsDto> {
    const rows = await this.database.playerRoleAssignment.findMany({ where: { playerId, revokedAt: null }, select: { role: true }, orderBy: { grantedAt: 'asc' } }); const roles = rows.map(({ role }) => role).filter((role): role is ModerationRole => role === 'MODERATOR' || role === 'TESTER' || role === 'ADMIN'); const superTools = roles.includes('ADMIN'); const selfResourceTools = superTools || roles.includes('TESTER'); return { roles, capabilities: { moderationAccess: selfResourceTools, selfResourceTools, superTools, canSelectPlayers: superTools, canManageTesters: superTools } }
  }
