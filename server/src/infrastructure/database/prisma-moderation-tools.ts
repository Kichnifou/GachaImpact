...
import { appearanceSelect, avatarAssetPath } from '../../application/appearance/appearance-service.js'
...
export class PrismaModerationTools implements ModerationTools {
  ...
  public async listPlayers(identity: AuthenticatedIdentity, query: ModerationPlayerListQuery) {
    ...
    const rows = await this.database.player.findMany({
      ...
      select: {
        ...
        displayName: true,
        elementKey: true,
        equippedAvatarCosmetic: appearanceSelect.equippedAvatarCosmetic,
    ...
    const players: ModerationPlayerDto[] = rows
      .map((row) => ({
        ...
        avatarAssetPath: avatarAssetPath(row),
  ...
  private async authorizeTarget(actorPlayerId: string, targetPlayerId: string, purpose: 'state' | 'resource' | 'gameplay' | 'roles') {
    ...
    if (purpose === 'state' && (self ? !(permissions.capabilities.selfResourceTools || permissions.capabilities.selfGameplayTools || permissions.capabilities.superTools) : !permissions.capabilities.superTools)) throw forbidden()
  ...
  private async permissionsFor(playerId: string): Promise<ModerationPermissionsDto> {
    const rows = await this.database.playerRoleAssignment.findMany({ where: { playerId, revokedAt: null }, select: { role: true }, orderBy: { grantedAt: 'asc' } }); const roles = rows.map(({ role }) => role).filter((role): role is ModerationRole => role === 'MODERATOR' || role === 'TESTER' || role === 'ADMIN'); const superTools = roles.includes('ADMIN'); const testerTools = roles.includes('TESTER'); const communityModeration = roles.includes('MODERATOR') || superTools; const selfResourceTools = superTools || testerTools; const selfGameplayTools = superTools || testerTools; return { roles, capabilities: { moderationAccess: selfResourceTools || selfGameplayTools || communityModeration, communityModeration, selfResourceTools, selfGameplayTools, superTools, canSelectPlayers: superTools, canManageTesters: superTools } }
  }
