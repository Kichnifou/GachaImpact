...
export async function registerChatRoutes(app: FastifyInstance, options: Options) {
  ...
  app.post('/api/v1/chat/updates', config, request => {
    const body = parse(updates, request.body);
    if (Boolean(body.cursorId) !== Boolean(body.cursorCreatedAt)) throw new AppError('Curseur Chat invalide.', 400, 'VALIDATION_ERROR');
    return options.service.updates(requireAuthenticatedIdentity(request), body.generation, body.cursorId && body.cursorCreatedAt ? { id: body.cursorId, createdAt: body.cursorCreatedAt } : undefined, body.knownIds);
  });
