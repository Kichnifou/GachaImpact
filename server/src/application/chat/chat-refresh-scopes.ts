export const chatRefreshScopes = [
  ...
  'expedition', 'dailyCombat', 'monthlyBoss', 'event', 'giftCodes', 'notifications',
] as const;
...
export function scopesForOperation(type: string): ChatRefreshScope[] {
  if (type === 'gacha.pull') return ['resources', 'gacha', 'box', 'inventory', 'dailyChallenge', 'progression'];
  ...
  if (type === 'box.stella.use') return ['box', 'inventory', 'resources'];
