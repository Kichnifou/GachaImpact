...
Les portraits de personnages suivent ce même contrat UI : aucune initiale de Player ou de personnage ne sert de fallback d’image.
...
13. Toute intervention qui modifie au moins un fichier doit, après les validations pertinentes et le contrôle du périmètre, créer un commit propre puis le pousser normalement sur la branche permanente `review`, sauf instruction explicite `local uniquement`. Ne jamais pousser implicitement sur `main`, ne jamais force-push, préserver sans réécriture les commits parallèles `docs/Story/**`, traiter chaque correction de review dans un commit séparé et rendre un worktree propre après le push. Le déroulement complet appartient à `docs/process/implementation-workflow.md`.
