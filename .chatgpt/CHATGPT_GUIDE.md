...
Avant chaque prompt Codex, ChatGPT indique au propriétaire, **hors du bloc de prompt à copier**, `Codex recommandé : <modèle> — <niveau de réflexion>`. Cette recommandation est obligatoire et applique d'abord une économie de modèle : choisir le modèle le moins coûteux estimé suffisamment capable de réussir correctement le lot, sans sélectionner automatiquement le plus puissant. Le niveau de réflexion est un second levier : Medium convient souvent par défaut ; High ou très élevé est réservé aux problèmes qui le justifient. Diminuer la réflexion ne compense jamais un modèle insuffisant, et un gros modèle ne s'emploie pas par habitude.

À titre d'exemple, un modèle léger ou économique convient aux microfix, CSS/polish, documentation, tests ciblés et corrections localisées déjà diagnostiquées ; un modèle intermédiaire convient à un nouveau lot sérieux, plusieurs systèmes liés, un domaine bien spécifié, un Bootstrap documentaire ou un refactor transverse raisonnable ; un modèle premium est réservé à une architecture incertaine, une concurrence complexe non diagnostiquée, un problème transverse important ou une investigation que l'intermédiaire ne résout pas correctement. Terra, Sol ou Astra peuvent illustrer cette hiérarchie si ces familles sont disponibles, sans devenir une règle durable : l'offre et les coûts évoluent. Ne pas inventer de multiplicateurs, ratios de tokens ou promesses de coût.

## Économie de contexte et preuves

L'ordre d'économie est : choisir le modèle le moins coûteux suffisamment capable ; utiliser CONTINUITÉ plutôt qu'un Bootstrap inutile ; limiter lectures, logs, captures et comptes rendus répétitifs ; garder des tests proportionnés au risque ; puis ajuster le niveau de réflexion à la difficulté réelle. Économiser le contexte signifie réduire le bruit, avec les mêmes garanties utiles. Ne pas répéter intégralement un compte rendu, un checkpoint, une liste de fichiers ou une explication déjà acquise. Préférer des commandes ciblées, recherches précises, extraits, résumés de diff, compteurs et statuts ; ne restituer un log volumineux que si son contenu complet sert réellement au diagnostic.
